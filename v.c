...
string V_c_type_definitions(V *v) {
  ...
  for (int l = 0; l < keys_tmp45.len; l++) {
    ...
    array_Var tmp50 = t.fields;
    ;
    for (int tmp51 = 0; tmp51 < tmp50.len; tmp51++) {
      Var field = ((Var *)tmp50.data)[tmp51];
    ...
    if (only_builtin_fields) {
      ...
      _PUSH(&top_types, (t), tmp54, Type);

      continue;
    };

    _PUSH(&types, (t), tmp55, Type);
  };
...
string types_to_c(array_Type types, Table *table) {
  ...
  array_Type tmp57 = types;
  ;
  for (int tmp58 = 0; tmp58 < tmp57.len; tmp58++) {
    Type t = ((Type *)tmp57.data)[tmp58];
    ...
    array_Var tmp60 = t.fields;
    ;
    for (int tmp61 = 0; tmp61 < tmp60.len; tmp61++) {
      Var field = ((Var *)tmp60.data)[tmp61];
...
void sort_structs(array_Type *types) {
  ...
  for (int i = 0; i < types->len; i++) {
    ...
    int tmp64 = 0;
    ;
    for (int tmp65 = tmp64; tmp65 < i; tmp65++) {
      int j = tmp65;
...
string Parser_bterm(Parser *p) {
  ...
  if (tok == main__Token_eq || tok == main__Token_gt || tok == main__Token_lt ||
    ...
    if (p->is_sql) {
      ...
      _PUSH(&p->sql_params, (sql_param), tmp130, string);

      _PUSH(&p->sql_types, (typ), tmp131, string);
...
string Parser_struct_init(Parser *p, string typ, bool is_c_struct_init) {
  ...
  if (peek == main__Token_colon || p->tok == main__Token_rcbr) {
    ...
    while (p->tok != main__Token_rcbr) {
      ...
      _PUSH(&inited_fields, (field), tmp287, string);
    ...
    for (int i = 0; i < tmp288.len; i++) {
      ...
      if (string_starts_with(field_typ, tos2((byte *)"map_"))) {
        ...
        _PUSH(&inited_fields, (field.name), tmp290, string);
    ...
    array_Var tmp293 = T.fields;
    ;
    for (int i = 0; i < tmp293.len; i++) {
      Var ffield = ((Var *)tmp293.data)[i];
...
Type Table_find_type(Table *t, string name_) {
  ...
  Type tmp48 = {0};
  bool tmp49 = map_get(t->typesmap, name, &tmp48);
...
bool Parser_satisfies_interface(Parser *p, string interface_name, string _typ,
  ...
  array_Fn tmp54 = int_typ.methods;
  ;
...
bool Table_is_interface(Table *table, string name) {
  ...
  Type tmp56 = {0};
  bool tmp57 = map_get(table->typesmap, name, &tmp56);
