...
#define compiler__help_text                                                    \
  tos3("Usage: v [options/commands] [file.v | directory]\n\n   When V is run " \
       ...
       "management commands:\n  search  keywords  Search the "                 \
       "https://vpm.vlang.io/ module repository for matching modules and "     \
...
array array_repeat(array a, int nr_repeats) {
  if (nr_repeats < 0) {
    v_panic(tos3("array.repeat: count is negative (count == nr_repeats)"));
  };
...
bool print_backtrace_skipping_top_frames_linux(int skipframes) {
  ...
  return 1;
#else
  println(tos3("backtrace_symbols_fd is missing, so printing backtraces is not "
               "available.\n"));
  println(
      tos3("Some libc implementations like musl simply do not provide it."));
#endif
  ;
#endif
  ;
...
int os__vpclose(void *f) {
  ...
  return _pclose(f);
#else
  _V_MulRet_int_V_bool _V_mret_1464_ret__ =
      os__posix_wait4_to_exit_status(pclose(f));
  int ret = _V_mret_1464_ret__.var_0;
  return ret;
#endif
  ;
}
...
#ifndef _WIN32
  _V_MulRet_int_V_bool _V_mret_1583_pret_is_signaled =
      os__posix_wait4_to_exit_status(ret);
  int pret = _V_mret_1583_pret_is_signaled.var_0;
  bool is_signaled = _V_mret_1583_pret_is_signaled.var_1;
  if (is_signaled) {
...
time__Time time__parse(string s) {
  Option_int tmp3 = string_index(s, tos3(" "));
  int pos;
  if (!tmp3.ok) {
...
Option_int time__days_in_month(int month, int year) {
  ...
  int tmp37 = OPTION_CAST(int)(res);
  return opt_ok(&tmp37, sizeof(int));
}
string time__Time_get_fmt_time_str(time__Time t, time__FormatTime fmt_time) {
  ...
  time__FormatTime tmp38 = fmt_time;

  return (
      (tmp38 == time__time__FormatTime_hhmm12)
          ? (_STR("%d:%02d %.*s", hour, t.minute, tp.len, tp.str))
          : ((tmp38 == time__time__FormatTime_hhmm24)
                 ? (_STR("%02d:%02d", t.hour, t.minute))
                 : ((tmp38 == time__time__FormatTime_hhmmss12)
                        ? (_STR("%d:%02d:%02d %.*s", hour, t.minute, t.second,
                                tp.len, tp.str))
                        : ((tmp38 == time__time__FormatTime_hhmmss24)
...
string time__Time_get_fmt_date_str(time__Time t,
  ...
  time__FormatDate tmp41 = fmt_date;

  time__FormatDelimiter tmp42 = fmt_dlmtr;

  return string_replace(
      ((tmp41 == time__time__FormatDate_ddmmyy)
           ? (_STR("%02d|%02d|%.*s", t.day, t.month, year.len, year.str))
           : ((tmp41 == time__time__FormatDate_ddmmyyyy)
                  ? (_STR("%02d|%02d|%d", t.day, t.month, t.year))
                  : ((tmp41 == time__time__FormatDate_mmddyy)
                         ? (_STR("%02d|%02d|%.*s", t.month, t.day, year.len,
                                 year.str))
                         : ((tmp41 == time__time__FormatDate_mmddyyyy)
                                ? (_STR("%02d|%02d|%d", t.month, t.day, t.year))
                                : ((tmp41 == time__time__FormatDate_mmmd)
                                                     ...
                                                     : ((tmp41 ==
                                                         time__time__FormatDate_yyyymmdd)
                                                            ...
                                                            : (_STR(
                                                                  ...
                                                                  fmt_date)))))))))),
      tos3("|"),
      ((tmp42 == time__time__FormatDelimiter_dot)
           ? (tos3("."))
           : ((tmp42 == time__time__FormatDelimiter_hyphen)
                  ? (tos3("-"))
                  : ((tmp42 == time__time__FormatDelimiter_slash)
                         ? (tos3("/"))
                         : ((tmp42 == time__time__FormatDelimiter_space)
                                ? (tos3(" "))
...
time__Time time__unix(int abs) {
  ...
  _V_MulRet_int_V_int_V_int _V_mret_35_year_month_day =
      time__calculate_date_from_offset(day_offset);
  int year = _V_mret_35_year_month_day.var_0;
  int month = _V_mret_35_year_month_day.var_1;
  int day = _V_mret_35_year_month_day.var_2;
  _V_MulRet_int_V_int_V_int _V_mret_45_hr_min_sec =
      time__calculate_time_from_offset(abs % time__seconds_per_day);
  int hr = _V_mret_45_hr_min_sec.var_0;
  int min = _V_mret_45_hr_min_sec.var_1;
  int sec = _V_mret_45_hr_min_sec.var_2;
  ...
  while (1) {
    ...
    if (tmp18 == compiler__compiler__TokenKind_key_import) {
      ...
      if (p->tok == compiler__compiler__TokenKind_assign) {
        compiler__Parser_next(p);
        g = string_add(g, tos3(" = "));
        _V_MulRet_string_V_string _V_mret_2295___expr =
            compiler__Parser_tmp_expr(p);
        string expr = _V_mret_2295___expr.var_1;
        g = string_add(g, expr);
      };
...
compiler__TypeCategory compiler__key_to_type_cat(compiler__TokenKind tok) {
  compiler__TokenKind tmp41 = tok;

  if (tmp41 == compiler__compiler__TokenKind_key_interface) {
    return compiler__compiler__TypeCategory_interface_;
  } else if (tmp41 == compiler__compiler__TokenKind_key_struct) {
    return compiler__compiler__TypeCategory_struct_;
  } else if (tmp41 == compiler__compiler__TokenKind_key_union) {
...
void compiler__Parser_check_not_reserved(compiler__Parser *p) {
  bool tmp42 = 0;
  bool tmp43 =
      map_get(/*aparser.v : 957*/ compiler__reserved_types, p->lit, &tmp42);
...
string compiler__Parser_get_type(compiler__Parser *p) {
  ...
  if ((_IN(string, (p->lit), map_keys(&/* ? */ ti)))) {
    string tmp45 = tos3("");
    bool tmp46 = map_get(/*aparser.v : 1145*/ ti, p->lit, &tmp45);

    if (!tmp46)
      tmp45 = tos((byte *)"", 0);

    typ = string_add(typ, tmp45);
  } else {
  ...
  if (string_eq(p->lit, tos3("C")) &&
    ...
    if (string_eq(t.name, tos3("")) && !p->builtin_mod) {
      ...
      if (string_eq(t.name, tos3("")) && !p->pref->translated &&
          ...
          !string_starts_with(typ, tos3("["))) {
        _V_MulRet_string_V_string _V_mret_5121_t_suggest_tc_suggest =
            compiler__Table_find_misspelled_type(&/* ? */ *p->table, typ, p,
                                                 0.50);
        string t_suggest = _V_mret_5121_t_suggest_tc_suggest.var_0;
        string tc_suggest = _V_mret_5121_t_suggest_tc_suggest.var_1;
        if (t_suggest.len > 0) {
  ...
  if (compiler__Parser_peek(&/* ? */ *p) == compiler__compiler__TokenKind_lt) {
    ...
    while (1) {
      ...
      if ((_IN_MAP((type_param), p->generic_dispatch.inst))) {
        string tmp47 = tos3("");
        bool tmp48 = map_get(/*aparser.v : 1207*/ p->generic_dispatch.inst,
                             type_param, &tmp47);

        if (!tmp48)
          tmp47 = tos((byte *)"", 0);

        typ = string_add(_STR("%.*s_", typ.len, typ.str), tmp47);
      };
      if (p->tok != compiler__compiler__TokenKind_comma) {
        break;
      };
...
string compiler__Parser_statement(compiler__Parser *p, bool add_semi) {
  ...
  string q = tos3("");
  compiler__TokenKind tmp55 = tok;

  if (tmp55 == compiler__compiler__TokenKind_name) {
    compiler__TokenKind next = compiler__Parser_peek(&/* ? */ *p);
    if (compiler__Parser_peek(&/* ? */ *p) ==
  ...
  } else if (tmp55 == compiler__compiler__TokenKind_key_goto) {
    compiler__Parser_check(p, compiler__compiler__TokenKind_key_goto);
    ;
    string label = compiler__Parser_check_name(p);
    compiler__Parser_genln(p, _STR("goto %.*s;", label.len, label.str));
    return tos3("");
  } else if (tmp55 == compiler__compiler__TokenKind_key_defer) {
    compiler__Parser_defer_st(p);
    return tos3("");
  } else if (tmp55 == compiler__compiler__TokenKind_hash) {
    compiler__Parser_chash(p);
    return tos3("");
  } else if (tmp55 == compiler__compiler__TokenKind_key_unsafe) {
    compiler__Parser_next(p);
    p->inside_unsafe = 1;
    ...
    compiler__Parser_statements(p);
    p->inside_unsafe = 0;
  } else if (tmp55 == compiler__compiler__TokenKind_dollar) {
    compiler__Parser_comp_time(p);
  } else if (tmp55 == compiler__compiler__TokenKind_key_if) {
    compiler__Parser_if_statement(p, 0, 0);
  } else if (tmp55 == compiler__compiler__TokenKind_key_for) {
    compiler__Parser_for_st(p);
  } else if (tmp55 == compiler__compiler__TokenKind_key_switch) {
    compiler__Parser_switch_statement(p);
  } else if (tmp55 == compiler__compiler__TokenKind_key_match) {
    compiler__Parser_match_statement(p, 0);
  } else if ((tmp55 == compiler__compiler__TokenKind_key_mut) ||
             (tmp55 == compiler__compiler__TokenKind_key_static)) {
    compiler__Parser_var_decl(p);
  } else if (tmp55 == compiler__compiler__TokenKind_key_return) {
    compiler__Parser_return_st(p);
  } else if (tmp55 == compiler__compiler__TokenKind_lcbr) {
    ...
    compiler__Parser_statements(p);
    return tos3("");
  } else if (tmp55 == compiler__compiler__TokenKind_key_continue) {
    if (p->for_expr_cnt == 0) {
  ...
  } else if (tmp55 == compiler__compiler__TokenKind_key_break) {
    if (p->for_expr_cnt == 0) {
  ...
  } else if (tmp55 == compiler__compiler__TokenKind_key_go) {
    compiler__Parser_go_statement(p);
  } else if (tmp55 == compiler__compiler__TokenKind_key_assert) {
    compiler__Parser_assert_statement(p);
  } else if (tmp55 == compiler__compiler__TokenKind_key_asm) {
...
void compiler__Parser_assign_statement(compiler__Parser *p, compiler__Var v,
  ...
  compiler__TokenKind tmp58 = tok;

  if (tmp58 == compiler__compiler__TokenKind_assign) {
    if (!is_map && !p->is_empty_c_struct_init) {
      compiler__Parser_gen(p, tos3(" = "));
    };
  } else if (tmp58 == compiler__compiler__TokenKind_plus_assign) {
    if (is_str && !p->is_js) {
  ...
  if (string_eq(expr_type, tos3("void"))) {
    _V_MulRet_bool_V_string _V_mret_6932___fn_name =
        compiler__Parser_is_expr_fn_call(&/* ? */ *p, p->token_idx - 3);
    string fn_name = _V_mret_6932___fn_name.var_1;
  ...
  if (string_starts_with(p->assigned_type, tos3("Option_")) &&
    ...
    string vname = string_replace(string_substr2(line, 0, pos, false),
                                  tos3("="), tos3(""));
    Option_int tmp69 = string_index(line, tos3("="));

    if (tmp69.ok) {
      int idx = *(int *)tmp69.data;
      compiler__CGen_resetln(
...
void compiler__Parser_var_decl(compiler__Parser *p) {
  ...
  while (p->tok == compiler__compiler__TokenKind_comma) {
    ...
    if (p->tok == compiler__compiler__TokenKind_key_mut) {
      compiler__Parser_check(p, compiler__compiler__TokenKind_key_mut);
      ;
      _PUSH(&var_mut, (/*typ = array_bool   tmp_typ=bool*/ 1), tmp86, bool);
    } else {
      _PUSH(&var_mut, (/*typ = array_bool   tmp_typ=bool*/ 0), tmp87, bool);
    };
    ...
    _PUSH(&var_names, (/*typ = array_string   tmp_typ=string*/ var_name), tmp89,
          string);
  };
  ...
  if (string_eq(t, tos3("void"))) {
    _V_MulRet_bool_V_string _V_mret_7803___fn_name =
        compiler__Parser_is_expr_fn_call(&/* ? */ *p, p->token_idx - 3);
    string fn_name = _V_mret_7803___fn_name.var_1;
    compiler__Parser_error_with_token_index(
        p,
        _STR("%.*s() %.*s", fn_name.len, fn_name.str,
             compiler__err_used_as_value.len, compiler__err_used_as_value.str),
  ...
  array_string tmp94 = var_names;
  for (int i = 0; i < tmp94.len; i++) {
    string var_name = ((string *)tmp94.data)[i];
...
string compiler__Parser_get_var_type(compiler__Parser *p, string name,
  ...
  if (is_ptr) {
    ...
    int tmp105 = 0;
    ;
    for (int tmp106 = tmp105; tmp106 < deref_nr; tmp106++) {
  ...
  if (deref_nr > 0) {
    ...
    int tmp107 = 0;
    ;
    for (int tmp108 = tmp107; tmp108 < deref_nr; tmp108++) {
...
string compiler__Parser_dot(compiler__Parser *p, string str_typ_,
  ...
  if (has_field) {
    string struct_field =
        ((string_ne(typ.name, tos3("Option")))
             ? (compiler__Table_var_cgen_name(&/* ? */ *p->table, field_name))
             : (field_name));
    Option_compiler__Var tmp115 = compiler__Table_find_field(
    ...
    field = *(compiler__Var *)tmp115.data;
    ;
  ...
  method = *(compiler__Fn *)tmp116.data;
  ;
...
string compiler__Parser_index_expr(compiler__Parser *p, string typ_,
  ...
  if (is_variadic_arg) {
    if (is_indexer) {
      ...
      Option_int tmp133 = string_last_index(l, tos3(" "));
      int idx;
      if (!tmp133.ok) {
...
string compiler__Parser_assoc(compiler__Parser *p) {
  ...
  while (p->tok != compiler__compiler__TokenKind_rcbr) {
    ...
    f = *(compiler__Var *)tmp142.data;
    ;
    _PUSH(&fields, (/*typ = array_string   tmp_typ=string*/ field), tmp143,
          string);
  ...
  array_compiler__Var tmp144 = typ.fields;
  for (int tmp145 = 0; tmp145 < tmp144.len; tmp145++) {
    compiler__Var ffield = ((compiler__Var *)tmp144.data)[tmp145];
...
string compiler__Parser_map_init(compiler__Parser *p) {
  ...
  if (p->tok == compiler__compiler__TokenKind_lcbr) {
    ...
    while (1) {
      ...
      compiler__Parser_check(p, compiler__compiler__TokenKind_colon);
      ;
      _V_MulRet_string_V_string _V_mret_11957_t_val_expr =
          compiler__Parser_tmp_expr(p);
      string t = _V_mret_11957_t_val_expr.var_0;
      string val_expr = _V_mret_11957_t_val_expr.var_1;
      if (i == 0) {
        val_type = t;
      };
      i++;
...
void compiler__Parser_return_st(compiler__Parser *p) {
  ...
  if (fn_returns) {
    ...
    _PUSH(&types, (/*typ = array_string   tmp_typ=string*/ expr_type), tmp157,
          string);
    while (p->tok == compiler__compiler__TokenKind_comma) {

      compiler__Parser_check(p, compiler__compiler__TokenKind_comma);
      _V_MulRet_string_V_string _V_mret_13405_typ_expr =
          compiler__Parser_tmp_expr(p);
      string typ = _V_mret_13405_typ_expr.var_0;
      string expr = _V_mret_13405_typ_expr.var_1;
    ...
    if (types.len > 1) {
      ...
      array_string tmp160 = mr_values;
      for (int ret_val_idx = 0; ret_val_idx < tmp160.len; ret_val_idx++) {
        string ret_val = ((string *)tmp160.data)[ret_val_idx];
...
string compiler__Parser_get_deferred_text(compiler__Parser *p) {
  ...
  array_string tmp165 = p->cur_fn.defer_text;
  for (int tmp166 = 0; tmp166 < tmp165.len; tmp166++) {
    string text = ((string *)tmp165.data)[tmp166];
...
void compiler__Parser_go_statement(compiler__Parser *p) {
  ...
  if (compiler__Parser_peek(&/* ? */ *p) == compiler__compiler__TokenKind_dot) {
    ...
    method = *(compiler__Fn *)tmp168.data;
    ;
    compiler__Parser_async_fn_call(p, method, 0, var_name, v.typ);
  } else {
    string f_name = p->lit;
    Option_compiler__Fn tmp169 = compiler__Table_find_fn(
...
string compiler__Parser_js_decode(compiler__Parser *p) {
  ...
  if (string_eq(op, tos3("decode"))) {
    ...
    string typ = compiler__Parser_get_type(p);
    compiler__Parser_check(p, compiler__compiler__TokenKind_comma);
    _V_MulRet_string_V_string _V_mret_14190_styp_expr =
        compiler__Parser_tmp_expr(p);
    string styp = _V_mret_14190_styp_expr.var_0;
    string expr = _V_mret_14190_styp_expr.var_1;
    ...
    compiler__Type T = compiler__Table_find_type(&/* ? */ *p->table, typ);
    array_compiler__Var tmp170 = T.fields;
    for (int tmp171 = 0; tmp171 < tmp170.len; tmp171++) {
      compiler__Var field = ((compiler__Var *)tmp170.data)[tmp171];
    ...
    compiler__Parser_check(p, compiler__compiler__TokenKind_lpar);
    _V_MulRet_string_V_string _V_mret_14370_typ_expr =
        compiler__Parser_tmp_expr(p);
    string typ = _V_mret_14370_typ_expr.var_0;
    string expr = _V_mret_14370_typ_expr.var_1;
    compiler__Type T = compiler__Table_find_type(&/* ? */ *p->table, typ);
    compiler__Parser_gen_json_for_type(p, T);
    compiler__Parser_check(p, compiler__compiler__TokenKind_rpar);
...
void compiler__Parser_check_and_register_used_imported_type(compiler__Parser *p,
                                                            string typ_name) {
  Option_int tmp177 = string_index(typ_name, tos3("__"));
  int us_idx;
  if (!tmp177.ok) {
...
void compiler__Parser_check_unused_imports(compiler__Parser *p) {
  ...
  string output = tos3("");
  map_string tmp180 = p->import_table.imports;
  array_string keys_tmp180 = map_keys(&tmp180);
  for (int l = 0; l < keys_tmp180.len; l++) {
    string alias = ((string *)keys_tmp180.data)[l];
    string mod = tos3("");
    map_get(tmp180, alias, &mod);
...
void compiler__V_cc_windows_cross(compiler__V *c) {
  ...
  cmd = tos3("");
#ifdef __APPLE__
  cmd = _STR("x86_64-w64-mingw32-gcc -std=gnu11 %.*s -municode", args.len,
             args.str);
#else
...
void compiler__V_build_thirdparty_obj_files(compiler__V *c) {
  array_compiler__CFlag tmp38 = compiler__V_get_os_cflags(&/* ? */ *c);
  for (int tmp39 = 0; tmp39 < tmp38.len; tmp39++) {
    compiler__CFlag flag = ((compiler__CFlag *)tmp38.data)[tmp39];
...
#ifndef _WIN32
  _PUSH(&cflags, (/*typ = array_string   tmp_typ=string*/ tos3("-fPIC")), tmp40,
        string);
#endif
  ;
...
string compiler__V_type_definitions(compiler__V *v) {
  ...
  for (int tmp39 = 0; tmp39 < tmp38.len; tmp39++) {
    ...
    bool tmp41 = map_get(/*cgen.v : 409*/ v->table->typesmap, builtin, &tmp40);
...
string compiler__Parser_bool_expression(compiler__Parser *p) {
  ...
  if (p->tok == compiler__compiler__TokenKind_key_as) {
    ...
    if ((_IN(string, (typ), p->table->sum_types))) {
      compiler__Type T =
          compiler__Table_find_type(&/* ? */ *p->table, cast_typ);
      if (string_ne(T.parent, typ)) {
        compiler__Parser_error(p, _STR("cannot cast `%.*s` to `%.*s`. `%.*s` "
                                       "is not a variant of `%.*s`",
                                       typ.len, typ.str, cast_typ.len,
                                       cast_typ.str, cast_typ.len, cast_typ.str,
                                       typ.len, typ.str));
      };
      compiler__CGen_set_placeholder(
...
string compiler__Parser_name_expr(compiler__Parser *p) {
  ...
  if ((_IN(string, (name), map_keys(&/* ? */ p->generic_dispatch.inst)))) {
    string tmp10 = tos3("");
    bool tmp11 =
        map_get(/*expression.v : 257*/ p->generic_dispatch.inst, name, &tmp10);
  ...
  if (compiler__Table_known_type(&/* ? */ *p->table, name)) {
    if (compiler__Parser_peek(&/* ? */ *p) ==
      ...
      if (byte_is_capital(string_at(val, 0))) {
        ...
        bool tmp18 = map_get(/*expression.v : 403*/ p->table->tuple_variants,
...
compiler__TypeInst compiler__Parser_extract_type_inst(compiler__Parser *p,
  ...
  for (int tmp87 = 0; tmp87 < tmp86.len; tmp87++) {
    ...
    string tmp98 = tos3("");
    bool tmp99 = map_get(/*fn.v : 1332*/ r.inst, tp, &tmp98);
    ...
    if (string_ne(tmp98, tos3(""))) {
      string tmp100 = tos3("");
      bool tmp101 = map_get(/*fn.v : 1333*/ r.inst, tp, &tmp100);
      ...
      if (string_ne(tmp100, ti)) {
        string tmp102 = tos3("");
        bool tmp103 = map_get(/*fn.v : 1334*/ r.inst, tp, &tmp102);
  ...
  string tmp104 = tos3("");
  bool tmp105 = map_get(/*fn.v : 1345*/ r.inst, f->typ, &tmp104);
  ...
  for (int tmp107 = 0; tmp107 < tmp106.len; tmp107++) {
    ...
    string tmp108 = tos3("");
    bool tmp109 = map_get(/*fn.v : 1349*/ r.inst, tp, &tmp108);
...
string compiler__replace_generic_type(string gen_type, compiler__TypeInst *ti) {
  ...
  if ((_IN_MAP((typ), ti->inst))) {
    string tmp112 = tos3("");
    bool tmp113 = map_get(/*fn.v : 1364*/ ti->inst, typ, &tmp112);
...
void compiler__replace_generic_type_params(compiler__Fn *f,
  ...
  if (string_ends_with(f->typ, tos3("_T"))) {
    ...
    string tmp125 = tos3("");
    bool tmp126 = map_get(/*fn.v : 1397*/ ti->inst, par, &tmp125);
...
void compiler__rename_generic_fn_instance(compiler__Fn *f,
  ...
  for (int tmp147 = 0; tmp147 < tmp146.len; tmp147++) {
    ...
    string tmp148 = tos3("");
    bool tmp149 = map_get(/*fn.v : 1501*/ ti->inst, k, &tmp148);
...
Option_int compiler__V_get_file_parser_index(compiler__V *v, string file) {
  ...
  if ((_IN_MAP((file_path), v->file_parser_idx))) {
    int tmp2 = 0;
    bool tmp3 = map_get(/*main.v : 183*/ v->file_parser_idx, file_path, &tmp2);
...
void compiler__V_generate_init(compiler__V *v) {
  ...
  if (v->pref->build_mode == compiler__compiler__BuildMode_default_mode) {
    ...
    array_string tmp16 = v->table->imports;
    for (int tmp17 = 0; tmp17 < tmp16.len; tmp17++) {
      string mod = ((string *)tmp16.data)[tmp17];
...
void compiler__V_generate_main(compiler__V *v) {
  ...
  if (v->pref->build_mode != compiler__compiler__BuildMode_build_module) {
    if (!compiler__Table_main_exists(&/* ? */ *v->table) && !v->pref->is_test) {
      ...
      array_string tmp19 = test_fn_names;
      for (int tmp20 = 0; tmp20 < tmp19.len; tmp20++) {
        string tfname = ((string *)tmp19.data)[tmp20];
...
array_string compiler__V_v_files_from_dir(compiler__V *v, string dir) {
  ...
  Option_array_string tmp23 = os__ls(dir);
  array_string files;
  if (!tmp23.ok) {
  ...
  files = *(array_string *)tmp23.data;
  ;
  if (v->pref->is_verbose) {
    printf("v_files_from_dir (\"%.*s\")\n", dir.len, dir.str);
  };
  array_string_sort(&/* ? */ files);
  array_string tmp24 = files;
  for (int tmp25 = 0; tmp25 < tmp24.len; tmp25++) {
    string file = ((string *)tmp24.data)[tmp25];
    ...
    if (v->compile_defines_all.len > 0 && string_contains(file, tos3("_d_"))) {
      bool allowed = 0;
      array_string tmp26 = v->compile_defines;
      for (int tmp27 = 0; tmp27 < tmp26.len; tmp27++) {
        string cdefine = ((string *)tmp26.data)[tmp27];
...
void compiler__V_add_v_files_to_compile(compiler__V *v) {
  ...
  if (v->pref->is_verbose) {
    string tmp30 = array_string_str(builtin_files);
...
array_string compiler__V_get_user_files(compiler__V *v) {
  ...
  if (is_test) {
    Option_string tmp51 = os__read_file(dir);
    string tcontent;
    if (!tmp51.ok) {
  ...
  if (is_internal_module_test) {
    ...
    _PUSH(&user_files,
          (/*typ = array_string   tmp_typ=string*/ single_test_v_file), tmp52,
          string);
  ...
  if (string_ends_with(dir, tos3(".v")) ||
    ...
    array_string tmp54 = files;
    for (int tmp55 = 0; tmp55 < tmp54.len; tmp55++) {
      string file = ((string *)tmp54.data)[tmp55];

      _PUSH(&user_files, (/*typ = array_string   tmp_typ=string*/ file), tmp56,
            string);
    };
  };
  if (user_files.len == 0) {
  ...
  if (v->pref->is_verbose) {
    string tmp57 = array_string_str(user_files);

    compiler__V_log(&/* ? */ *v,
                    _STR("user_files: %.*s ", tmp57.len, tmp57.str));
  };
  return user_files;
}
array_string compiler__V_get_imported_module_files(compiler__V *v, string mod) {
  array_string files = new_array_from_c_array(
      0, 0, sizeof(string), EMPTY_ARRAY_OF_ELEMS(string, 0){TCCSKIP(0)});
  array_compiler__Parser tmp58 = v->parsers;
  for (int tmp59 = 0; tmp59 < tmp58.len; tmp59++) {
    compiler__Parser p = ((compiler__Parser *)tmp58.data)[tmp59];
...
void compiler__V_parse_lib_imports(compiler__V *v) {
  ...
  int tmp61 = 0;
  ;
  for (int tmp62 = tmp61; tmp62 < v->parsers.len; tmp62++) {
    int i = tmp62;
    ...
    for (int l = 0; l < keys_tmp63.len; l++) {
      ...
      Option_string tmp66 = compiler__V_find_module_path(&/* ? */ *v, mod);
      string import_path;
      if (!tmp66.ok) {
      ...
      array_string tmp75 = vfiles;
      for (int tmp76 = 0; tmp76 < tmp75.len; tmp76++) {
        string file = ((string *)tmp75.data)[tmp76];
      ...
      _PUSH(&done_imports, (/*typ = array_string   tmp_typ=string*/ mod), tmp83,
            string);
    };
  };
}
...
compiler__V *compiler__new_v(array_string args) {
  if (!os__is_dir(compiler__v_modules_path)) {
    Option_bool tmp84 = os__mkdir(compiler__v_modules_path);
    if (!tmp84.ok) {
      string err = tmp84.error;
      int errcode = tmp84.ecode;
      v_panic(err);
    };
    Option_bool tmp85 =
  ...
  _V_MulRet_array_string_V_array_string
      _V_mret_4557_compile_defines_compile_defines_all =
          compiler__parse_defines(defines);
  array_string compile_defines =
      _V_mret_4557_compile_defines_compile_defines_all.var_0;
  array_string compile_defines_all =
      _V_mret_4557_compile_defines_compile_defines_all.var_1;
...
array_string compiler__non_empty(array_string a) {
  ...
  array_string tmp95 = new_array(0, a.len, sizeof(string));
  for (int i = 0; i < a.len; i++) {
...
compiler__OS compiler__os_from_string(string os) {
  ...
  if (string_eq(tmp104, tos3("linux"))) {
  ...
  } else if (string_eq(tmp104, tos3("openbsd"))) {
    return compiler__compiler__OS_openbsd;
  } else if (string_eq(tmp104, tos3("netbsd"))) {
    return compiler__compiler__OS_netbsd;
  } else if (string_eq(tmp104, tos3("dragonfly"))) {
    return compiler__compiler__OS_dragonfly;
  } else if (string_eq(tmp104, tos3("js"))) {
    return compiler__compiler__OS_js;
  } else if (string_eq(tmp104, tos3("solaris"))) {
    return compiler__compiler__OS_solaris;
  } else if (string_eq(tmp104, tos3("android"))) {
  ...
  } else if (string_eq(tmp104, tos3("linux_or_macos"))) {
    return compiler__compiler__OS_linux;
  } else // default:
  {
...
compiler__V *compiler__new_v_compiler_with_args(array_string args) {
  ...
  _PUSH_MANY(&allargs, (/*typ = array_string   tmp_typ=string*/ args), tmp105,
             array_string);
