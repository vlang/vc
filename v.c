...
#define V_CURRENT_COMMIT_HASH "99c7feb"
#endif
	...
	if (!is_range) {
		...
		if (v__table__type_is(node.left_type, v__table__TypeFlag_variadic)) {
			...
			if (g->is_assign_lhs && !is_selector && node.is_setter) {
				...
				if ((g->assign_op == v__token__Kind_mult_assign || g->assign_op == v__token__Kind_plus_assign || g->assign_op == v__token__Kind_minus_assign)) {
					...
					v__gen__Gen_expr(g, node.index);
					v__gen__Gen_write(g, tos3(") "));
					string op = (g->assign_op == v__token__Kind_mult_assign) ? tos3("*") : (g->assign_op == v__token__Kind_plus_assign) ? tos3("+") : (g->assign_op == v__token__Kind_minus_assign) ? tos3("-") : tos3("");
...
void v__gen__Gen_or_block(v__gen__Gen* g, string var_name, array_v__ast__Stmt stmts, v__table__Type return_type) {
	...
	multi_return_string_string mr_69068 = v__gen__Gen_type_of_last_statement(g, stmts);
	string last_type = mr_69068.arg0;
	string type_of_last_expression = mr_69068.arg1;
