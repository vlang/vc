...
int utf8_len(byte c);
int utf8_getchar();
string string_utf8_to_upper(string s);
string string_utf8_to_lower(string s);
int utf8util_char_len(byte b);
string string_utf8_up_low(string s, bool uppper_flag);
int find_char(u16 inCode);
...
string compiler__Parser_register_multi_return_stuct(compiler__Parser *p,
                                                    array_string types);
void compiler__Parser_save_generic_tmpl(compiler__Parser *p, compiler__Fn *f,
                                        int pos);
string compiler__Fn_generic_tmpl_to_inst(compiler__Fn *f,
                                         compiler__TypeInst *ti);
...
string compiler__Parser_fn_signature_v(compiler__Parser *p, compiler__Fn *f);
...
array_u16 builtin__unicode_con_table;
...
#define compiler__compiler__TokenKind_line_comment 53
#define compiler__compiler__TokenKind_mline_comment 54
#define compiler__compiler__TokenKind_nl 55
#define compiler__compiler__TokenKind_dot 56
#define compiler__compiler__TokenKind_dotdot 57
...
array_string string_split_single(string s, byte delim) {
  ...
  while (i < s.len) {
    ...
    if (is_delim || last) {
      ...
      string val = string_substr(s, start, i);
      if (val.len > 0) {
        _PUSH(&res, (/*typ = array_string   tmp_typ=string*/ val), tmp11,
              string);
      };
      start = i + 1;
    };
...
array_string string_split_into_lines(string s) {
  ...
  for (int i = 0; i < s.len; i++) {
    ...
    if (((int)(s.str[i] /*rbyte 0*/)) == 10 || last) {
      ...
      _PUSH(&res, (/*typ = array_string   tmp_typ=string*/ line), tmp12,
            string);
      start = i + 1;
    };
  };
...
string string_utf8_up_low(string s, bool uppper_flag) {
  ...
  while (1) {
    int ch_len = utf8util_char_len(s.str[/*ptr!*/ _index] /*rbyte 0*/);
    if (ch_len == 1) {
      if (uppper_flag == 1) {
        str_res[/*ptr!*/ _index] /*rbyte 1*/ =
            toupper(s.str[/*ptr!*/ _index] /*rbyte 0*/);
      } else {
        str_res[/*ptr!*/ _index] /*rbyte 1*/ =
            tolower(s.str[/*ptr!*/ _index] /*rbyte 0*/);
      };
      ...
      if (ch_index == 0 || (ch_index & 1) == test_result) {
        ...
        for (int tmp2 = tmp1; tmp2 < ch_len; tmp2++) {
          ...
          str_res[/*ptr!*/ _index + i] /*rbyte 1*/ =
              s.str[/*ptr!*/ _index + i] /*rbyte 0*/;
        };
      } else {
        u16 tab_char = ((u16)((
        ...
        if (ch_len == 2) {
          ...
          int tmp5 = 0;
          ;
          for (int tmp6 = tmp5; tmp6 < ch_len; tmp6++) {
            int i = tmp6;
      ...
      int tmp7 = 0;
      ;
      for (int tmp8 = tmp7; tmp8 < ch_len; tmp8++) {
        int i = tmp8;
    ...
    old_index = _index;
    _index += ch_len;
    string my_substr = string_substr(s, old_index, _index);
    u16 uct = (*(u16 *)array_get(builtin__unicode_con_table, 0));
    if (_index >= s.len) {
      break;
    };
  };
...
int find_char(u16 inCode) {
  array_u16 tmp11 = builtin__unicode_con_table;
  for (int c = 0; c < tmp11.len; c++) {
    u16 x = ((u16 *)tmp11.data)[c];
...
Option_array_byte os__read_bytes(string path) {
  ...
  fclose(fp);
  array_byte tmp1 = OPTION_CAST(array_byte)(array_slice(res, 0, nreadbytes));
  return opt_ok(&tmp1, sizeof(array_byte));
}
Option_string os__read_file(string path) {
  string mode = tos3("rb");
  ...
  string tmp2 = OPTION_CAST(string)((tos((byte *)str, fsize)));
  return opt_ok(&tmp2, sizeof(string));
}
int os__file_size(string path) {
...
Option_bool os__cp_r(string osource_path, string odest_path, bool overwrite) {
  ...
  for (int tmp8 = 0; tmp8 < tmp7.len; tmp8++) {
    ...
    Option_bool tmp10 = os__cp_r(sp, dp, overwrite);
    if (!tmp10.ok) {
...
Option_bool os__mv_by_cp(string source, string target) {
  Option_bool tmp12 = os__cp(source, target);
  if (!tmp12.ok) {
  ...
  os__rm(source);
  bool tmp13 = OPTION_CAST(bool)(1);
  return opt_ok(&tmp13, sizeof(bool));
}
FILE *os__vfopen(string path, string mode) {
#ifdef _WIN32
#else
...
Option_array_string os__read_lines(string path) {
  ...
  fclose(fp);
  array_string tmp15 = OPTION_CAST(array_string)(res);
  return opt_ok(&tmp15, sizeof(array_string));
}
Option_array_ustring os__read_ulines(string path) {
  Option_array_string tmp16 = os__read_lines(path);
  array_string lines;
  if (!tmp16.ok) {
    string err = tmp16.error;
    int errcode = tmp16.ecode;
    string tmp17 = OPTION_CAST(string)(err);
    return opt_ok(&tmp17, sizeof(string));
  }
  lines = *(array_string *)tmp16.data;
  ;
  ...
  array_ustring tmp21 = OPTION_CAST(array_ustring)(ulines);
  return opt_ok(&tmp21, sizeof(array_ustring));
}
Option_os__File os__open(string path) {
...
#ifdef _WIN32
#else
  _V_MulRet_int_V_bool _V_mret_1925_ret__ =
      os__posix_wait4_to_exit_status(((int)(pclose(f))));
  int ret = _V_mret_1925_ret__.var_0;
  return ret;
#endif
  ;
}
...
string os__sigint_to_signal_name(int si) {
  ...
  if ((tmp26 == 30) || (tmp26 == 10) || (tmp26 == 16)) {
    return tos3("SIGUSR1");
  } else if ((tmp26 == 31) || (tmp26 == 12) || (tmp26 == 17)) {
    return tos3("SIGUSR2");
  } else if ((tmp26 == 20) || (tmp26 == 17) || (tmp26 == 18)) {
    return tos3("SIGCHLD");
  } else if ((tmp26 == 19) || (tmp26 == 18) || (tmp26 == 25)) {
    return tos3("SIGCONT");
  } else if ((tmp26 == 17) || (tmp26 == 19) || (tmp26 == 23)) {
    return tos3("SIGSTOP");
  } else if ((tmp26 == 18) || (tmp26 == 20) || (tmp26 == 24)) {
    return tos3("SIGTSTP");
  } else if ((tmp26 == 21) || (tmp26 == 21) || (tmp26 == 26)) {
    return tos3("SIGTTIN");
  } else if ((tmp26 == 22) || (tmp26 == 22) || (tmp26 == 27)) {
    return tos3("SIGTTOU");
  } else if (tmp26 == 5) {
    return tos3("SIGTRAP");
  } else if (tmp26 == 7) {
...
array_string os__walk_ext(string path, string ext) {
  ...
  for (int i = 0; i < tmp42.len; i++) {
    string file = ((string *)tmp42.data)[i];
    ...
    if (os__is_dir(p)) {
      ...
      _PUSH(&res, (/*typ = array_string   tmp_typ=string*/ p), tmp44, string);
    };
  };
  return res;
}
...
void os__mkdir_all(string path) {
  string p =
      ((string_starts_with(path, os__path_separator)) ? (os__path_separator)
                                                      : (tos3("")));
  array_string tmp48 = string_split(path, os__path_separator);
  for (int tmp49 = 0; tmp49 < tmp48.len; tmp49++) {
    string subdir = ((string *)tmp48.data)[tmp49];

    p = string_add(p, string_add(subdir, os__path_separator));
    if (!os__dir_exists(p)) {
      Option_bool tmp50 = os__mkdir(p);
      if (!tmp50.ok) {
...
string compiler__V_interface_table(compiler__V *v) {
  ...
  map_compiler__Type tmp57 = v->table->typesmap;
  array_string keys_tmp57 = map_keys(&tmp57);
  for (int l = 0; l < keys_tmp57.len; l++) {
    string _ = ((string *)keys_tmp57.data)[l];
    compiler__Type t = {0};
    map_get(tmp57, _, &t);
    ...
    for (int i = 0; i < tmp58.len; i++) {
      ...
      array_compiler__Fn tmp59 = t.methods;
      for (int i = 0; i < tmp59.len; i++) {
        compiler__Fn method = ((compiler__Fn *)tmp59.data)[i];
...
void compiler__Parser_gen_array_str(compiler__Parser *p, compiler__Type typ) {
  ...
  compiler__Parser_add_method(
      ...
      (compiler__Fn){.name = tos3("str"),
                     ...
                     .generic_tmpl = new_array(0, 1, sizeof(compiler__Token)),
                     .fn_name_token_idx = 0,
...
string compiler__Parser_gen_array_map(compiler__Parser *p, string str_typ,
  ...
  string a = p->expr_var.name;
  _V_MulRet_string_V_string _V_mret_2095_map_type_expr =
      compiler__Parser_tmp_expr(p);
  string map_type = _V_mret_2095_map_type_expr.var_0;
  string expr = _V_mret_2095_map_type_expr.var_1;
  compiler__CGen_set_placeholder(
      p->cgen, method_ph,
...
void compiler__Parser_fn_decl(compiler__Parser *p) {
  ...
  array_compiler__Var tmp46 = f.args;
  for (int tmp47 = 0; tmp47 < tmp46.len; tmp47++) {
    compiler__Var arg = ((compiler__Var *)tmp46.data)[tmp47];
  ...
  if (!is_c && !is_live && !p->is_vh && !is_fn_header && !skip_main_in_test) {
    ...
    if (f.is_generic) {
      if (compiler__Parser_first_pass(&/* ? */ *p)) {
        compiler__Parser_save_generic_tmpl(
            p, &/*114*/ f, compiler__Parser_cur_tok_index(&/* ? */ *p));
        if (f.is_method) {
          compiler__Type rcv =
              compiler__Table_find_type(&/* ? */ *p->table, receiver_typ);
          if (compiler__Parser_first_pass(&/* ? */ *p) &&
  ...
  if (p->is_vh || compiler__Parser_first_pass(&/* ? */ *p) || is_live ||
    ...
    if (is_live && compiler__Parser_first_pass(&/* ? */ *p) &&
      ...
      _PUSH(&p->cgen->so_fns,
            (/*typ = array_string   tmp_typ=string*/ fn_name_cgen), tmp49,
            string);
...
void compiler__Parser_check_unused_variables(compiler__Parser *p) {
  array_compiler__Var tmp53 = p->local_vars;
  for (int tmp54 = 0; tmp54 < tmp53.len; tmp54++) {
    compiler__Var var = ((compiler__Var *)tmp53.data)[tmp54];
...
void compiler__Parser_async_fn_call(compiler__Parser *p, compiler__Fn f,
  ...
  array_compiler__Var tmp55 = f.args;
  for (int i = 0; i < tmp55.len; i++) {
    compiler__Var arg = ((compiler__Var *)tmp55.data)[i];
    ...
    if (string_ends_with(receiver.typ, tos3("er"))) {
      ...
      if (t.cat == compiler__compiler__TypeCategory_interface_) {
        ...
        for (int i = 0; i < tmp60.len; i++) {
          compiler__Fn method = ((compiler__Fn *)tmp60.data)[i];
...
void compiler__Parser_fn_args(compiler__Parser *p, compiler__Fn *f) {
  ...
  while (p->tok != compiler__compiler__TokenKind_rpar) {
    ...
    array_string tmp66 = names;
    for (int tmp67 = 0; tmp67 < tmp66.len; tmp67++) {
      string name = ((string *)tmp66.data)[tmp67];
...
void compiler__Parser_fn_call_args(compiler__Parser *p, compiler__Fn *f) {
  ...
  array_string saved_args = new_array_from_c_array(
      0, 0, sizeof(string), EMPTY_ARRAY_OF_ELEMS(string, 0){TCCSKIP(0)});
  array_compiler__Var tmp70 = f->args;
  for (int i = 0; i < tmp70.len; i++) {
    compiler__Var arg = ((compiler__Var *)tmp70.data)[i];
    ...
    if (type_mismatch && f->is_generic) {
      _PUSH(&saved_args, (/*typ = array_string   tmp_typ=string*/ got), tmp72,
            string);
...
compiler__TypeInst compiler__Parser_extract_type_inst(compiler__Parser *p,
  ...
  if (string_ne(f->typ, tos3("void"))) {
    _PUSH(&args, (/*typ = array_string   tmp_typ=string*/ f->typ), tmp77,
          string);
  };
  array_string tmp78 = args;
  for (int tmp79 = 0; tmp79 < tmp78.len; tmp79++) {
    string e = ((string *)tmp78.data)[tmp79];
    ...
    if (string_starts_with(ti, tos3("fn ("))) {
      array_string fn_args = string_split(
          ...
          tos3(","));
      bool found = 0;
      array_string tmp84 = fn_args;
      for (int tmp85 = 0; tmp85 < tmp84.len; tmp85++) {
        string fa_ = ((string *)tmp84.data)[tmp85];
      ...
      if (string_ne(tmp92, ti)) {
        ...
        compiler__Parser_error(
            p, _STR("type parameter `%.*s` has type %.*s, not `%.*s`", tp.len,
                    tp.str, tmp94.len, tmp94.str, ti.len, ti.str));
      };
      continue;
    };
  ...
  if (string_eq(tmp96, tos3("")) && (_IN(string, (f->typ), f->type_pars))) {
    map_set(&r.inst, f->typ, &(string[]){tos3("_ANYTYPE_")});
  };
  array_string tmp98 = f->type_pars;
...
string compiler__replace_generic_type(string gen_type, compiler__TypeInst *ti) {
  ...
  if ((_IN_MAP((typ), ti->inst))) {
    ...
    typ = string_replace(gen_type, typ, tmp104);
    return typ;
  };
  typ = gen_type;
  if (string_starts_with(typ, tos3("fn ("))) {
    ...
    array_string args_r = new_array_from_c_array(
        0, 0, sizeof(string), EMPTY_ARRAY_OF_ELEMS(string, 0){TCCSKIP(0)});
    array_string tmp108 = args;
    for (int tmp109 = 0; tmp109 < tmp108.len; tmp109++) {
      string arg = ((string *)tmp108.data)[tmp109];
...
compiler__Parser_fn_call_vargs(compiler__Parser *p, compiler__Fn f) {
  ...
  while (p->tok != compiler__compiler__TokenKind_rpar) {
    ...
    if (!f.is_generic) {
      ...
      if (types.len > 0) {
        array_string tmp124 = types;
        for (int tmp125 = 0; tmp125 < tmp124.len; tmp125++) {
          string t = ((string *)tmp124.data)[tmp125];
                         ...
                         !string_ends_with(varg_type, tos3("*")))
                            ...
                            : (((!string_ends_with(last_arg.typ, tos3("*")) &&
                                 string_ends_with(varg_type, tos3("*")))
                                    ? (tos3("*"))
                                    : (tos3("")))));
    _PUSH(&types, (/*typ = array_string   tmp_typ=string*/ varg_type), tmp126,
          string);
    _PUSH(&values,
          (/*typ = array_string   tmp_typ=string*/ _STR(
  ...
  array_compiler__VargAccess tmp128 = p->table->varg_access;
  for (int tmp129 = 0; tmp129 < tmp128.len; tmp129++) {
    compiler__VargAccess va = ((compiler__VargAccess *)tmp128.data)[tmp129];
...
void compiler__Parser_save_generic_tmpl(compiler__Parser *p, compiler__Fn *f,
                                        int pos) {
  int cbr_depth = 1;
  array_compiler__Token tokens = new_array_from_c_array(
      0, 0, sizeof(compiler__Token),
      EMPTY_ARRAY_OF_ELEMS(compiler__Token, 0){TCCSKIP(0)});
  int tmp136 = pos;
  ;
  for (int tmp137 = tmp136; tmp137 < p->tokens.len - 1; tmp137++) {
    int i = tmp137;

    compiler__Token tok = (*(compiler__Token *)array_get(p->tokens, i));
    if (tok.tok == compiler__compiler__TokenKind_lcbr) {
      cbr_depth++;
    };
    if (tok.tok == compiler__compiler__TokenKind_rcbr) {
      cbr_depth--;
      if (cbr_depth == 0) {
        break;
      };
    ...
    _PUSH(&tokens,
          (/*typ = array_compiler__Token   tmp_typ=compiler__Token*/ tok),
          tmp140, compiler__Token);
  };
...
string compiler__Fn_generic_tmpl_to_inst(compiler__Fn *f,
                                         compiler__TypeInst *ti) {
  string fn_body = tos3("");
  array_compiler__Token tmp141 = f->generic_tmpl;
  for (int tmp142 = 0; tmp142 < tmp141.len; tmp142++) {
    compiler__Token tok = ((compiler__Token *)tmp141.data)[tmp142];
...
void compiler__rename_generic_fn_instance(compiler__Fn *f,
  ...
  array_string tmp145 = map_keys(&/* ? */ ti->inst);
  for (int tmp146 = 0; tmp146 < tmp145.len; tmp146++) {
    ...
    f->name = string_add(string_add(f->name, tos3("_")),
                         compiler__type_to_safe_str(tmp147));
  };
}
void compiler__Parser_dispatch_generic_fn_instance(compiler__Parser *p,
                                                   compiler__Fn *f,
                                                   compiler__TypeInst *ti) {
  bool new_inst = 1;
  array_compiler__TypeInst tmp149 = f->type_inst;
  for (int tmp150 = 0; tmp150 < tmp149.len; tmp150++) {
    compiler__TypeInst e = ((compiler__TypeInst *)tmp149.data)[tmp150];
  ...
  string fn_code = _STR("%.*s {\n%.*s\n}",
                        compiler__Parser_fn_signature_v(&/* ? */ *p, f).len,
                        compiler__Parser_fn_signature_v(&/* ? */ *p, f).str,
  ...
  if ((_IN_MAP((f->mod), p->v->gen_parser_idx))) {
    ...
    compiler__Parser_add_text(
        ...
        fn_code);
    array_string tmp157 = p->table->imports;
    for (int tmp158 = 0; tmp158 < tmp157.len; tmp158++) {
      string mod = ((string *)tmp157.data)[tmp158];
      ...
      compiler__Parser_register_import(
          &/* ? */ (*(compiler__Parser *)array_get(p->v->parsers, pidx)), mod,
          0);
    };
  } else {
  };
...
string compiler__Fn_str_args_v(compiler__Fn *f, compiler__Table *table) {
  string str_args = tos3("");
  array_compiler__Var tmp166 = f->args;
  for (int i = 0; i < tmp166.len; i++) {
    compiler__Var arg = ((compiler__Var *)tmp166.data)[i];

    if (f->is_method && i == 0) {
      continue;
    };
    string arg_typ =
        string_replace(string_replace(arg.typ, tos3("array_"), tos3("[]")),
    ...
    } else if (string_ends_with(arg_typ, tos3("*")) || arg.ptr) {
      arg_typ = string_add(tos3("&"), string_trim_right(arg_typ, tos3("*")));
    };
    str_args =
        string_add(str_args, _STR("%.*s %.*s", arg.name.len, arg.name.str,
                                  arg_typ.len, arg_typ.str));
    if (i < f->args.len - 1) {
      str_args = string_add(str_args, tos3(","));
    };
  };
...
string compiler__Parser_find_misspelled_local_var(compiler__Parser *p,
  ...
  array_compiler__Var tmp167 = p->local_vars;
  for (int tmp168 = 0; tmp168 < tmp167.len; tmp168++) {
    compiler__Var var = ((compiler__Var *)tmp167.data)[tmp168];
...
bool array_compiler__Fn_contains(array_compiler__Fn fns, compiler__Fn f) {
  array_compiler__Fn tmp169 = fns;
  for (int tmp170 = 0; tmp170 < tmp169.len; tmp170++) {
    compiler__Fn ff = ((compiler__Fn *)tmp169.data)[tmp170];
...
string compiler__Parser_fn_signature_v(compiler__Parser *p, compiler__Fn *f) {
  string method = tos3("");
  string f_name = string_all_after(f->name, tos3("__"));
  if (f->is_method) {
    compiler__Var receiver_arg = (*(compiler__Var *)array_get(f->args, 0));
    string receiver_type = string_trim(receiver_arg.typ, tos3("*"));
    f_name = string_all_after(
        f_name, _STR("%.*s_", receiver_type.len, receiver_type.str));
    string rcv_typ = string_replace(
        string_replace(receiver_arg.typ, tos3("array_"), tos3("[]")),
        tos3("map_"), tos3("map[string]"));
    if (receiver_arg.is_mut) {
      rcv_typ = string_add(tos3("mut "), string_trim(rcv_typ, tos3("*")));
    } else if (string_ends_with(rcv_typ, tos3("*")) || receiver_arg.ptr) {
      rcv_typ = string_add(tos3("&"), string_trim_right(rcv_typ, tos3("&*")));
    };
    method = _STR("(%.*s %.*s) ", receiver_arg.name.len, receiver_arg.name.str,
                  rcv_typ.len, rcv_typ.str);
  };
  string vis = ((f->is_public) ? (tos3("pub ")) : (tos3("")));
  string f_type =
  ...
  return _STR("%.*sfn %.*s%.*s(%.*s) %.*s", vis.len, vis.str, method.len,
              ...
              compiler__Fn_str_args_v(&/* ? */ *f, p->table).str, f_type.len,
              f_type.str);
}
...
string compiler__Fn_str_for_error(compiler__Fn *f) {
  string s = tos3("");
  array_compiler__Var tmp173 = f->args;
  for (int i = 0; i < tmp173.len; i++) {
    compiler__Var a = ((compiler__Var *)tmp173.data)[i];
...
void compiler__Parser_for_st(compiler__Parser *p) {
  ...
  if (p->tok == compiler__compiler__TokenKind_lcbr) {
    ...
    ;
    string tmp = compiler__Parser_get_tmp(p);
    _V_MulRet_string_V_string _V_mret_287_typ_expr =
        compiler__Parser_tmp_expr(p);
...
void compiler__Parser_gen_blank_identifier_assign(compiler__Parser *p) {
  ...
  _V_MulRet_bool_V_string _V_mret_355_is_fn_call_next_expr =
      compiler__Parser_is_expr_fn_call(p, p->token_idx);
  bool is_fn_call = _V_mret_355_is_fn_call_next_expr.var_0;
  string next_expr = _V_mret_355_is_fn_call_next_expr.var_1;
  int pos = compiler__CGen_add_placeholder(&/* ? */ *p->cgen);
  p->is_var_decl = 1;
  string typ = compiler__Parser_bool_expression(p);
  if (string_eq(typ, tos3("void"))) {
...
string compiler__Table_fn_gen_name(compiler__Table *table, compiler__Fn *f) {
  ...
  if (table->obfuscate && !f->is_c && string_ne(f->name, tos3("main")) &&
    ...
    int tmp30 = 0;
    bool tmp31 = map_get(/*gen_c.v : 295*/ table->obf_ids, name, &tmp30);
...
void compiler__V_compile(compiler__V *v) {
  ...
  for (int i = 0; i < tmp10.len; i++) {
    string file = ((string *)tmp10.data)[i];
  ...
  compiler__V_add_parser(v, vgen_parser);
  array_compiler__Parser tmp11 = v->parsers;
  for (int i = 0; i < tmp11.len; i++) {

    if (!(*(compiler__Parser *)array_get(v->parsers, i)).is_vgen) {
      continue;
    };
    compiler__Parser_parse(
        &/* ? */ (*(compiler__Parser *)array_get(v->parsers, i)),
        compiler__compiler__Pass_main);
  };
  if (v->pref->build_mode == compiler__compiler__BuildMode_build_module) {
  ...
  if (v->pref->is_verbose) {
    ...
    array_compiler__CFlag tmp16 = compiler__V_get_os_cflags(&/* ? */ *v);
    for (int tmp17 = 0; tmp17 < tmp16.len; tmp17++) {
      compiler__CFlag flag = ((compiler__CFlag *)tmp16.data)[tmp17];
          ...
          &/* ? */ *v, filepath__join(v->pref->vlib_path,
                                      &(varg_string){.len = 2,
                                                     .args = {tos3("builtin"),
                                                              tos3("bare")}}))),
      tmp18, array_string);
  _PUSH(&v->files, (/*typ = array_string   tmp_typ=string*/ v->dir), tmp19,
        string);
...
void compiler__V_generate_init(compiler__V *v) {
  ...
  if (v->pref->build_mode == compiler__compiler__BuildMode_default_mode) {
    ...
    for (int tmp25 = 0; tmp25 < tmp24.len; tmp25++) {
      string mod = ((string *)tmp24.data)[tmp25];
...
void compiler__V_generate_main(compiler__V *v) {
  ...
  if (v->pref->build_mode != compiler__compiler__BuildMode_build_module) {
    if (!compiler__Table_main_exists(&/* ? */ *v->table) && !v->pref->is_test) {
      ...
      map_compiler__Fn tmp27 = v->table->fns;
...
void compiler__V_run_compiled_executable_and_exit(compiler__V v) {
  ...
  array_string tmp28 = args;
  for (int i = 0; i < tmp28.len; i++) {
    string a = ((string *)tmp28.data)[i];
...
array_string compiler__V_v_files_from_dir(compiler__V *v, string dir) {
  ...
  Option_array_string tmp31 = os__ls(dir);
  array_string files;
  if (!tmp31.ok) {
    string err = tmp31.error;
    int errcode = tmp31.ecode;
...
void compiler__V_add_v_files_to_compile(compiler__V *v) {
  ...
  array_string tmp36 = builtin_files;
  ...
  array_string tmp39 = compiler__V_get_user_files(&/* ? */ *v);
  for (int tmp40 = 0; tmp40 < tmp39.len; tmp40++) {
    ...
    if (p.v_script) {
      ...
      _PUSH(&p.table->imports,
            (/*typ = array_string   tmp_typ=string*/ tos3("os")), tmp41,
            string);
  ...
  array_string tmp42 = imported_mods;
  for (int tmp43 = 0; tmp43 < tmp42.len; tmp43++) {
    string mod = ((string *)tmp42.data)[tmp43];

    if (!(_IN_MAP((mod), v->gen_parser_idx))) {
      compiler__Parser gp = compiler__V_new_parser_from_string(
          v, string_add(
                 string_add(tos3("module "), string_all_after(mod, tos3("."))),
                 tos3("\n")));
      gp.is_vgen = 1;
      gp.mod = mod;
      map_set(&v->gen_parser_idx, mod, &(int[]){compiler__V_add_parser(v, gp)});
    };
  ...
  array_compiler__Parser tmp49 = v->parsers;
  for (int tmp50 = 0; tmp50 < tmp49.len; tmp50++) {
    compiler__Parser p = ((compiler__Parser *)tmp49.data)[tmp50];
...
array_string compiler__V_get_user_files(compiler__V *v) {
  ...
  if (v->pref->is_test) {
    _PUSH(&user_files,
          (/*typ = array_string   tmp_typ=string*/ filepath__join(
              v->pref->vlib_path,
              &(varg_string){.len = 3,
                             .args = {tos3("compiler"), tos3("preludes"),
  ...
  if (is_test_with_imports) {
    _PUSH(&user_files, (/*typ = array_string   tmp_typ=string*/ dir), tmp54,
          string);
...
array_string compiler__V_get_imported_module_files(compiler__V *v, string mod) {
  array_string files = new_array_from_c_array(
      0, 0, sizeof(string), EMPTY_ARRAY_OF_ELEMS(string, 0){TCCSKIP(0)});
  array_compiler__Parser tmp61 = v->parsers;
  for (int tmp62 = 0; tmp62 < tmp61.len; tmp62++) {
    compiler__Parser p = ((compiler__Parser *)tmp61.data)[tmp62];
...
void compiler__V_parse_lib_imports(compiler__V *v) {
  ...
  int tmp64 = 0;
  ;
  for (int tmp65 = tmp64; tmp65 < v->parsers.len; tmp65++) {
    int i = tmp65;
...
Option_compiler__VsInstallation compiler__find_vs(string vswhere_dir,
  ...
  res = *(os__Result *)tmp1.data;
  ;
  Option_string tmp2 = os__read_file(
...
void compiler__V_cc_msvc(compiler__V *v) {
  Option_compiler__MsvcResult tmp6 = compiler__find_msvc();
  ...
  r = *(compiler__MsvcResult *)tmp6.data;
  ;
  ...
  if (v->pref->is_prod) {
    _PUSH(&a, (/*typ = array_string   tmp_typ=string*/ tos3("/O2")), tmp7,
          string);
    _PUSH(&a, (/*typ = array_string   tmp_typ=string*/ tos3("/MD")), tmp8,
          string);
    _PUSH(&a, (/*typ = array_string   tmp_typ=string*/ tos3("/Zi")), tmp9,
          string);
    _PUSH(&a, (/*typ = array_string   tmp_typ=string*/ tos3("/DNDEBUG")), tmp10,
          string);
  } else {
    _PUSH(&a, (/*typ = array_string   tmp_typ=string*/ tos3("/Zi")), tmp11,
          string);
    _PUSH(&a, (/*typ = array_string   tmp_typ=string*/ tos3("/MDd")), tmp12,
          string);
  };
  if (v->pref->is_so) {
    ...
    _PUSH(&a, (/*typ = array_string   tmp_typ=string*/ tos3("/LD")), tmp13,
          string);
  ...
  _PUSH_MANY(&real_libs,
             (/*typ = array_string   tmp_typ=string*/ sflags.real_libs), tmp16,
             array_string);
  ...
  _PUSH(
      ...
      tmp20, string);
  _PUSH_MANY(&a, (/*typ = array_string   tmp_typ=string*/ inc_paths), tmp21,
             array_string);
  _PUSH_MANY(&a, (/*typ = array_string   tmp_typ=string*/ other_flags), tmp22,
             array_string);
  _PUSH(&a,
        (/*typ = array_string   tmp_typ=string*/ array_string_join(real_libs,
                                                                   tos3(" "))),
        tmp23, string);
  _PUSH(&a, (/*typ = array_string   tmp_typ=string*/ tos3("/link")), tmp24,
        string);
  _PUSH(&a, (/*typ = array_string   tmp_typ=string*/ tos3("/NOLOGO")), tmp25,
        string);
  _PUSH(&a,
        (/*typ = array_string   tmp_typ=string*/ _STR(
  ...
  if (v->pref->is_prod) {
    _PUSH(&a, (/*typ = array_string   tmp_typ=string*/ tos3("/INCREMENTAL:NO")),
          tmp31, string);
    _PUSH(&a, (/*typ = array_string   tmp_typ=string*/ tos3("/OPT:REF")), tmp32,
          string);
    _PUSH(&a, (/*typ = array_string   tmp_typ=string*/ tos3("/OPT:ICF")), tmp33,
          string);
  };
  _PUSH_MANY(&a, (/*typ = array_string   tmp_typ=string*/ lib_paths), tmp34,
             array_string);
  string args = array_string_join(a, tos3(" "));
...
void compiler__build_thirdparty_obj_file_with_msvc(
  ...
  msvc = *(compiler__MsvcResult *)tmp36.data;
  ;
  ...
  printf("%.*s not found, building it (with msvc)...\n", obj_path.len,
         obj_path.str);
  string parent = os__dir(obj_path);
  Option_array_string tmp37 = os__ls(parent);
  array_string files;
  if (!tmp37.ok) {
    string err = tmp37.error;
    int errcode = tmp37.ecode;
...
array_compiler__CFlag_msvc_string_flags(array_compiler__CFlag cflags) {
  ...
  array_string lpaths = new_array_from_c_array(
      0, 0, sizeof(string), EMPTY_ARRAY_OF_ELEMS(string, 0){TCCSKIP(0)});
  array_string tmp49 = lib_paths;
  for (int tmp50 = 0; tmp50 < tmp49.len; tmp50++) {
    string l = ((string *)tmp49.data)[tmp50];
...
void compiler__Parser_parse(compiler__Parser *p, compiler__Pass pass) {
  ...
  while (1) {
    ...
    if (tmp16 == compiler__compiler__TokenKind_key_import) {
      ...
      } else if ((tmp17 == compiler__compiler__TokenKind_key_struct) ||
                 (tmp17 == compiler__compiler__TokenKind_key_union) ||
                 (tmp17 == compiler__compiler__TokenKind_key_interface)) {
    ...
    } else if ((tmp16 == compiler__compiler__TokenKind_key_struct) ||
               (tmp16 == compiler__compiler__TokenKind_key_interface) ||
               (tmp16 == compiler__compiler__TokenKind_key_union) ||
               (tmp16 == compiler__compiler__TokenKind_lsbr)) {
      compiler__Parser_struct_decl(p);
      ...
      if (p->tok == compiler__compiler__TokenKind_assign) {
        compiler__Parser_next(p);
        g = string_add(g, tos3(" = "));
        _V_MulRet_string_V_string _V_mret_2055___expr =
            compiler__Parser_tmp_expr(p);
        string expr = _V_mret_2055___expr.var_1;
        g = string_add(g, expr);
      };
...
void compiler__Parser_const_decl(compiler__Parser *p) {
  ...
  while (p->tok == compiler__compiler__TokenKind_name) {
    ...
    Option_compiler__Var tmp24 =
        compiler__Table_find_const(&/* ? */ *p->v->table, name);

    if (tmp24.ok) {
      compiler__Var my_const = *(compiler__Var *)tmp24.data;
      if (string_eq(my_const.typ, tos3("void"))) {
        array_compiler__Var tmp25 = p->v->table->consts;
        for (int i = 0; i < tmp25.len; i++) {
          compiler__Var v = ((compiler__Var *)tmp25.data)[i];
...
compiler__TypeCategory compiler__key_to_type_cat(compiler__TokenKind tok) {
  ...
  if (tmp33 == compiler__compiler__TokenKind_key_interface) {
    return compiler__compiler__TypeCategory_interface_;
  } else if (tmp33 == compiler__compiler__TokenKind_key_struct) {
    return compiler__compiler__TypeCategory_struct_;
  } else if (tmp33 == compiler__compiler__TokenKind_key_union) {
  ...
  if (string_eq(p->lit, tos3("C")) &&
    ...
    if (string_eq(t.name, tos3("")) && !p->builtin_mod) {
      ...
      if (string_eq(t.name, tos3("")) && !p->pref->translated &&
        ...
        _V_MulRet_string_V_string _V_mret_4516_t_suggest_tc_suggest =
            compiler__Table_find_misspelled_type(&/* ? */ *p->table, typ, p,
                                                 0.50);
        string t_suggest = _V_mret_4516_t_suggest_tc_suggest.var_0;
        string tc_suggest = _V_mret_4516_t_suggest_tc_suggest.var_1;
        if (t_suggest.len > 0) {
  ...
  if (p->returns) {
    if (!v.is_returned && string_ne(v.typ, tos3("FILE*"))) {
      string prev_line =
          (*(string *)array_get(p->cgen->lines, p->cgen->lines.len - 2));
      array_set(
          &/*q*/ p->cgen->lines, p->cgen->lines.len - 2,
          &(string[]){string_add(
...
string compiler__Parser_statement(compiler__Parser *p, bool add_semi) {
  ...
  compiler__TokenKind tok = p->tok;
  string q = tos3("");
  compiler__TokenKind tmp43 = tok;

  if (tmp43 == compiler__compiler__TokenKind_name) {
    compiler__TokenKind next = compiler__Parser_peek(&/* ? */ *p);
    if (compiler__Parser_peek(&/* ? */ *p) ==
  ...
  } else if (tmp43 == compiler__compiler__TokenKind_key_goto) {
    compiler__Parser_check(p, compiler__compiler__TokenKind_key_goto);
    ;
    string label = compiler__Parser_check_name(p);
    compiler__Parser_genln(p, _STR("goto %.*s;", label.len, label.str));
    return tos3("");
  } else if (tmp43 == compiler__compiler__TokenKind_key_defer) {
    compiler__Parser_defer_st(p);
    return tos3("");
  } else if (tmp43 == compiler__compiler__TokenKind_hash) {
    compiler__Parser_chash(p);
    return tos3("");
  } else if (tmp43 == compiler__compiler__TokenKind_key_unsafe) {
    compiler__Parser_next(p);
    p->inside_unsafe = 1;
    compiler__Parser_check(p, compiler__compiler__TokenKind_lcbr);
    compiler__Parser_genln(p, tos3("{"));
    compiler__Parser_statements(p);
    p->inside_unsafe = 0;
  } else if (tmp43 == compiler__compiler__TokenKind_dollar) {
    compiler__Parser_comp_time(p);
  } else if (tmp43 == compiler__compiler__TokenKind_key_if) {
    compiler__Parser_if_st(p, 0, 0);
  } else if (tmp43 == compiler__compiler__TokenKind_key_for) {
    compiler__Parser_for_st(p);
  } else if (tmp43 == compiler__compiler__TokenKind_key_switch) {
    compiler__Parser_switch_statement(p);
  } else if (tmp43 == compiler__compiler__TokenKind_key_match) {
    compiler__Parser_match_statement(p, 0);
  } else if ((tmp43 == compiler__compiler__TokenKind_key_mut) ||
             (tmp43 == compiler__compiler__TokenKind_key_static)) {
    compiler__Parser_var_decl(p);
  } else if (tmp43 == compiler__compiler__TokenKind_key_return) {
    compiler__Parser_return_st(p);
  } else if (tmp43 == compiler__compiler__TokenKind_lcbr) {
    compiler__Parser_check(p, compiler__compiler__TokenKind_lcbr);
    if (p->tok == compiler__compiler__TokenKind_rcbr) {
    ...
    compiler__Parser_statements(p);
    return tos3("");
  } else if (tmp43 == compiler__compiler__TokenKind_key_continue) {
    if (p->for_expr_cnt == 0) {
  ...
  } else if (tmp43 == compiler__compiler__TokenKind_key_break) {
    if (p->for_expr_cnt == 0) {
  ...
  } else if (tmp43 == compiler__compiler__TokenKind_key_go) {
...
void compiler__Parser_assign_statement(compiler__Parser *p, compiler__Var v,
  ...
  compiler__TokenKind tmp46 = tok;

  if (tmp46 == compiler__compiler__TokenKind_assign) {
    if (!is_map && !p->is_empty_c_struct_init) {
      compiler__Parser_gen(p, tos3(" = "));
    };
  } else if (tmp46 == compiler__compiler__TokenKind_plus_assign) {
    if (is_str && !p->is_js) {
      compiler__Parser_gen(p,
  ...
  if (string_eq(expr_type, tos3("void"))) {
    _V_MulRet_bool_V_string _V_mret_6116___fn_name =
        compiler__Parser_is_expr_fn_call(p, p->token_idx - 3);
    string fn_name = _V_mret_6116___fn_name.var_1;
    compiler__Parser_error_with_token_index(
        p,
  ...
  if (string_starts_with(p->assigned_type, tos3("Option_")) &&
    ...
    compiler__CGen_resetln(
        p->cgen, string_add(left, _STR("opt_ok(%.*s, sizeof(%.*s))", expr.len,
             ...
             string_eq(
                 p->assigned_type,
                 string_substr2(expr_type, tos3("Option_").len, -1, true)) &&
    ...
    compiler__CGen_resetln(
        ...
        string_replace(
            line,
            string_substr2(line, 0, string_index(line, tos3("=")) + 1, false),
            tos3("")));
...
void compiler__Parser_var_decl(compiler__Parser *p) {
  ...
  while (p->tok == compiler__compiler__TokenKind_comma) {
    ...
    if (p->tok == compiler__compiler__TokenKind_key_mut) {
      compiler__Parser_check(p, compiler__compiler__TokenKind_key_mut);
      _PUSH(&var_mut, (/*typ = array_bool   tmp_typ=bool*/ 1), tmp73, bool);
    } else {
      _PUSH(&var_mut, (/*typ = array_bool   tmp_typ=bool*/ 0), tmp74, bool);
    };
    ...
    _PUSH(&var_names, (/*typ = array_string   tmp_typ=string*/ var_name), tmp76,
          string);
  };
  ...
  if (string_eq(t, tos3("void"))) {
    _V_MulRet_bool_V_string _V_mret_6931___fn_name =
        compiler__Parser_is_expr_fn_call(p, p->token_idx - 3);
    string fn_name = _V_mret_6931___fn_name.var_1;
    compiler__Parser_error_with_token_index(
        p,
        _STR("%.*s() %.*s", fn_name.len, fn_name.str,
             compiler__err_used_as_value.len, compiler__err_used_as_value.str),
  ...
  array_string tmp81 = var_names;
  for (int i = 0; i < tmp81.len; i++) {
    string var_name = ((string *)tmp81.data)[i];
...
string compiler__Parser_get_var_type(compiler__Parser *p, string name,
  ...
  if (is_ptr) {
    ...
    int tmp92 = 0;
    ;
    for (int tmp93 = tmp92; tmp93 < deref_nr; tmp93++) {
  ...
  if (deref_nr > 0) {
    ...
    int tmp94 = 0;
    ;
    for (int tmp95 = tmp94; tmp95 < deref_nr; tmp95++) {
...
string compiler__Parser_get_c_func_type(compiler__Parser *p, string name) {
  ...
  p->is_c_fn_call = 0;
  Option_compiler__Fn tmp97 = compiler__Table_find_fn(&/* ? */ *p->table, name);
  compiler__Fn cfn;
  if (!tmp97.ok) {
...
string compiler__Parser_dot(compiler__Parser *p, string str_typ_,
  ...
  if (has_field) {
    string struct_field =
        ((string_ne(typ.name, tos3("Option")))
             ? (compiler__Table_var_cgen_name(&/* ? */ *p->table, field_name))
             : (field_name));
    Option_compiler__Var tmp102 = compiler__Table_find_field(
    ...
    field = *(compiler__Var *)tmp102.data;
    ;
  ...
  method = *(compiler__Fn *)tmp103.data;
  ;
...
string compiler__Parser_assoc(compiler__Parser *p) {
  compiler__Parser_next(p);
  string name = compiler__Parser_check_name(p);
  Option_compiler__Var tmp123 = compiler__Parser_find_var_or_const(p, name);
  compiler__Var var;
  if (!tmp123.ok) {
    string err = tmp123.error;
    int errcode = tmp123.ecode;
    compiler__Parser_error(p,
                           _STR("unknown variable `%.*s`", name.len, name.str));
    v_exit(1);
  }
  var = *(compiler__Var *)tmp123.data;
  ;
  if (!var.is_const) {
  ...
  while (p->tok != compiler__compiler__TokenKind_rcbr) {
    ...
    string field = compiler__Parser_check_name(p);
    _PUSH(&fields, (/*typ = array_string   tmp_typ=string*/ field), tmp124,
          string);
    compiler__Parser_gen(p, _STR(".%.*s = ", field.len, field.str));
    compiler__Parser_check(p, compiler__compiler__TokenKind_colon);
    compiler__Parser_bool_expression(p);
  ...
  array_compiler__Var tmp125 = T.fields;
  for (int tmp126 = 0; tmp126 < tmp125.len; tmp126++) {
    compiler__Var ffield = ((compiler__Var *)tmp125.data)[tmp126];
...
void compiler__Parser_string_expr(compiler__Parser *p) {
  ...
  while (p->tok == compiler__compiler__TokenKind_str) {
    ...
    _V_MulRet_string_V_string _V_mret_10922_typ_val_ =
        compiler__Parser_tmp_expr(p);
    string typ = _V_mret_10922_typ_val_.var_0;
    string val_ = _V_mret_10922_typ_val_.var_1;
...
string compiler__Parser_map_init(compiler__Parser *p) {
  ...
  if (p->tok == compiler__compiler__TokenKind_lcbr) {
    ...
    while (1) {
      ...
      compiler__Parser_check(p, compiler__compiler__TokenKind_colon);
      ;
      _V_MulRet_string_V_string _V_mret_11426_t_val_expr =
          compiler__Parser_tmp_expr(p);
      string t = _V_mret_11426_t_val_expr.var_0;
      string val_expr = _V_mret_11426_t_val_expr.var_1;
      if (i == 0) {
        val_type = t;
      };
      i++;
...
string compiler__Parser_array_init(compiler__Parser *p) {
  ...
  if (p->tok == compiler__compiler__TokenKind_name && i == 0 &&
    ...
    if (exp_array) {
      compiler__Parser_error(
          p, _STR("no need to specify the full array type here, use `[]` "
                  "instead of `[]%.*s`",
                  string_substr2(p->expected_type, 6, -1, true).len,
                  string_substr2(p->expected_type, 6, -1, true).str));
    };
    typ = compiler__Parser_get_type(p);
...
string compiler__Parser_if_st(compiler__Parser *p, bool is_expr,
  ...
  if (p->tok == compiler__compiler__TokenKind_name &&
    ...
    p->is_var_decl = 1;
    _V_MulRet_string_V_string _V_mret_12536_option_type_expr =
        compiler__Parser_tmp_expr(p);
    string option_type = _V_mret_12536_option_type_expr.var_0;
    string expr = _V_mret_12536_option_type_expr.var_1;
    p->is_var_decl = 0;
...
void compiler__Parser_return_st(compiler__Parser *p) {
  ...
  if (fn_returns) {
    ...
    _PUSH(&types, (/*typ = array_string   tmp_typ=string*/ expr_type), tmp140,
          string);
    while (p->tok == compiler__compiler__TokenKind_comma) {

      compiler__Parser_check(p, compiler__compiler__TokenKind_comma);
      _V_MulRet_string_V_string _V_mret_13301_typ_expr =
          compiler__Parser_tmp_expr(p);
      string typ = _V_mret_13301_typ_expr.var_0;
      string expr = _V_mret_13301_typ_expr.var_1;
      _PUSH(&types, (/*typ = array_string   tmp_typ=string*/ typ), tmp141,
            string);
      _PUSH(&mr_values,
    ...
    if (types.len > 1) {
      ...
      array_string tmp143 = mr_values;
      for (int ret_val_idx = 0; ret_val_idx < tmp143.len; ret_val_idx++) {
        string ret_val = ((string *)tmp143.data)[ret_val_idx];
...
string compiler__Parser_get_deferred_text(compiler__Parser *p) {
  ...
  array_string tmp148 = p->cur_fn.defer_text;
  for (int tmp149 = 0; tmp149 < tmp148.len; tmp149++) {
    string text = ((string *)tmp148.data)[tmp149];
...
void compiler__Parser_go_statement(compiler__Parser *p) {
  ...
  if (compiler__Parser_peek(&/* ? */ *p) == compiler__compiler__TokenKind_dot) {
    string var_name = p->lit;
    Option_compiler__Var tmp150 =
        compiler__Parser_find_var(&/* ? */ *p, var_name);
    compiler__Var v;
    if (!tmp150.ok) {
      string err = tmp150.error;
      int errcode = tmp150.ecode;

      return;
    }
    v = *(compiler__Var *)tmp150.data;
    ;
    compiler__Parser_mark_var_used(p, v);
    ...
    method = *(compiler__Fn *)tmp151.data;
    ;
    compiler__Parser_async_fn_call(p, method, 0, var_name, v.typ);
  } else {
    string f_name = p->lit;
    Option_compiler__Fn tmp152 = compiler__Table_find_fn(
...
string compiler__Parser_js_decode(compiler__Parser *p) {
  ...
  if (string_eq(op, tos3("decode"))) {
    ...
    string typ = compiler__Parser_get_type(p);
    compiler__Parser_check(p, compiler__compiler__TokenKind_comma);
    _V_MulRet_string_V_string _V_mret_14080_styp_expr =
        compiler__Parser_tmp_expr(p);
    string styp = _V_mret_14080_styp_expr.var_0;
    string expr = _V_mret_14080_styp_expr.var_1;
    ...
    compiler__Type T = compiler__Table_find_type(&/* ? */ *p->table, typ);
    array_compiler__Var tmp153 = T.fields;
    for (int tmp154 = 0; tmp154 < tmp153.len; tmp154++) {
      compiler__Var field = ((compiler__Var *)tmp153.data)[tmp154];
    ...
    compiler__Parser_check(p, compiler__compiler__TokenKind_lpar);
    _V_MulRet_string_V_string _V_mret_14260_typ_expr =
        compiler__Parser_tmp_expr(p);
    string typ = _V_mret_14260_typ_expr.var_0;
    string expr = _V_mret_14260_typ_expr.var_1;
    compiler__Type T = compiler__Table_find_type(&/* ? */ *p->table, typ);
    compiler__Parser_gen_json_for_type(p, T);
    compiler__Parser_check(p, compiler__compiler__TokenKind_rpar);
...
void compiler__Parser_check_unused_imports(compiler__Parser *p) {
  ...
  string output = tos3("");
  map_string tmp162 = p->import_table.imports;
  array_string keys_tmp162 = map_keys(&tmp162);
  for (int l = 0; l < keys_tmp162.len; l++) {
    string alias = ((string *)keys_tmp162.data)[l];
    string mod = tos3("");
    map_get(tmp162, alias, &mod);
...
void compiler__Parser_struct_decl(compiler__Parser *p) {
  ...
  if (p->scanner->is_fmt && p->pass == compiler__compiler__Pass_main) {
    array_compiler__Var tmp5 = typ.fields;
    for (int tmp6 = 0; tmp6 < tmp5.len; tmp6++) {
      compiler__Var field = ((compiler__Var *)tmp5.data)[tmp6];
  ...
  while (p->tok != compiler__compiler__TokenKind_rcbr) {
    ...
    _PUSH(&names, (/*typ = array_string   tmp_typ=string*/ field_name), tmp7,
          string);
    if (is_interface) {
    ...
    if (p->tok == compiler__compiler__TokenKind_lsbr) {
      ...
      if (p->tok == compiler__compiler__TokenKind_colon) {
        compiler__Parser_check(p, compiler__compiler__TokenKind_colon);
        string val = tos3("");
        compiler__TokenKind tmp8 = p->tok;

        if (tmp8 == compiler__compiler__TokenKind_name) {
          val = compiler__Parser_check_name(p);
        } else if (tmp8 == compiler__compiler__TokenKind_str) {
...
string compiler__Parser_struct_init(compiler__Parser *p, string typ) {
  ...
  if (peek == compiler__compiler__TokenKind_colon ||
    ...
    while (p->tok != compiler__compiler__TokenKind_rcbr) {
      ...
      Option_compiler__Var tmp9 = compiler__Type_find_field(&/* ? */ t, field);
      compiler__Var f;
      if (!tmp9.ok) {
        string err = tmp9.error;
        int errcode = tmp9.ecode;
    ...
    array_compiler__Var tmp11 = t.fields;
    for (int i = 0; i < tmp11.len; i++) {
      compiler__Var field = ((compiler__Var *)tmp11.data)[i];
      ...
      if (string_starts_with(field_typ, tos3("map_"))) {
        ...
        _PUSH(&inited_fields,
              (/*typ = array_string   tmp_typ=string*/ sanitized_name), tmp14,
              string);
        if (i != t.fields.len - 1) {
          compiler__Parser_gen(p, tos3(","));
        };
...
bool compiler__Table_known_type(compiler__Table *table, string typ_) {
  ...
  compiler__Type tmp9 = {0};
  bool tmp10 = map_get(/*table.v : 340*/ table->typesmap, typ, &tmp9);

  compiler__Type t = tmp9;
...
Option_compiler__Fn compiler__Table_find_fn(compiler__Table *t, string name) {
  compiler__Fn tmp11 = {0};
  bool tmp12 = map_get(/*table.v : 349*/ t->fns, name, &tmp11);

  compiler__Fn f = tmp11;
  if (f.name.str != 0) {
    compiler__Fn tmp13 = OPTION_CAST(compiler__Fn)(f);
    return opt_ok(&tmp13, sizeof(compiler__Fn));
  };
  return opt_none();
}
Option_compiler__Fn compiler__Table_find_fn_is_script(compiler__Table *t,
                                                      string name,
                                                      bool is_script) {
  compiler__Fn tmp14 = {0};
  bool tmp15 = map_get(/*table.v : 357*/ t->fns, name, &tmp14);

  compiler__Fn f = tmp14;
  if (f.name.str != 0) {
    compiler__Fn tmp16 = OPTION_CAST(compiler__Fn)(f);
    return opt_ok(&tmp16, sizeof(compiler__Fn));
  };
  if (is_script) {
    printf("trying replace %.*s\n", name.len, name.str);
    compiler__Fn tmp17 = {0};
    bool tmp18 =
        map_get(/*table.v : 364*/ t->fns,
                string_replace(name, tos3("main__"), tos3("os__")), &tmp17);

    f = tmp17;
    if (f.name.str != 0) {
      compiler__Fn tmp19 = OPTION_CAST(compiler__Fn)(f);
      return opt_ok(&tmp19, sizeof(compiler__Fn));
    };
  };
  return opt_none();
}
bool compiler__Table_known_fn(compiler__Table *t, string name) {
  Option_compiler__Fn tmp20 = compiler__Table_find_fn(&/* ? */ *t, name);
  if (!tmp20.ok) {
...
bool compiler__Table_known_const(compiler__Table *t, string name) {
  Option_compiler__Var tmp21 = compiler__Table_find_const(&/* ? */ *t, name);
  if (!tmp21.ok) {
...
void compiler__Table_add_field(compiler__Table *table, string type_name,
  ...
  bool tmp23 = map_get(/*table.v : 433*/ table->typesmap, type_name, &tmp22);
...
void compiler__Table_add_gen_type(compiler__Table *table, string type_name,
  ...
  _PUSH(&t.gen_types, (/*typ = array_string   tmp_typ=string*/ gen_type), tmp56,
        string);
  map_set(&table->typesmap, type_name, &(compiler__Type[]){t});
}
compiler__Type compiler__Parser_find_type(compiler__Parser *p, string name) {
...
bool compiler__Parser_satisfies_interface(compiler__Parser *p,
  ...
  array_compiler__Fn tmp63 = int_typ.methods;
  for (int tmp64 = 0; tmp64 < tmp63.len; tmp64++) {
    compiler__Fn method = ((compiler__Fn *)tmp63.data)[tmp64];
...
bool compiler__Table_has_at_least_one_test_fn(compiler__Table *t) {
  map_compiler__Fn tmp68 = t->fns;
  array_string keys_tmp68 = map_keys(&tmp68);
  for (int l = 0; l < keys_tmp68.len; l++) {
    string _ = ((string *)keys_tmp68.data)[l];
    compiler__Fn f = {0};
    map_get(tmp68, _, &f);
...
Option_compiler__Var compiler__Table_find_const(compiler__Table *t,
                                                string name) {
  array_compiler__Var tmp69 = t->consts;
  for (int tmp70 = 0; tmp70 < tmp69.len; tmp70++) {
    compiler__Var c = ((compiler__Var *)tmp69.data)[tmp70];

    if (string_eq(c.name, name)) {
      compiler__Var tmp71 = OPTION_CAST(compiler__Var)(c);
      return opt_ok(&tmp71, sizeof(compiler__Var));
    };
  };
  return opt_none();
}
...
bool compiler__is_valid_int_const(string val, string typ) {
  ...
  if (string_eq(tmp74, tos3("byte"))) {
    return 0 <= x && x <= 255;
  } else if (string_eq(tmp74, tos3("u16"))) {
    return 0 <= x && x <= 65535;
  } else if (string_eq(tmp74, tos3("i8"))) {
    return -128 <= x && x <= 127;
  };
  return 1;
}
string compiler__Parser_typ_to_fmt(compiler__Parser *p, string typ, int level) {
  ...
  if (string_eq(tmp75, tos3("string"))) {
    return tos3("%.*s");
  } else if (string_eq(tmp75, tos3("ustring"))) {
    return tos3("%.*s");
  } else if ((string_eq(tmp75, tos3("byte"))) ||
             (string_eq(tmp75, tos3("bool"))) ||
             (string_eq(tmp75, tos3("int"))) ||
             (string_eq(tmp75, tos3("char"))) ||
             (string_eq(tmp75, tos3("byte"))) ||
             (string_eq(tmp75, tos3("i16"))) ||
             (string_eq(tmp75, tos3("i8")))) {
    return tos3("%d");
  } else if ((string_eq(tmp75, tos3("u16"))) ||
             (string_eq(tmp75, tos3("u32")))) {
    return tos3("%u");
  } else if ((string_eq(tmp75, tos3("f64"))) ||
             (string_eq(tmp75, tos3("f32")))) {
    return tos3("%f");
  } else if (string_eq(tmp75, tos3("i64"))) {
    return tos3("%lld");
  } else if (string_eq(tmp75, tos3("u64"))) {
    return tos3("%llu");
  } else if ((string_eq(tmp75, tos3("byte*"))) ||
             (string_eq(tmp75, tos3("byteptr")))) {
    return tos3("%s");
  } else if (string_eq(tmp75, tos3("void"))) {
...
bool compiler__Type_contains_field_type(compiler__Type *t, string typ) {
  ...
  array_compiler__Var tmp80 = t->fields;
  for (int tmp81 = 0; tmp81 < tmp80.len; tmp81++) {
    compiler__Var field = ((compiler__Var *)tmp80.data)[tmp81];
...
string compiler__Parser_identify_typo(compiler__Parser *p, string name) {
  ...
  _V_MulRet_string_V_string _V_mret_3701_typ_type_cat =
      compiler__Table_find_misspelled_type(&/* ? */ *p->table, name, p,
                                           min_match);
  string typ = _V_mret_3701_typ_type_cat.var_0;
  string type_cat = _V_mret_3701_typ_type_cat.var_1;
  if (typ.len > 0) {
    output = string_add(output, _STR("\n  * %.*s: `%.*s`", type_cat.len,
...
string compiler__Table_find_misspelled_fn(compiler__Table *table, string name,
  ...
  map_compiler__Fn tmp88 = table->fns;
  array_string keys_tmp88 = map_keys(&tmp88);
  for (int l = 0; l < keys_tmp88.len; l++) {
    string _ = ((string *)keys_tmp88.data)[l];
    compiler__Fn f = {0};
    map_get(tmp88, _, &f);
...
string compiler__Table_find_misspelled_imported_mod(compiler__Table *table,
  ...
  string n1 = ((string_starts_with(name, tos3("main.")))
                   ...
                   : (name));
  map_string tmp91 = p->import_table.imports;
  array_string keys_tmp91 = map_keys(&tmp91);
  for (int l = 0; l < keys_tmp91.len; l++) {
    string alias = ((string *)keys_tmp91.data)[l];
    string mod = tos3("");
    map_get(tmp91, alias, &mod);
...
string compiler__Table_find_misspelled_const(compiler__Table *table,
  ...
  array_compiler__Var tmp92 = table->consts;
  for (int tmp93 = 0; tmp93 < tmp92.len; tmp93++) {
    compiler__Var cnst = ((compiler__Var *)tmp92.data)[tmp93];
...
compiler__Table_find_misspelled_type(compiler__Table *table, string name,
  ...
  map_compiler__Type tmp94 = table->typesmap;
...
string compiler__type_cat_str(compiler__TypeCategory tc) {
  compiler__TypeCategory tmp95 = tc;

  string tc_str =
      ((tmp95 == compiler__compiler__TypeCategory_builtin)
           ? (tos3("builtin"))
           : ((tmp95 == compiler__compiler__TypeCategory_struct_)
                  ? (tos3("struct"))
                  : ((tmp95 == compiler__compiler__TypeCategory_func)
                         ? (tos3("function"))
                         : ((tmp95 ==
...
void compiler__Parser_fnext(compiler__Parser *p) {
  ...
  if (((*(compiler__Token *)array_get(p->tokens, p->token_idx)).tok ==
    ...
    bool is_rcbr = p->tok == compiler__compiler__TokenKind_rcbr;
    while (p->token_idx < p->tokens.len - 1) {
      ...
      if (next.tok == compiler__compiler__TokenKind_line_comment &&
          comment_token.line_nr < next.line_nr) {
        ;
      };
      p->token_idx++;
    };
...
void compiler__Parser_gen_fmt(compiler__Parser *p) {
  ...
  printf("generating %.*s.v\n", p->file_name.len, p->file_name.str);
  Option_os__File tmp15 =
...
void init() {
  ...
  builtin__unicode_con_table = new_array_from_c_array(
      1332, 1332, sizeof(u16),
      EMPTY_ARRAY_OF_ELEMS(u16, 1332){
          ((u16)(0x0061)), 0x0041, 0x0062, 0x0042, 0x0063, 0x0043, 0x0064,
          0x0044,          0x0065, 0x0045, 0x0066, 0x0046, 0x0067, 0x0047,
          0x0068,          0x0048, 0x0069, 0x0049, 0x006A, 0x004A, 0x006B,
          0x004B,          0x006C, 0x004C, 0x006D, 0x004D, 0x006E, 0x004E,
          0x006F,          0x004F, 0x0070, 0x0050, 0x0071, 0x0051, 0x0072,
          0x0052,          0x0073, 0x0053, 0x0074, 0x0054, 0x0075, 0x0055,
          0x0076,          0x0056, 0x0077, 0x0057, 0x0078, 0x0058, 0x0079,
          0x0059,          0x007A, 0x005A, 0x00E0, 0x00C0, 0x00E1, 0x00C1,
          0x00E2,          0x00C2, 0x00E3, 0x00C3, 0x00E4, 0x00C4, 0x00E5,
          0x00C5,          0x00E6, 0x00C6, 0x00E7, 0x00C7, 0x00E8, 0x00C8,
          0x00E9,          0x00C9, 0x00EA, 0x00CA, 0x00EB, 0x00CB, 0x00EC,
          0x00CC,          0x00ED, 0x00CD, 0x00EE, 0x00CE, 0x00EF, 0x00CF,
          0x00F0,          0x00D0, 0x00F1, 0x00D1, 0x00F2, 0x00D2, 0x00F3,
          0x00D3,          0x00F4, 0x00D4, 0x00F5, 0x00D5, 0x00F6, 0x00D6,
          0x00F8,          0x00D8, 0x00F9, 0x00D9, 0x00FA, 0x00DA, 0x00FB,
          0x00DB,          0x00FC, 0x00DC, 0x00FD, 0x00DD, 0x00FE, 0x00DE,
          0x00FF,          0x0178, 0x0101, 0x0100, 0x0103, 0x0102, 0x0105,
          0x0104,          0x0107, 0x0106, 0x0109, 0x0108, 0x010B, 0x010A,
          0x010D,          0x010C, 0x010F, 0x010E, 0x0111, 0x0110, 0x0113,
          0x0112,          0x0115, 0x0114, 0x0117, 0x0116, 0x0119, 0x0118,
          0x011B,          0x011A, 0x011D, 0x011C, 0x011F, 0x011E, 0x0121,
          0x0120,          0x0123, 0x0122, 0x0125, 0x0124, 0x0127, 0x0126,
          0x0129,          0x0128, 0x012B, 0x012A, 0x012D, 0x012C, 0x012F,
          0x012E,          0x0131, 0x0049, 0x0133, 0x0132, 0x0135, 0x0134,
