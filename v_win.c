...
Option_os__Result tmp116 =  os__exec ( _STR("%.*s/make.bat", vroot.len, vroot.str) ) ;
 if (!tmp116 .ok) {
...
Option_os__Result tmp117 =  os__exec ( _STR("make -C \"%.*s\"", vroot.len, vroot.str) ) ;
 if (!tmp117 .ok) {
...
Option_os__Result tmp123 =  os__exec ( _STR("%.*s -o %.*s vget.v", vexec.len, vexec.str, vget.len, vget.str) ) ;
 if (!tmp123 .ok) {
 ...
 os__Result vgetcompilation = *(os__Result*) tmp123 . data;
 ;
...
Option_os__Result tmp124 =  os__exec (string_add( _STR("%.*s ", vget.len, vget.str) , array_string_join( names , tos2((byte*)" ") ) ) ) ;
 if (!tmp124 .ok) {
 ...
 os__Result vgetresult = *(os__Result*) tmp124 . data;
 ;
