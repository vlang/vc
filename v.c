...
time__Time res= (time__Time){ .year = 0 , .day = 0 , .month = 0 , .hour = 0 , .minute = 0 , .second = 0 , .uni = 0 } ;
 ...
 return  (time__Time){ .year =  rand__next ( 2 ) + 201 , .month =  rand__next ( 12 ) + 1 , .day =  rand__next ( 30 ) + 1 , .hour =  rand__next ( 24 ) , .minute =  rand__next ( 60 ) , .second =  rand__next ( 60 ) , .uni = 0 } ;
...
time__Time res= (time__Time){ .year = 0 , .day = 0 , .month = 0 , .hour = 0 , .minute = 0 , .second = 0 , .uni = 0 } ;
 ...
 return  (time__Time){ .year =  t .tm_year + 1900 , .month =  t .tm_mon + 1 , .day =  t .tm_mday , .hour =  t .tm_hour , .minute =  t .tm_min , .second =  t .tm_sec , .uni = 0 } ;
...
string second= ( *(string*) array__get( hms , 2) ) ;
 ...
 return  (time__Time){ .uni =  time__Time_calc_unix(& /* ? */ t ) , .year = t . year, .day = t . day, .month = t . month, .hour = t . hour, .minute = t . minute, .second = t . second, } ;
...
Fn* f= ALLOC_INIT(Fn, { .pkg =  pkg , .local_vars =  array_repeat(&tmp19,  main__MaxLocalVars , sizeof(Var) ) , .var_idx = 0 , .args = new_array(0, 1, sizeof(Var)) , .is_interface = 0 , .scope_level = 0 , .typ = tos("", 0) , .name = tos("", 0) , .is_c = 0 , .receiver_typ = tos("", 0) , .is_private = 0 , .is_method = 0 , .returns_error = 0 , .is_decl = 0 , .defer = tos("", 0) } ) ;
...
Fn dec_fn= (Fn){ .pkg =  p ->pkg , .typ =  _STR("Option_%.*s", typ .name.len, typ .name.str) , .name =  js_dec_name ( t ) , .local_vars = new_array(0, 1, sizeof(Var)) , .var_idx = 0 , .args = new_array(0, 1, sizeof(Var)) , .is_interface = 0 , .scope_level = 0 , .is_c = 0 , .receiver_typ = tos("", 0) , .is_private = 0 , .is_method = 0 , .returns_error = 0 , .is_decl = 0 , .defer = tos("", 0) } ;
...
Fn enc_fn= (Fn){ .pkg =  p ->pkg , .typ =  tos2("cJSON*") , .name =  js_enc_name ( t ) , .local_vars = new_array(0, 1, sizeof(Var)) , .var_idx = 0 , .args = new_array(0, 1, sizeof(Var)) , .is_interface = 0 , .scope_level = 0 , .is_c = 0 , .receiver_typ = tos("", 0) , .is_private = 0 , .is_method = 0 , .returns_error = 0 , .is_decl = 0 , .defer = tos("", 0) } ;
...
f64 qq= (/*casttt*/ (f64)( /*77*/ 10.0 ) ) ;
 
int ee= (/*casttt*/ (int)( /*77*/ qq ) ) ;
...
bool imports_json= array_string_contains( c ->table ->imports , tos2("json") ) ;
 ...
 CGen_genln( cgen , tos2("i64 total_m = 0; // For counting total RAM allocated") ) ;
...
array_string vfiles= V_v_files_from_dir(& /* ? */* c , _STR("%.*s/%.*s", c ->lang_dir.len, c ->lang_dir.str, pkg.len, pkg.str) ) ;
...
string module_path= _STR("%.*s/%.*s", c ->lang_dir.len, c ->lang_dir.str, pkg.len, pkg.str) ;
...
string vroot= string_trim_space( os__read_file ( vroot_path ) ) ;
 ...
 if ( os__dir_exists ( vroot )  &&  os__dir_exists (string_add( vroot , tos2("/builtin") ) ) ) {
 /*if*/
...
string f= _STR("%.*s/builtin/%.*s", lang_dir.len, lang_dir.str, builtin.len, builtin.str) ;
...
string source_code=string_add(string_add(string_add(string_add( tos2("fn main(){") , array_string_join( lines , tos2("\n") ) ) , tos2("\n") ) , line ) , tos2("}") ) ;
...
Fn* interface_method= ALLOC_INIT(Fn, { .name =  field_name , .is_interface =  1 , .is_method =  1 , .receiver_typ =  name , .pkg = tos("", 0) , .local_vars = new_array(0, 1, sizeof(Var)) , .var_idx = 0 , .args = new_array(0, 1, sizeof(Var)) , .scope_level = 0 , .typ = tos("", 0) , .is_c = 0 , .is_private = 0 , .returns_error = 0 , .is_decl = 0 , .defer = tos("", 0) } ) ;
...
AccessMod access_mod= ( is_pub ) ? ( PUBLIC ) : ( PRIVATE ) ;
 ...
 println ( _STR("%.*s %d mut=%d", field_name.len, field_name.str, access_mod, is_mut) ) ;
...
os__File file_types= os__create_file ( _STR("%.*s/types", main__TmpPath.len, main__TmpPath.str) ) ;
 
os__File file_vars= os__create_file ( _STR("%.*s/vars", main__TmpPath.len, main__TmpPath.str) ) ;
...
Fn f= (Fn){ .name =  tos2("_") , .pkg =  p ->pkg , .local_vars = new_array(0, 1, sizeof(Var)) , .var_idx = 0 , .args = new_array(0, 1, sizeof(Var)) , .is_interface = 0 , .scope_level = 0 , .typ = tos("", 0) , .is_c = 0 , .receiver_typ = tos("", 0) , .is_private = 0 , .is_method = 0 , .returns_error = 0 , .is_decl = 0 , .defer = tos("", 0) } ;
...
Var v= Fn_find_var(& /* ? */* p ->cur_fn , name ) ;
 ...
 Parser_error( p , _STR("variable `%.*s` is already declared.", name.len, name.str) ) ;
...
bool is_c_struct_init= is_c  &&  ptr ;
 ...
 if ( Table_known_pkg(& /* ? */* p ->table , name )  &&  ! Fn_known_var(& /* ? */* p ->cur_fn , name ) ) {
 /*if*/
...
Fn f= (Fn){ .name =  name , .is_c =  1 , .pkg = tos("", 0) , .local_vars = new_array(0, 1, sizeof(Var)) , .var_idx = 0 , .args = new_array(0, 1, sizeof(Var)) , .is_interface = 0 , .scope_level = 0 , .typ = tos("", 0) , .receiver_typ = tos("", 0) , .is_private = 0 , .is_method = 0 , .returns_error = 0 , .is_decl = 0 , .defer = tos("", 0) } ;
...
Token tok= p ->tok ;
 ...
 if ( string_contains( p ->lit , tos2(".") )  ||  string_contains( p ->lit , tos2("e") ) ) {
 /*if*/
...
_PUSH(& p ->cgen ->typedefs , ( _STR("typedef array %.*s;", typ.len, typ.str) ), tmp230, string) ;
...
string expr_typ= Parser_bool_expression( p ) ;
 ...
 Parser_error( p , _STR("cannot convert `%.*s` to `%.*s`", expr_typ.len, expr_typ.str, typ.len, typ.str) ) ;
...
_PUSH(& p ->cgen ->includes , ( _STR("#%.*s", hash.len, hash.str) ), tmp258, string) ;
...
_PUSH(& p ->cgen ->typedefs , ( _STR("%.*s", hash.len, hash.str) ), tmp259, string) ;
...
string os= string_trim_space( string_right( hash , 6 ) ) ;
 ...
 if ( p ->tok == HASH  &&  string_contains( p ->lit , tos2("else") )  ||  string_contains( p ->lit , tos2("endif") ) ) {
 /*if*/
...
_PUSH(& p ->cgen ->includes , ( _STR("#%.*s", hash.len, hash.str) ), tmp263, string) ;
 ...
 return  string_contains( hash , tos2("ifdef") )  ||  string_contains( hash , tos2("define") )  ||  string_contains( hash , tos2("endif") )  ||  string_contains( hash , tos2("elif") )  ||  string_contains( hash , tos2("ifndef") )  ||  (/*lpar*/ string_contains( hash , tos2("else") )  &&  ! string_contains( hash , tos2("{") ) ) ;
...
_PUSH(& p ->cgen ->typedefs , ( _STR("typedef Option %.*s;", opt_type.len, opt_type.str) ), tmp311, string) ;
...
Scanner* scanner= ALLOC_INIT(Scanner, { .file_path =  file_path , .text =  os__read_file ( file_path ) , .fmt_out =  new_string_builder ( 1000 ) , .pos = 0 , .line_nr = 0 , .inside_string = 0 , .dollar_start = 0 , .dollar_end = 0 , .debug = 0 , .line_comment = tos("", 0) , .started = 0 , .is_fmt = 0 , .fmt_indent = 0 , .fmt_line_empty = 0 } ) ;
...
bool is_hex= string_at( s ->text , s ->pos) == '0'  &&  string_at( s ->text , s ->pos + 1) == 'x' ;
...
int pos_start= pos ;
 ...
 for (
  ;  pos_start < pos  &&  (/*lpar*/ string_at( s ->text , pos_start) == ' '  ||  string_at( s ->text , pos_start) == '\n' )  ;  pos_start ++ ) { 
 ...
 s ->text  = string_add(string_add(string_add(string_add(string_add( string_substr( s ->text , 0 , s ->pos - 2 ) , tos2(" = ") ) , Scanner_get_until_start( s , s ->pos - 2 ) ) , tos2(" ") ) , operator ) , string_substr( s ->text , s ->pos + 1 , s ->text .len ) ) ;
...
for (int tmp18 = 0; tmp18 < tmp17 .len; tmp18 ++) {
 ...
 return  (Fn){ .pkg = tos("", 0) , .local_vars = new_array(0, 1, sizeof(Var)) , .var_idx = 0 , .args = new_array(0, 1, sizeof(Var)) , .is_interface = 0 , .scope_level = 0 , .typ = tos("", 0) , .name = tos("", 0) , .is_c = 0 , .receiver_typ = tos("", 0) , .is_private = 0 , .is_method = 0 , .returns_error = 0 , .is_decl = 0 , .defer = tos("", 0) } ;
...
Type datyp= (Type){ .name =  typ , .pkg = tos("", 0) , .fields = new_array(0, 1, sizeof(Var)) , .methods = new_array(0, 1, sizeof(Fn)) , .parent = tos("", 0) , .gen_types = new_array(0, 1, sizeof(string)) , .is_c = 0 , .is_interface = 0 , .is_enum = 0 , .is_placeholder = 0 } ;
...
string name= f ->name ;
 ...
 /*if*/
int tmp65 = 0; bool tmp66 = map_get( table ->obf_ids , name, & tmp65);
...
map_int res= new_map(1, sizeof(int)) ;
 ...
 for (
int t= (/*casttt*/ (int)( /*77*/ keyword_beg ) ) + 1  ;  t < (/*casttt*/ (int)( /*77*/ keyword_end ) )  ;  t ++ ) { 
...
Token a= (/*casttt*/ (Token)( /*77*/ tmp103 ) ) ;
 ...
 return  t == ENUM  ||  t == INTERFACE  ||  t == FUNC  ||  t == STRUCT  ||  t == TIP  ||  t == CONST  ||  t == IMPORT_CONST  ||  t == AT  ||  t == EOF ;
...
main__EmptyFn =  ALLOC_INIT(Fn, { .pkg = tos("", 0) , .local_vars = new_array(0, 1, sizeof(Var)) , .var_idx = 0 , .args = new_array(0, 1, sizeof(Var)) , .is_interface = 0 , .scope_level = 0 , .typ = tos("", 0) , .name = tos("", 0) , .is_c = 0 , .receiver_typ = tos("", 0) , .is_private = 0 , .is_method = 0 , .returns_error = 0 , .is_decl = 0 , .defer = tos("", 0) } );
