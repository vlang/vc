#define V_COMMIT_HASH "fbc1931"
#ifndef V_COMMIT_HASH
#define V_COMMIT_HASH "69b498d"
#endif
...
Option_Var Fn_find_var(Fn *f, string name);
Option_Var Parser_find_var_check_new_var(Parser *p, string name);
Var Fn_find_var2(Fn *f, string name);
...
void Parser_error_with_position(Parser *p, string e, ScannerPos sp);
void Parser_production_error(Parser *p, string e, ScannerPos sp);
...
FileImportTable Table_get_file_import_table(Table *table, string file_path);
...
#define main__Token_key_none 83
#define main__Token_key_return 84
#define main__Token_key_select 85
#define main__Token_key_sizeof 86
#define main__Token_key_struct 87
#define main__Token_key_switch 88
#define main__Token_key_true 89
#define main__Token_key_type 90
#define main__Token_key_orelse 91
#define main__Token_key_union 92
#define main__Token_key_pub 93
#define main__Token_key_static 94
#define main__Token_keyword_end 95
...
array_string os__init_os_args(int argc, byteptr *argv) {
  ...
  for (int i = 0; i < argc; i++) {
    ...
    _PUSH(&args, ((tos2((byte *)argv[/*ptr*/ i] /*rbyteptr 0*/))), tmp3,
          string);
  };
...
void V_cc(V *v) {
  ...
  if (v->os != main__OS_msvc && v->os != main__OS_freebsd) {
    ...
    _PUSH(&a, (tos2((byte *)"-Werror=implicit-function-declaration")), tmp11,
          string);
  };
  ...
  _PUSH(&a, (_STR("-o \"%.*s\"", v->out_name.len, v->out_name.str)), tmp20,
        string);
  ...
  _PUSH(&a, (_STR("\"%.*s\"", v->out_name_c.len, v->out_name_c.str)), tmp22,
        string);
...
string V_type_definitions(V *v) {
  ...
  string res = string_add(string_add(types_to_c(builtin_types, v->table),
                                     tos2((byte *)"\n//----\n")),
...
Option_Var Fn_find_var(Fn *f, string name) {
  ...
  for (int tmp2 = tmp1; tmp2 < f->var_idx; tmp2++) {
    ...
    if (string_eq((*(Var *)array__get(f->local_vars, i)).name, name)) {
      ...
      Var tmp7 = OPTION_CAST(Var)((*(Var *)array__get(f->local_vars, i)));
...
Option_Var Parser_find_var_check_new_var(Parser *p, string name) {
  ...
  int tmp8 = 0;
  ;
  for (int tmp9 = tmp8; tmp9 < p->cur_fn.var_idx; tmp9++) {
    int i = tmp9;

    if (string_eq((*(Var *)array__get(p->cur_fn.local_vars, i)).name, name)) {

      Var tmp14 =
          OPTION_CAST(Var)((*(Var *)array__get(p->cur_fn.local_vars, i)));
...
Var Fn_find_var2(Fn *f, string name) {
  ...
  int tmp16 = 0;
  ;
  for (int tmp17 = tmp16; tmp17 < f->var_idx; tmp17++) {
    int i = tmp17;
...
void Parser_open_scope(Parser *p) {
  ...
  _PUSH(&p->cur_fn.defer_text, (tos2((byte *)"")), tmp22, string);

  p->cur_fn.scope_level++;
}
void Parser_mark_var_used(Parser *p, Var v) {

  array_Var tmp23 = p->cur_fn.local_vars;
  for (int i = 0; i < tmp23.len; i++) {
    Var vv = ((Var *)tmp23.data)[i];
...
void Parser_mark_var_returned(Parser *p, Var v) {
  ...
  array_Var tmp26 = p->cur_fn.local_vars;
  for (int i = 0; i < tmp26.len; i++) {
    Var vv = ((Var *)tmp26.data)[i];
...
void Parser_mark_var_changed(Parser *p, Var v) {
  ...
  array_Var tmp29 = p->cur_fn.local_vars;
  for (int i = 0; i < tmp29.len; i++) {
    Var vv = ((Var *)tmp29.data)[i];
...
bool Fn_known_var(Fn *f, string name) {
  ...
  Option_Var tmp32 = Fn_find_var(&/* ? */ *f, name);
  if (!tmp32.ok) {
    string err = tmp32.error;

    return 0;
  }
  Var _ = *(Var *)tmp32.data;
  ;
...
void Fn_register_var(Fn *f, Var v) {
  ...
  Var new_var = (Var){
      ...
      .is_c = v.is_c,
      .moved = v.moved,
      .scanner_pos = v.scanner_pos,
      .line_nr = v.line_nr,
  };

  if (f->var_idx >= f->local_vars.len) {

    _PUSH(&f->local_vars, (new_var), tmp34, Var);
...
void Parser_fn_decl(Parser *p) {
  ...
  if (p->tok == main__Token_lpar) {
    ...
    Var receiver = (Var){
        ...
        .scanner_pos = Scanner_get_scanner_pos(&/* ? */ *p->scanner),
        .is_const = 0,
        .args = new_array(0, 1, sizeof(Var)),
        .attr = tos((byte *)"", 0),
        .is_alloc = 0,
    ...
    _PUSH(&f.args, (receiver), tmp45, Var);
  ...
  if (Parser_first_pass(&/* ? */ *p) && receiver_typ.len == 0) {
    ...
    while (1) {
      ...
      Option_Fn tmp49 = Table_find_fn(&/* ? */ *p->table, f.name);
      if (!tmp49.ok) {
        string err = tmp49.error;

        break;
      }
      Fn existing_fn = *(Fn *)tmp49.data;
      ;
  ...
  if (string_starts_with(typ, tos2((byte *)"MultiReturn_"))) {
    ...
    if (!Parser_first_pass(&/* ? */ *p) &&
      ...
      for (int i = 0; i < tmp53.len; i++) {
        ...
        Table_add_field(p->table, typ, _STR("var_%d", i), t, 0,
                        tos2((byte *)""), main__AccessMod_public);
      };

      _PUSH(&p->cgen->typedefs,
            (_STR("typedef struct %.*s %.*s;", typ.len, typ.str, typ.len,
                  typ.str)),
            tmp54, string);
    };
  };
  ...
  if (!is_c && !is_live && !is_sig && !is_fn_header && !skip_main_in_test) {
    ...
    if (is_generic) {
      ...
      array_string tmp65 = gen_types;
      for (int tmp66 = 0; tmp66 < tmp65.len; tmp66++) {
        string gen_type = ((string *)tmp65.data)[tmp66];
  ...
  if (is_sig || Parser_first_pass(&/* ? */ *p) || is_live || is_fn_header ||
    ...
    if (is_live && Parser_first_pass(&/* ? */ *p) &&
      ...
      _PUSH(&p->cgen->so_fns, (fn_name_cgen), tmp79, string);
    ...
    if (!is_c && string_ne(f.name, tos2((byte *)"main")) &&
      ...
      _PUSH(&p->cgen->fns, (string_add(fn_decl, tos2((byte *)";"))), tmp81,
            string);
    };
...
void Parser_check_unused_variables(Parser *p) {
  ...
  array_Var tmp88 = p->cur_fn.local_vars;
  for (int tmp89 = 0; tmp89 < tmp88.len; tmp89++) {
    Var var = ((Var *)tmp88.data)[tmp89];
...
void Parser_async_fn_call(Parser *p, Fn f, int method_ph, string receiver_var,
  ...
  array_Var tmp97 = f.args;
  for (int i = 0; i < tmp97.len; i++) {
    Var arg = ((Var *)tmp97.data)[i];
...
void Parser_fn_args(Parser *p, Fn *f) {
  ...
  if (f->is_interface) {
    ...
    _PUSH(&f->args, (int_arg), tmp107, Var);
  };
  ...
  while (p->tok != main__Token_rpar) {
    ...
    while (p->tok == main__Token_comma) {
      ...
      _PUSH(&names, (Parser_check_name(p)), tmp113, string);
    };
    ...
    array_string tmp116 = names;
    for (int tmp117 = 0; tmp117 < tmp116.len; tmp117++) {
      string name = ((string *)tmp116.data)[tmp117];
...
Fn *Parser_fn_call_args(Parser *p, Fn *f) {
  ...
  array_Var tmp126 = f->args;
  for (int i = 0; i < tmp126.len; i++) {
    Var arg = ((Var *)tmp126.data)[i];
    ...
    if (arg.is_mut) {
      ...
      Option_Var tmp131 = Fn_find_var(&/* ? */ p->cur_fn, var_name);
      if (!tmp131.ok) {
    ...
    if (is_interface) {
      ...
      array_Fn tmp145 = interface_type.methods;
      for (int tmp146 = 0; tmp146 < tmp145.len; tmp146++) {
        Fn method = ((Fn *)tmp145.data)[tmp146];
...
string Fn_typ_str(Fn f) {
  ...
  array_Var tmp152 = f.args;
  for (int i = 0; i < tmp152.len; i++) {
    Var arg = ((Var *)tmp152.data)[i];
...
string Fn_str_args(Fn *f, Table *table) {
  ...
  array_Var tmp154 = f->args;
  for (int i = 0; i < tmp154.len; i++) {
    Var arg = ((Var *)tmp154.data)[i];
    ...
    if (Table_is_interface(&/* ? */ *table, arg.typ)) {
      ...
      array_Fn tmp156 = interface_type.methods;
      for (int tmp157 = 0; tmp157 < tmp156.len; tmp157++) {
        Fn method = ((Fn *)tmp156.data)[tmp157];
        ...
        if (method.args.len > 1) {
          ...
          array_Var tmp158 = array_right(method.args, 1);
          for (int tmp159 = 0; tmp159 < tmp158.len; tmp159++) {
            Var a = ((Var *)tmp158.data)[tmp159];
...
string Fn_find_misspelled_local_var(Fn *f, string name, f32 min_match) {
  ...
  array_Var tmp162 = f->local_vars;
  for (int tmp163 = 0; tmp163 < tmp162.len; tmp163++) {
    Var var = ((Var *)tmp162.data)[tmp163];
...
void V_add_parser(V *v, Parser parser) {
  ...
  array_Parser tmp7 = v->parsers;
  for (int tmp8 = 0; tmp8 < tmp7.len; tmp8++) {
    Parser p = ((Parser *)tmp7.data)[tmp8];

    if (string_eq(p.file_path, parser.file_path)) {

      return;
    };
  };

  _PUSH(&v->parsers, (parser), tmp9, Parser);
}
void V_compile(V *v) {
...
void V_add_v_files_to_compile(V *v) {
  ...
  for (int l = 0; l < keys_tmp75.len; l++) {
    ...
    _PUSH(&v->files, (fit.file_path), tmp76, string);
...
V *new_v(array_string args) {
  ...
  for (int tmp97 = 0; tmp97 < tmp96.len; tmp97++) {
    ...
    f = _STR("%.*s/vlib/builtin/js/%.*s", vroot.len, vroot.str, builtin.len,
             builtin.str);
...
array_string env_vflags_and_os_args() {
  ...
  if (string_ne(tos2((byte *)""), vflags)) {
    ...
    _PUSH(&args, ((*(string *)array__get(os__args, 0))), tmp109, string);

    _PUSH_MANY(&args, (string_split(vflags, tos2((byte *)" "))), tmp112,
               array_string);

    if (os__args.len > 1) {

      _PUSH_MANY(&args, (array_right(os__args, 1)), tmp113, array_string);
    };

  } else {

    _PUSH_MANY(&args, (os__args), tmp114, array_string);
  };
...
void update_v() {
  ...
  Option_os__Result tmp116 = os__exec(_STR(
  ...
  os__Result s = *(os__Result *)tmp116.data;
  ;
  ...
  os__Result s2 = *(os__Result *)tmp119.data;
  ;
...
void install_v(array_string args) {
  ...
  if (1) {
    ...
    Option_os__Result tmp125 = os__exec(
    ...
    os__Result vgetcompilation = *(os__Result *)tmp125.data;
    ;
  ...
  os__Result vgetresult = *(os__Result *)tmp126.data;
  ;
...
void V_test_v(V *v) {
  ...
  array_string tmp135 = test_files;
  for (int tmp136 = 0; tmp136 < tmp135.len; tmp136++) {
    string dot_relative_file = ((string *)tmp135.data)[tmp136];
    ...
    Option_os__Result tmp141 = os__exec(cmd);
    if (!tmp141.ok) {
    ...
    os__Result r = *(os__Result *)tmp141.data;
    ;
  ...
  array_string tmp144 = examples;
  for (int tmp145 = 0; tmp145 < tmp144.len; tmp145++) {
    string relative_file = ((string *)tmp144.data)[tmp145];
...
array_string DepGraph_imports(DepGraph *graph) {
  ...
  for (int tmp7 = 0; tmp7 < tmp6.len; tmp7++) {
    ...
    _PUSH(&mods, (node.name), tmp8, string);
  };
...
void Parser_parse(Parser *p, Pass pass) {
  ...
  if (p->pass == main__Pass_imports) {
    ...
    map__set(&p->table->file_imports, p->file_path,
  ...
  while (1) {
    ...
    if (p->tok == main__Token_key_import) { /* case */
      ...
      if (0 && !Parser_first_pass(&/* ? */ *p) &&
        ...
        Option_os__File tmp13 = os__create(tos2((byte *)"/var/tmp/fmt.v"));
        if (!tmp13.ok) {
          string err = tmp13.error;
        ...
        os__File out = *(os__File *)tmp13.data;
        ;
...
string Parser_get_type(Parser *p) {
  ...
  if (p->tok == main__Token_lpar) {
    ...
    while (1) {
      ...
      _PUSH(&types, (Parser_get_type(p)), tmp79, string);
...
void Parser_vh_genln(Parser *p, string s) {
  ...
  _PUSH(&p->vh_lines, (s), tmp102, string);
}
string Parser_statement(Parser *p, bool add_semi) {
...
void Parser_var_decl(Parser *p) {
  ...
  _PUSH(&names, (Parser_check_name(p)), tmp123, string);

  while (p->tok == main__Token_comma) {

    Parser_check(p, main__Token_comma);

    _PUSH(&names, (Parser_check_name(p)), tmp124, string);
  };
  ...
  array_string tmp130 = names;
  for (int i = 0; i < tmp130.len; i++) {
    string name = ((string *)tmp130.data)[i];
    ...
    Parser_register_var(
        p, (Var){
               ...
               .is_mut = is_mut,
               .is_alloc = p->is_alloc ||
                           string_starts_with(typ, tos2((byte *)"array_")),
...
string Parser_bterm(Parser *p) {
  ...
  if (tok == main__Token_eq || tok == main__Token_gt || tok == main__Token_lt ||
    ...
    if (p->is_sql) {
      ...
      _PUSH(&p->sql_params, (sql_param), tmp145, string);

      _PUSH(&p->sql_types, (typ), tmp146, string);
...
string Parser_name_expr(Parser *p) {
  ...
  while (1) {
    ...
    Option_Var tmp157 = Parser_find_var_check_new_var(&/* ? */ *p, name);
    if (!tmp157.ok) {
      string err = tmp157.error;

      break;
    }
    Var v = *(Var *)tmp157.data;
    ;

    if (ptr) {

      Parser_gen(p, tos2((byte *)"& /*v*/ "));
  ...
  if (is_c) {
    ...
    Option_Fn tmp163 = Table_find_fn(&/* ? */ *p->table, name);
    if (!tmp163.ok) {
  ...
  while (1) {
    ...
    Option_Var tmp164 = Table_find_const(&/* ? */ *p->table, name);
    if (!tmp164.ok) {
      string err = tmp164.error;

      break;
    }
    Var c = *(Var *)tmp164.data;
    ;
  ...
  Option_Fn tmp166 = Table_find_fn(&/* ? */ *p->table, name);
  if (!tmp166.ok) {
    ...
    if (!Parser_first_pass(&/* ? */ *p)) {
      ...
      string suggested = Table_identify_typo(
          &/* ? */ *p->table, name, &/*112 EXP:"Fn*" GOT:"Fn" */ p->cur_fn,
...
string Parser_dot(Parser *p, string str_typ, int method_ph) {
  ...
  if (has_field) {
    ...
    Option_Var tmp181 = Table_find_field(
        &/* ? */ *p->table, &/*112 EXP:"Type*" GOT:"Type" */ typ, struct_field);
    if (!tmp181.ok) {
      string err = tmp181.error;

      v_panic(tos2((byte *)"field"));
    }
    Var field = *(Var *)tmp181.data;
    ;
  ...
  Option_Fn tmp186 = Table_find_method(
      &/* ? */ *p->table, &/*112 EXP:"Type*" GOT:"Type" */ typ, field_name);
  if (!tmp186.ok) {
    string err = tmp186.error;
...
string Parser_assoc(Parser *p) {
  ...
  Option_Var tmp242 = Fn_find_var(&/* ? */ p->cur_fn, name);
  if (!tmp242.ok) {
  ...
  while (p->tok != main__Token_rcbr) {
    ...
    _PUSH(&fields, (field), tmp245, string);
  ...
  array_Var tmp247 = T.fields;
  for (int tmp248 = 0; tmp248 < tmp247.len; tmp248++) {
    Var ffield = ((Var *)tmp247.data)[tmp248];
...
string Parser_array_init(Parser *p) {
  ...
  if (p->tok == main__Token_name && !p->inside_const) {
    ...
    if (Table_known_const(&/* ? */ *p->table, const_name)) {
      ...
      Option_Var tmp279 = Table_find_const(&/* ? */ *p->table, const_name);
      if (!tmp279.ok) {
        string err = tmp279.error;

        v_exit(1);
      }
      Var c = *(Var *)tmp279.data;
      ;
...
string Parser_struct_init(Parser *p, string typ) {
  ...
  if (peek == main__Token_colon || p->tok == main__Token_rcbr) {
    ...
    while (p->tok != main__Token_rcbr) {
      ...
      Option_Var tmp299 = Type_find_field(&/* ? */ t, field);
      if (!tmp299.ok) {
        string err = tmp299.error;

        v_panic(tos2((byte *)"field"));
      }
      Var f = *(Var *)tmp299.data;
      ;

      _PUSH(&inited_fields, (field), tmp300, string);
    ...
    for (int i = 0; i < tmp301.len; i++) {
      ...
      if (string_starts_with(field_typ, tos2((byte *)"map_"))) {
        ...
        _PUSH(&inited_fields, (field.name), tmp303, string);
    ...
    array_Var tmp306 = T.fields;
    for (int i = 0; i < tmp306.len; i++) {
      Var ffield = ((Var *)tmp306.data)[i];
...
void Parser_return_st(Parser *p) {
  ...
  if (fn_returns) {
    ...
    if (p->tok == main__Token_rcbr) {
      ...
      _PUSH(&types, (expr_type), tmp369, string);

      while (p->tok == main__Token_comma) {

        Parser_check(p, main__Token_comma);

        _PUSH(&types, (Parser_bool_expression(p)), tmp370, string);
      };
      ...
      if (types.len > 1) {
        ...
        array_string tmp373 = string_split(ret_vals, tos2((byte *)" "));
        for (int ret_val_idx = 0; ret_val_idx < tmp373.len; ret_val_idx++) {
          string ret_val = ((string *)tmp373.data)[ret_val_idx];
        ...
        CGen_resetln(p->cgen, _STR("(%.*s){%.*s}", expr_type.len, expr_type.str,
      ...
      if (string_ends_with(p->cur_fn.typ, expr_type) && !is_none &&
        ...
        array_string tmp379 = p->cur_fn.defer_text;
        for (int tmp380 = 0; tmp380 < tmp379.len; tmp380++) {
          string text = ((string *)tmp379.data)[tmp380];
...
void Parser_go_statement(Parser *p) {
  ...
  if (Parser_peek(p) == main__Token_dot) {
    ...
    Option_Var tmp383 = Fn_find_var(&/* ? */ p->cur_fn, var_name);
    if (!tmp383.ok) {
      string err = tmp383.error;

      return;
    }
    Var v = *(Var *)tmp383.data;
    ;
    ...
    Option_Fn tmp385 = Table_find_method(
        &/* ? */ *p->table, &/*112 EXP:"Type*" GOT:"Type" */ typ, p->lit);
    if (!tmp385.ok) {
      string err = tmp385.error;

      v_panic(tos2((byte *)"go method"));
    }
    Fn method = *(Fn *)tmp385.data;
    ;
    ...
    Option_Fn tmp386 = Table_find_fn(&/* ? */ *p->table, p->lit);
    if (!tmp386.ok) {
      string err = tmp386.error;

      v_panic(tos2((byte *)"fn"));
    }
    Fn f = *(Fn *)tmp386.data;
    ;
...
void Parser_register_var(Parser *p, Var v) {
  ...
  if (v.line_nr == 0) {
    ...
    ScannerPos spos = Scanner_get_scanner_pos(&/* ? */ *p->scanner);

    Fn_register_var(&/* ? */ p->cur_fn, (Var){
                                            .scanner_pos = spos,
                                            .line_nr = spos.line_nr,
                                            .typ = v.typ,
                                            .name = v.name,
                                            .is_arg = v.is_arg,
                                            .is_const = v.is_const,
...
string Parser_js_decode(Parser *p) {
  ...
  if (string_eq(op, tos2((byte *)"decode"))) {
    ...
    array_Var tmp395 = T.fields;
    for (int tmp396 = 0; tmp396 < tmp395.len; tmp396++) {
      Var field = ((Var *)tmp395.data)[tmp396];
    ...
    _PUSH(&p->cgen->typedefs,
          (_STR("typedef Option %.*s;", opt_type.len, opt_type.str)), tmp399,
          string);
...
void Parser_check_unused_imports(Parser *p) {
  ...
  map_string tmp407 = p->import_table.imports;
  array_string keys_tmp407 = map_keys(&tmp407);
  for (int l = 0; l < keys_tmp407.len; l++) {
    string alias = ((string *)keys_tmp407.data)[l];
    string mod = {0};
    map_get(tmp407, alias, &mod);
  ...
  output = _STR("%.*s: the following imports were never used:%.*s",
                p->file_path.len, p->file_path.str, output.len, output.str);
...
void Parser_insert_query(Parser *p, int fn_ph) {
  ...
  Option_Var tmp36 = Fn_find_var(&/* ? */ p->cur_fn, var_name);
  if (!tmp36.ok) {
    string err = tmp36.error;
      ...
      if (!func_call && s.exit_code == 0) {
        ...
        while (r.temp_lines.len > 0) {
          ...
          if (!string_starts_with((*(string *)array__get(r.temp_lines, 0)),
            ...
            _PUSH(&r.lines, ((*(string *)array__get(r.temp_lines, 0))), tmp46,
                  string);
          };
...
string ScannerPos_str(ScannerPos s) {
  ...
  return _STR("ScannerPos{ %5d , %5d }", s.pos, s.line_nr);
}
ScannerPos Scanner_get_scanner_pos(Scanner *s) {
...
int Scanner_find_current_line_end_position(Scanner *s) {
  ...
  while (1) {
    ...
    if (string_at(s->text, lineend) == 10 ||
        string_at(s->text, lineend) == 13) {
...
Option_Fn Table_find_fn(Table *t, string name) {
  ...
  if (!isnil(f.name.str)) {
    ...
    Fn tmp23 = OPTION_CAST(Fn)(f);
    return opt_ok(&tmp23, sizeof(Fn));
  };
...
bool Table_known_fn(Table *t, string name) {
  ...
  Option_Fn tmp24 = Table_find_fn(&/* ? */ *t, name);
  if (!tmp24.ok) {
...
bool Table_known_const(Table *t, string name) {
  ...
  Option_Var tmp25 = Table_find_const(&/* ? */ *t, name);
  if (!tmp25.ok) {
...
void Table_add_field(Table *table, string type_name, string field_name,
  ...
  Type tmp27 = {0};
  bool tmp28 = map_get(table->typesmap, type_name, &tmp27);
...
bool Type_has_field(Type *t, string name) {
  ...
  Option_Var tmp31 = Type_find_field(&/* ? */ *t, name);
  if (!tmp31.ok) {
...
Option_Var Type_find_field(Type *t, string name) {
  ...
  array_Var tmp32 = t->fields;
  for (int tmp33 = 0; tmp33 < tmp32.len; tmp33++) {
    Var field = ((Var *)tmp32.data)[tmp33];

    if (string_eq(field.name, name)) {

      Var tmp34 = OPTION_CAST(Var)(field);
      return opt_ok(&tmp34, sizeof(Var));
    };
  };
...
bool Table_type_has_field(Table *table, Type *typ, string name) {
  ...
  Option_Var tmp35 = Table_find_field(&/* ? */ *table, typ, name);
  if (!tmp35.ok) {
    string err = tmp35.error;
...
Option_Var Table_find_field(Table *table, Type *typ, string name) {
  ...
  for (int tmp37 = 0; tmp37 < tmp36.len; tmp37++) {
    ...
    if (string_eq(field.name, name)) {
      ...
      Var tmp38 = OPTION_CAST(Var)(field);
      return opt_ok(&tmp38, sizeof(Var));
    };
  };
  ...
  if (string_ne(typ->parent, tos2((byte *)""))) {
    ...
    array_Var tmp40 = parent.fields;
    for (int tmp41 = 0; tmp41 < tmp40.len; tmp41++) {
      Var field = ((Var *)tmp40.data)[tmp41];

      if (string_eq(field.name, name)) {

        Var tmp42 = OPTION_CAST(Var)(field);
        return opt_ok(&tmp42, sizeof(Var));
      };
    };
  };
...
void Parser_add_method(Parser *p, string type_name, Fn f) {
  ...
  Type tmp43 = {0};
  bool tmp44 = map_get(p->table->typesmap, type_name, &tmp43);

  Type t = tmp43;

  if (string_eq(type_name, tos2((byte *)"str"))) {

    /*opt*/ printf("%d\n", t.methods.len);
  };

  _PUSH(&t.methods, (f), tmp46, Fn);
...
bool Type_has_method(Type *t, string name) {
  ...
  Option_Fn tmp47 = Type_find_method(&/* ? */ *t, name);
  if (!tmp47.ok) {
...
bool Table_type_has_method(Table *table, Type *typ, string name) {
  ...
  Option_Fn tmp48 = Table_find_method(&/* ? */ *table, typ, name);
  if (!tmp48.ok) {
...
Option_Fn Table_find_method(Table *table, Type *typ, string name) {
  ...
  Type tmp49 = {0};
  bool tmp50 = map_get(table->typesmap, typ->name, &tmp49);

  Type t = tmp49;

  array_Fn tmp52 = t.methods;
  for (int tmp53 = 0; tmp53 < tmp52.len; tmp53++) {
    Fn method = ((Fn *)tmp52.data)[tmp53];

    if (string_eq(method.name, name)) {

      Fn tmp54 = OPTION_CAST(Fn)(method);
      return opt_ok(&tmp54, sizeof(Fn));
    };
  };
  ...
  if (string_ne(typ->parent, tos2((byte *)""))) {
    ...
    array_Fn tmp56 = parent.methods;
    for (int tmp57 = 0; tmp57 < tmp56.len; tmp57++) {
      Fn method = ((Fn *)tmp56.data)[tmp57];

      if (string_eq(method.name, name)) {

        Fn tmp58 = OPTION_CAST(Fn)(method);
        return opt_ok(&tmp58, sizeof(Fn));
      };
    };
...
Option_Fn Type_find_method(Type *t, string name) {
  ...
  array_Fn tmp59 = t->methods;
  for (int tmp60 = 0; tmp60 < tmp59.len; tmp60++) {
    Fn method = ((Fn *)tmp59.data)[tmp60];

    if (string_eq(method.name, name)) {

      Fn tmp61 = OPTION_CAST(Fn)(method);
      return opt_ok(&tmp61, sizeof(Fn));
    };
  };
...
Type Table_find_type(Table *t, string name_) {
  ...
  Type tmp64 = {0};
  bool tmp65 = map_get(t->typesmap, name, &tmp64);
...
bool Parser_satisfies_interface(Parser *p, string interface_name, string _typ,
  ...
  array_Fn tmp70 = int_typ.methods;
  for (int tmp71 = 0; tmp71 < tmp70.len; tmp71++) {
    Fn method = ((Fn *)tmp70.data)[tmp71];
...
bool Table_is_interface(Table *table, string name) {
  ...
  Type tmp72 = {0};
  bool tmp73 = map_get(table->typesmap, name, &tmp72);
...
Option_Var Table_find_const(Table *t, string name) {
  ...
  array_Var tmp77 = t->consts;
  for (int tmp78 = 0; tmp78 < tmp77.len; tmp78++) {
    Var c = ((Var *)tmp77.data)[tmp78];

    if (string_eq(c.name, name)) {

      Var tmp79 = OPTION_CAST(Var)(c);
      return opt_ok(&tmp79, sizeof(Var));
    };
  };
...
array_string Table_fn_gen_types(Table *t, string fn_name) {
  ...
  array_GenTable tmp88 = t->generic_fns;
  for (int _ = 0; _ < tmp88.len; _++) {
    GenTable f = ((GenTable *)tmp88.data)[_];
...
void Table_register_generic_fn_type(Table *t, string fn_name, string typ) {
  ...
  array_GenTable tmp89 = t->generic_fns;
  for (int i = 0; i < tmp89.len; i++) {
    GenTable f = ((GenTable *)tmp89.data)[i];

    if (string_eq(f.fn_name, fn_name)) {

      _PUSH(&(*(GenTable *)array__get(t->generic_fns, i)).types, (typ), tmp92,
            string);
...
bool is_compile_time_const(string s_) {
  ...
  string tmp95 = s;
  array_byte bytes_tmp95 = string_bytes(tmp95);
  ;
  for (int tmp96 = 0; tmp96 < tmp95.len; tmp96++) {
    byte c = ((byte *)bytes_tmp95.data)[tmp96];
...
string Table_qualify_module(Table *table, string mod, string file_path) {
  ...
  array_string tmp97 = table->imports;
  for (int tmp98 = 0; tmp98 < tmp97.len; tmp98++) {
    string m = ((string *)tmp97.data)[tmp98];
...
FileImportTable Table_get_file_import_table(Table *table, string file_path) {
  ...
  FileImportTable fit = new_file_import_table(file_path);

  if (_IN_MAP((file_path), table->file_imports)) {

    FileImportTable tmp104 = {0};
    bool tmp105 = map_get(table->file_imports, file_path, &tmp104);

    fit.imports = tmp104.imports;
  };
...
void FileImportTable_register_alias(FileImportTable *fit, string alias,
  ...
  string tmp106 = tos((byte *)"", 0);
  bool tmp107 = map_get(fit->imports, alias, &tmp106);
  ...
  if (string_contains(mod, tos2((byte *)".internal."))) {
    ...
    for (int tmp111 = 0; tmp111 < tmp110.len; tmp111++) {
      ...
      _PUSH(&internal_mod_parts, (part), tmp112, string);
    };
...
string FileImportTable_resolve_alias(FileImportTable *fit, string alias) {
  ...
  bool tmp116 = map_get(fit->imports, alias, &tmp115);
...
void FileImportTable_register_used_import(FileImportTable *fit, string alias) {
  ...
  if (!(_IN(string, (alias), fit->used_imports))) {
    ...
    _PUSH(&fit->used_imports, (alias), tmp117, string);
  };
}
bool FileImportTable_is_used_import(FileImportTable *fit, string alias) {
...
bool Type_contains_field_type(Type *t, string typ) {
  ...
  array_Var tmp120 = t->fields;
  for (int tmp121 = 0; tmp121 < tmp120.len; tmp121++) {
    Var field = ((Var *)tmp120.data)[tmp121];
