...
void Fn_mark_var_returned(Fn *f, Var v);
...
void Fn_mark_var_returned(Fn *f, Var v) {
  ...
  for (int i = 0; i < tmp11.len; i++) {
    ...
    if (string_eq(vv.name, v.name)) {
      ...
      (*(Var *)array__get(f->local_vars, i)).is_returned = 1;
...
V *new_v(array_string args) {
  ...
  for (int tmp92 = 0; tmp92 < tmp91.len; tmp92++) {
    ...
    _PUSH(&files, (f), tmp94, string);
  };

  string cflags = tos2((byte *)"");

  array_string tmp96 = args;
  ;
  for (int ci = 0; ci < tmp96.len; ci++) {
    string cv = ((string *)tmp96.data)[ci];
  ...
  Preferences *pref = (Preferences *)memdup(
      &(Preferences){
          ...
          .building_v =
              !is_repl && (string_eq(dir, tos2((byte *)"compiler")) ||
...
array_string env_vflags_and_os_args() {
  ...
  if (string_ne(tos2((byte *)""), vflags)) {
    ...
    _PUSH(&args, ((*(string *)array__get(os__args, 0))), tmp104, string);

    _PUSH_MANY(&args, (string_split(vflags, tos2((byte *)" "))), tmp107,
               array_string);

    if (os__args.len > 1) {

      _PUSH_MANY(&args, (array_right(os__args, 1)), tmp108, array_string);
    };

  } else {

    _PUSH_MANY(&args, (os__args), tmp109, array_string);
  };
...
void update_v() {
  ...
  Option_os__Result tmp111 = os__exec(_STR(
  ...
  os__Result s = *(os__Result *)tmp111.data;
  ;
...
void install_v(array_string args) {
  ...
  if (1) {
    ...
    Option_os__Result tmp118 = os__exec(
    ...
    os__Result vgetcompilation = *(os__Result *)tmp118.data;
    ;
  ...
  os__Result vgetresult = *(os__Result *)tmp119.data;
  ;
...
void test_v() {
  ...
  array_string tmp127 = test_files;
  ;
  for (int tmp128 = 0; tmp128 < tmp127.len; tmp128++) {
    string dot_relative_file = ((string *)tmp127.data)[tmp128];
    ...
    Option_os__Result tmp133 = os__exec(cmd);
    if (!tmp133.ok) {
        ...
        if (p->returns) {
          ...
          if (!v.is_returned) {
            ...
            array_set(&/*q*/ p->cgen->lines, p->cgen->lines.len - 2,
                      &(string[]){string_add(
                          _STR("v_array_free(%.*s); /* :) close_scope free */",
...
string Parser_bterm(Parser *p) {
  ...
  if (tok == main__Token_eq || tok == main__Token_gt || tok == main__Token_lt ||
    ...
    if (p->is_sql) {
      ...
      _PUSH(&p->sql_params, (sql_param), tmp134, string);

      _PUSH(&p->sql_types, (typ), tmp135, string);
...
string Parser_name_expr(Parser *p) {
  ...
  if (v.name.len != 0) {
    ...
    if (p->inside_return_expr) {
      ...
      Fn_mark_var_returned(p->cur_fn, v);
    };
