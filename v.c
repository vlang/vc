#define V_COMMIT_HASH "9726e18"
#ifndef V_COMMIT_HASH
#define V_COMMIT_HASH "6d5e9f8"
#endif
#include <inttypes.h>
...
array_string string_split_nth(string s, string delim, int nth) {
  ...
  if (string_ends_with(s, delim) && (nth < 1 || res.len < nth)) {
    _PUSH(&res, (/*typ = array_string   tmp_typ=string*/ tos3("")), tmp12,
          string);
  };
  return res;
}
array_string string_split_into_lines(string s) {
  ...
  for (int i = 0; i < s.len; i++) {
    ...
    if (((int)(s.str[i] /*rbyte 0*/)) == 10 || last) {
      ...
      _PUSH(&res, (/*typ = array_string   tmp_typ=string*/ line), tmp13,
            string);
      start = i + 1;
    };
  };
...
Option_int string_index(string s, string p) {
  ...
  while (i < s.len) {
    ...
    if (j == p.len) {
      int tmp14 = OPTION_CAST(int)(i);
      return opt_ok(&tmp14, sizeof(int));
    };
    i++;
  };
  return opt_none();
...
int string_index_any(string s, string chars) {
  ...
  for (int tmp20 = 0; tmp20 < tmp19.len; tmp20++) {
    byte c = tmp19.str[tmp20];

    Option_int tmp21 = string_index(s, byte_str(c));
    int index;
    if (!tmp21.ok) {
...
bool string_contains(string s, string p) {
  Option_int tmp22 = string_index(s, p);
  if (!tmp22.ok) {
...
string string_title(string s) {
  ...
  array_string tit = new_array_from_c_array(
      0, 0, sizeof(string), EMPTY_ARRAY_OF_ELEMS(string, 0){TCCSKIP(0)});
  array_string tmp24 = words;
  for (int tmp25 = 0; tmp25 < tmp24.len; tmp25++) {
    string word = ((string *)tmp24.data)[tmp25];
...
string string_find_between(string s, string start, string end) {
  Option_int tmp27 = string_index(s, start);
  int start_pos;
  if (!tmp27.ok) {
  ...
  start_pos = *(int *)tmp27.data;
  ;
  string val = string_right(s, start_pos + start.len);
  Option_int tmp28 = string_index(val, end);
  int end_pos;
  if (!tmp28.ok) {
  ...
  end_pos = *(int *)tmp28.data;
  ;
  return string_left(val, end_pos);
}
bool array_string_contains(array_string ar, string val) {
  array_string tmp29 = ar;
  for (int tmp30 = 0; tmp30 < tmp29.len; tmp30++) {
    string s = ((string *)tmp29.data)[tmp30];
...
ustring string_ustring(string s) {
  ...
  for (int i = 0; i < s.len; i++) {
    ...
    _PUSH(&res.runes, (/*typ = array_int   tmp_typ=int*/ i), tmp32, int);
    i += char_len - 1;
    res.len++;
  };
  return res;
...
ustring ustring_add(ustring u, ustring a) {
  ...
  for (int i = 0; i < u.s.len; i++) {
    ...
    _PUSH(&res.runes, (/*typ = array_int   tmp_typ=int*/ j), tmp33, int);
    i += char_len - 1;
    j += char_len;
    res.len++;
  };
...
string string_all_before(string s, string dot) {
  Option_int tmp39 = string_index(s, dot);
  int pos;
  if (!tmp39.ok) {
...
string array_string_join(array_string a, string del) {
  ...
  int len = 0;
  array_string tmp40 = a;
  for (int i = 0; i < tmp40.len; i++) {
    string val = ((string *)tmp40.data)[i];
...
string string_repeat(string s, int count) {
  ...
  int tmp44 = 0;
  ;
  for (int tmp45 = tmp44; tmp45 < count; tmp45++) {
    int i = tmp45;

    int tmp46 = 0;
    ;
    for (int tmp47 = tmp46; tmp47 < s.len; tmp47++) {
      int j = tmp47;
...
string compiler__V_type_definitions(compiler__V *v) {
  ...
  for (int tmp38 = 0; tmp38 < tmp37.len; tmp38++) {
    ...
    bool tmp40 = map_get(/*cgen.v : 342*/ v->table->typesmap, builtin, &tmp39);
...
string compiler__Table_fn_gen_name(compiler__Table *table, compiler__Fn *f) {
  ...
  if (table->obfuscate && !f->is_c && string_ne(f->name, tos3("main")) &&
    ...
    int tmp30 = 0;
    bool tmp31 = map_get(/*gen_c.v : 296*/ table->obf_ids, name, &tmp30);
...
array_string compiler__V_get_user_files(compiler__V *v) {
  ...
  if (string_ends_with(dir, tos3(".v")) ||
    ...
    for (int tmp57 = 0; tmp57 < tmp56.len; tmp57++) {
      ...
      _PUSH(&user_files, (/*typ = array_string   tmp_typ=string*/ file), tmp58,
            string);
    };
  };
  if (user_files.len == 0) {
...
string compiler__get_param_after(string joined_args, string arg, string def) {
  ...
  Option_int tmp85 = string_index(joined_args, key);
  int pos;
  if (!tmp85.ok) {
...
string compiler__get_cmdline_option(array_string args, string param,
                                    string def) {
  bool found = 0;
  array_string tmp88 = args;
  for (int tmp89 = 0; tmp89 < tmp88.len; tmp89++) {
    string arg = ((string *)tmp88.data)[tmp89];
...
void compiler__Parser_check_not_reserved(compiler__Parser *p) {
  bool tmp36 = 0;
  bool tmp37 =
      map_get(/*parser.v : 810*/ compiler__reserved_types, p->lit, &tmp36);
...
string compiler__Parser_get_type(compiler__Parser *p) {
  ...
  if ((_IN(string, (p->lit), map_keys(&/* ? */ ti)))) {
    string tmp39 = tos3("");
    bool tmp40 = map_get(/*parser.v : 989*/ ti, p->lit, &tmp39);
  ...
  if (string_eq(p->lit, tos3("C")) &&
    ...
    if (string_eq(t.name, tos3("")) && !p->builtin_mod) {
      ...
      if (string_eq(t.name, tos3("")) && !p->pref->translated &&
          ...
          !string_starts_with(typ, tos3("["))) {
        println(tos3("get_type() bad type"));
        _V_MulRet_string_V_string _V_mret_4661_t_suggest_tc_suggest =
            compiler__Table_find_misspelled_type(&/* ? */ *p->table, typ, p,
                                                 0.50);
        string t_suggest = _V_mret_4661_t_suggest_tc_suggest.var_0;
        string tc_suggest = _V_mret_4661_t_suggest_tc_suggest.var_1;
        if (t_suggest.len > 0) {
          t_suggest = _STR(". did you mean: (%.*s) `%.*s`", tc_suggest.len,
                           tc_suggest.str, t_suggest.len, t_suggest.str);
        };
...
void compiler__Parser_assign_statement(compiler__Parser *p, compiler__Var v,
  ...
  if (string_eq(expr_type, tos3("void"))) {
    _V_MulRet_bool_V_string _V_mret_6285___fn_name =
        compiler__Parser_is_expr_fn_call(&/* ? */ *p, p->token_idx - 3);
    string fn_name = _V_mret_6285___fn_name.var_1;
    compiler__Parser_error_with_token_index(
        p,
...
void compiler__Parser_var_decl(compiler__Parser *p) {
  ...
  if (string_eq(t, tos3("void"))) {
    _V_MulRet_bool_V_string _V_mret_7139___fn_name =
        compiler__Parser_is_expr_fn_call(&/* ? */ *p, p->token_idx - 3);
    string fn_name = _V_mret_7139___fn_name.var_1;
    compiler__Parser_error_with_token_index(
        p,
        _STR("%.*s() %.*s", fn_name.len, fn_name.str,
             compiler__err_used_as_value.len, compiler__err_used_as_value.str),
...
string compiler__Parser_map_init(compiler__Parser *p) {
  ...
  if (p->tok == compiler__compiler__TokenKind_lcbr) {
    ...
    while (1) {
      ...
      compiler__Parser_check(p, compiler__compiler__TokenKind_colon);
      ;
      _V_MulRet_string_V_string _V_mret_10983_t_val_expr =
          compiler__Parser_tmp_expr(p);
      string t = _V_mret_10983_t_val_expr.var_0;
      string val_expr = _V_mret_10983_t_val_expr.var_1;
      if (i == 0) {
        val_type = t;
      };
      i++;
...
void compiler__Parser_return_st(compiler__Parser *p) {
  ...
  if (fn_returns) {
    ...
    while (p->tok == compiler__compiler__TokenKind_comma) {
      ...
      compiler__Parser_check(p, compiler__compiler__TokenKind_comma);
      _V_MulRet_string_V_string _V_mret_12336_typ_expr =
          compiler__Parser_tmp_expr(p);
      string typ = _V_mret_12336_typ_expr.var_0;
      string expr = _V_mret_12336_typ_expr.var_1;
...
string compiler__Parser_js_decode(compiler__Parser *p) {
  ...
  if (string_eq(op, tos3("decode"))) {
    ...
    string typ = compiler__Parser_get_type(p);
    compiler__Parser_check(p, compiler__compiler__TokenKind_comma);
    _V_MulRet_string_V_string _V_mret_13115_styp_expr =
        compiler__Parser_tmp_expr(p);
    string styp = _V_mret_13115_styp_expr.var_0;
    string expr = _V_mret_13115_styp_expr.var_1;
    ...
    compiler__Parser_check(p, compiler__compiler__TokenKind_lpar);
    _V_MulRet_string_V_string _V_mret_13295_typ_expr =
        compiler__Parser_tmp_expr(p);
    string typ = _V_mret_13295_typ_expr.var_0;
    string expr = _V_mret_13295_typ_expr.var_1;
    compiler__Type T = compiler__Table_find_type(&/* ? */ *p->table, typ);
    compiler__Parser_gen_json_for_type(p, T);
    compiler__Parser_check(p, compiler__compiler__TokenKind_rpar);
