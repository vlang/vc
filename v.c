#define V_COMMIT_HASH "d308fef"
#ifndef V_COMMIT_HASH
#define V_COMMIT_HASH "b31ec4c"
#endif
#include <inttypes.h>
...
array_string string_split_into_lines(string s) {
  ...
  for (int i = 0; i < s.len; i++) {
    ...
    if (((int)(s.str[i] /*rbyte 0*/)) == 10 || last) {
      ...
      _PUSH(&res, (/*typ = array_string   tmp_typ=string*/ line), tmp14,
            string);
      start = i + 1;
    };
  };
...
void compiler__Parser_comptime_method_call(compiler__Parser *p,
  ...
  for (int i = 0; i < tmp12.len; i++) {
    compiler__Fn method = ((compiler__Fn *)tmp12.data)[i];
...
void compiler__Parser_gen_struct_str(compiler__Parser *p, compiler__Type typ) {
  ...
  for (int tmp19 = 0; tmp19 < tmp18.len; tmp19++) {
    compiler__Var field = ((compiler__Var *)tmp18.data)[tmp19];
...
string compiler__get_param_after(string joined_args, string arg, string def) {
  ...
  Option_int tmp82 = string_index(joined_args, key);
  int pos;
  if (!tmp82.ok) {
...
string compiler__get_cmdline_option(array_string args, string param,
  ...
  for (int tmp86 = 0; tmp86 < tmp85.len; tmp86++) {
    string arg = ((string *)tmp85.data)[tmp86];
  ...
  while (1) {
    ...
    if (tmp16 == compiler__compiler__TokenKind_key_import) {
      ...
      if (p->tok == compiler__compiler__TokenKind_assign) {
        compiler__Parser_next(p);
        g = string_add(g, tos3(" = "));
        _V_MulRet_string_V_string _V_mret_2171___expr =
            compiler__Parser_tmp_expr(p);
        string expr = _V_mret_2171___expr.var_1;
        g = string_add(g, expr);
      };
...
void compiler__Parser_assign_statement(compiler__Parser *p, compiler__Var v,
  ...
  if (string_eq(expr_type, tos3("void"))) {
    _V_MulRet_bool_V_string _V_mret_6233___fn_name =
        compiler__Parser_is_expr_fn_call(p, p->token_idx - 3);
    string fn_name = _V_mret_6233___fn_name.var_1;
    compiler__Parser_error_with_token_index(
        p,
...
void compiler__Parser_var_decl(compiler__Parser *p) {
  ...
  if (string_eq(t, tos3("void"))) {
    _V_MulRet_bool_V_string _V_mret_7054___fn_name =
        compiler__Parser_is_expr_fn_call(p, p->token_idx - 3);
    string fn_name = _V_mret_7054___fn_name.var_1;
    compiler__Parser_error_with_token_index(
        p,
        _STR("%.*s() %.*s", fn_name.len, fn_name.str,
             compiler__err_used_as_value.len, compiler__err_used_as_value.str),
...
string compiler__Parser_assoc(compiler__Parser *p) {
  compiler__Parser_next(p);
  string name = compiler__Parser_check_name(p);
  Option_compiler__Var tmp130 = compiler__Parser_find_var_or_const(p, name);
  compiler__Var var;
  if (!tmp130.ok) {
    string err = tmp130.error;
    int errcode = tmp130.ecode;
    compiler__Parser_error(p,
                           _STR("unknown variable `%.*s`", name.len, name.str));
    v_exit(1);
  }
  var = *(compiler__Var *)tmp130.data;
  ;
  if (!var.is_const) {
  ...
  while (p->tok != compiler__compiler__TokenKind_rcbr) {
    ...
    string field = compiler__Parser_check_name(p);
    _PUSH(&fields, (/*typ = array_string   tmp_typ=string*/ field), tmp131,
          string);
    compiler__Parser_gen(p, _STR(".%.*s = ", field.len, field.str));
    compiler__Parser_check(p, compiler__compiler__TokenKind_colon);
    compiler__Parser_bool_expression(p);
  ...
  array_compiler__Var tmp132 = T.fields;
  for (int tmp133 = 0; tmp133 < tmp132.len; tmp133++) {
    compiler__Var ffield = ((compiler__Var *)tmp132.data)[tmp133];
...
void compiler__Parser_string_expr(compiler__Parser *p) {
  ...
  while (p->tok == compiler__compiler__TokenKind_str) {
    ...
    _V_MulRet_string_V_string _V_mret_11044_typ_val_ =
        compiler__Parser_tmp_expr(p);
    string typ = _V_mret_11044_typ_val_.var_0;
    string val_ = _V_mret_11044_typ_val_.var_1;
...
string compiler__Parser_if_st(compiler__Parser *p, bool is_expr,
  ...
  if (p->tok == compiler__compiler__TokenKind_name &&
    ...
    p->is_var_decl = 1;
    _V_MulRet_string_V_string _V_mret_12665_option_type_expr =
        compiler__Parser_tmp_expr(p);
    string option_type = _V_mret_12665_option_type_expr.var_0;
    string expr = _V_mret_12665_option_type_expr.var_1;
    p->is_var_decl = 0;
...
void compiler__Parser_return_st(compiler__Parser *p) {
  ...
  if (fn_returns) {
    ...
    while (p->tok == compiler__compiler__TokenKind_comma) {
      ...
      compiler__Parser_check(p, compiler__compiler__TokenKind_comma);
      _V_MulRet_string_V_string _V_mret_13435_typ_expr =
          compiler__Parser_tmp_expr(p);
      string typ = _V_mret_13435_typ_expr.var_0;
      string expr = _V_mret_13435_typ_expr.var_1;
...
string compiler__Parser_js_decode(compiler__Parser *p) {
  ...
  if (string_eq(op, tos3("decode"))) {
    ...
    string typ = compiler__Parser_get_type(p);
    compiler__Parser_check(p, compiler__compiler__TokenKind_comma);
    _V_MulRet_string_V_string _V_mret_14214_styp_expr =
        compiler__Parser_tmp_expr(p);
    string styp = _V_mret_14214_styp_expr.var_0;
    string expr = _V_mret_14214_styp_expr.var_1;
    ...
    compiler__Parser_check(p, compiler__compiler__TokenKind_lpar);
    _V_MulRet_string_V_string _V_mret_14394_typ_expr =
        compiler__Parser_tmp_expr(p);
    string typ = _V_mret_14394_typ_expr.var_0;
    string expr = _V_mret_14394_typ_expr.var_1;
    compiler__Type T = compiler__Table_find_type(&/* ? */ *p->table, typ);
    compiler__Parser_gen_json_for_type(p, T);
    compiler__Parser_check(p, compiler__compiler__TokenKind_rpar);
...
bool compiler__Table_known_type(compiler__Table *table, string typ_) {
  ...
  bool tmp10 = map_get(/*table.v : 341*/ table->typesmap, typ, &tmp9);
...
Option_compiler__Fn compiler__Table_find_fn(compiler__Table *t, string name) {
  ...
  bool tmp12 = map_get(/*table.v : 350*/ t->fns, name, &tmp11);
...
Option_compiler__Fn compiler__Table_find_fn_is_script(compiler__Table *t,
  ...
  bool tmp15 = map_get(/*table.v : 358*/ t->fns, name, &tmp14);
...
void compiler__Table_add_field(compiler__Table *table, string type_name,
  ...
  bool tmp23 = map_get(/*table.v : 434*/ table->typesmap, type_name, &tmp22);
...
void compiler__Table_add_default_val(compiler__Table *table, int idx,
  ...
  bool tmp26 = map_get(/*table.v : 447*/ table->typesmap, type_name, &tmp25);
...
void compiler__Parser_add_method(compiler__Parser *p, string type_name,
  ...
  bool tmp39 = map_get(/*table.v : 504*/ p->table->typesmap, type_name, &tmp38);
...
Option_compiler__Fn compiler__Table_find_method(compiler__Table *table,
  ...
  bool tmp44 = map_get(/*table.v : 523*/ table->typesmap, typ->name, &tmp43);
...
void compiler__Table_add_gen_type(compiler__Table *table, string type_name,
  ...
  bool tmp55 = map_get(/*table.v : 553*/ table->typesmap, type_name, &tmp54);
...
compiler__Type compiler__Table_find_type(compiler__Table *t, string name_) {
  ...
  bool tmp58 = map_get(/*table.v : 578*/ t->typesmap, name, &tmp57);
...
f32 compiler__typo_compare_name_mod(string a, string b, string b_mod) {
  ...
  Option_int tmp82 = string_index(a, tos3("__"));
  int auidx;
  if (!tmp82.ok) {
    string err = tmp82.error;
    int errcode = tmp82.ecode;
...
string compiler__Table_find_misspelled_fn(compiler__Table *table, string name,
  ...
  map_compiler__Fn tmp90 = table->fns;
...
string compiler__Table_find_misspelled_imported_mod(compiler__Table *table,
  ...
  string n1 = ((string_starts_with(name, tos3("main.")))
                   ...
                   : (name));
  map_string tmp93 = p->import_table.imports;
  array_string keys_tmp93 = map_keys(&tmp93);
  for (int l = 0; l < keys_tmp93.len; l++) {
    string alias = ((string *)keys_tmp93.data)[l];
    string mod = tos3("");
    map_get(tmp93, alias, &mod);
...
string compiler__Table_find_misspelled_const(compiler__Table *table,
  ...
  array_compiler__Var tmp94 = table->consts;
  for (int tmp95 = 0; tmp95 < tmp94.len; tmp95++) {
    compiler__Var cnst = ((compiler__Var *)tmp94.data)[tmp95];
...
compiler__Table_find_misspelled_type(compiler__Table *table, string name,
  ...
  map_compiler__Type tmp96 = table->typesmap;
...
string compiler__type_cat_str(compiler__TypeCategory tc) {
  compiler__TypeCategory tmp97 = tc;

  string tc_str =
      ((tmp97 == compiler__compiler__TypeCategory_builtin)
           ? (tos3("builtin"))
           : ((tmp97 == compiler__compiler__TypeCategory_struct_)
                  ? (tos3("struct"))
                  : ((tmp97 == compiler__compiler__TypeCategory_func)
                         ? (tos3("function"))
                         : ((tmp97 ==
