#define V_COMMIT_HASH "da43267"
#ifndef V_COMMIT_HASH
#define V_COMMIT_HASH "a1e45e3"
#endif
  ...
  while (i < s.len) {
    ...
    if (is_delim || last) {
      ...
      if (val.len > 0) {
        ...
        _PUSH(&res, (/*typ = array_string   tmp_typ=string*/ val), tmp43,
              string);
      };
...
array_string string_split_into_lines(string s) {
  ...
  for (int i = 0; i < s.len; i++) {
    ...
    if (((int)(s.str[i] /*rbyte 0*/)) == 10 || last) {
      ...
      _PUSH(&res, (/*typ = array_string   tmp_typ=string*/ line), tmp49,
            string);
...
void Parser_gen_struct_str(Parser *p, Type typ) {
  ...
  os__File_writeln(p->v->vgen_file, strings__Builder_str(sb));
...
void Parser_register_var(Parser *p, Var v) {
  ...
  if (p->var_idx >= p->local_vars.len) {
    ...
    _PUSH(&p->local_vars, (/*typ = array_Var   tmp_typ=Var*/ new_var), tmp29,
          Var);
...
void Parser_fn_decl(Parser *p) {
  ...
  if (p->tok == main__Token_lpar) {
    ...
    if (!Parser_first_pass(&/* ? */ *p) && !p->builtin_mod &&
        ...
        !Parser_fileis(&/* ? */ *p, main__vgen_file_name)) {
    ...
    _PUSH(&f.args, (/*typ = array_Var   tmp_typ=Var*/ receiver), tmp39, Var);
  ...
  if (is_sig || Parser_first_pass(&/* ? */ *p) || is_live || is_fn_header ||
    ...
    if (is_live && Parser_first_pass(&/* ? */ *p) &&
      ...
      _PUSH(&p->cgen->so_fns,
            (/*typ = array_string   tmp_typ=string*/ fn_name_cgen), tmp73,
            string);
...
void Parser_fn_args(Parser *p, Fn *f) {
  ...
  if (f->is_interface) {
    ...
    _PUSH(&f->args, (/*typ = array_Var   tmp_typ=Var*/ int_arg), tmp101, Var);
  };
  ...
  if (types_only) {
    ...
    while (p->tok != main__Token_rpar) {
      ...
      _PUSH(&f->args, (/*typ = array_Var   tmp_typ=Var*/ v), tmp105, Var);
  ...
  while (p->tok != main__Token_rpar) {
    ...
    for (int tmp111 = 0; tmp111 < tmp110.len; tmp111++) {
      ...
      _PUSH(&f->args, (/*typ = array_Var   tmp_typ=Var*/ v), tmp113, Var);
    };
...
Fn *Parser_fn_call_args(Parser *p, Fn *f) {
  ...
  for (int i = 0; i < tmp120.len; i++) {
    ...
    if (arg.is_mut) {
      ...
      Option_Var tmp125 = Parser_find_var(&/* ? */ *p, var_name);
      if (!tmp125.ok) {
...
string Fn_typ_str(Fn f) {
  ...
  array_Var tmp144 = f.args;
  for (int i = 0; i < tmp144.len; i++) {
    Var arg = ((Var *)tmp144.data)[i];
...
string Fn_str_args(Fn *f, Table *table) {
  ...
  for (int i = 0; i < tmp146.len; i++) {
    ...
    if (Table_is_interface(&/* ? */ *table, arg.typ)) {
      ...
      array_Fn tmp148 = interface_type.methods;
      for (int tmp149 = 0; tmp149 < tmp148.len; tmp149++) {
        Fn method = ((Fn *)tmp148.data)[tmp149];
        ...
        if (method.args.len > 1) {
          ...
          array_Var tmp150 = array_right(method.args, 1);
          for (int tmp151 = 0; tmp151 < tmp150.len; tmp151++) {
            Var a = ((Var *)tmp150.data)[tmp151];
...
string Parser_find_misspelled_local_var(Parser *p, string name, f32 min_match) {
  ...
  array_Var tmp154 = p->local_vars;
  for (int tmp155 = 0; tmp155 < tmp154.len; tmp155++) {
    Var var = ((Var *)tmp154.data)[tmp155];
...
void V_add_parser(V *v, Parser parser) {
  ...
  _PUSH(&v->parsers, (/*typ = array_Parser   tmp_typ=Parser*/ parser), tmp9,
        Parser);
}
void V_compile(V *v) {
  ...
  if (v->pref->is_debug) {
    ...
    array_Parser tmp11 = v->parsers;
    for (int tmp12 = 0; tmp12 < tmp11.len; tmp12++) {
      Parser q = ((Parser *)tmp11.data)[tmp12];
    ...
    for (int tmp14 = 0; tmp14 < tmp13.len; tmp14++) {
      string q = ((string *)tmp13.data)[tmp14];
  ...
  for (int tmp16 = 0; tmp16 < tmp15.len; tmp16++) {
    ...
    array_Parser tmp17 = v->parsers;
    for (int i = 0; i < tmp17.len; i++) {
      Parser p = ((Parser *)tmp17.data)[i];

      if (string_eq(p.file_path, file)) {

        Parser_parse(&/* ? */ (*(Parser *)array__get(v->parsers, i)),
                     main__Pass_decl);
  ...
  for (int tmp23 = 0; tmp23 < tmp22.len; tmp23++) {
    ...
    array_Parser tmp24 = v->parsers;
    for (int i = 0; i < tmp24.len; i++) {
      Parser p = ((Parser *)tmp24.data)[i];
  ...
  os__File_close(v->vgen_file);

  Parser vgen_parser = V_new_parser(v, main__vgen_file_name);
  ...
  if (v->pref->is_verbose) {
    ...
    array_CFlag tmp29 = V_get_os_cflags(&/* ? */ *v);
...
void V_add_v_files_to_compile(V *v) {
  ...
  if (string_ends_with(dir, tos2((byte *)".v"))) {
    ...
    for (int tmp51 = 0; tmp51 < tmp50.len; tmp51++) {
      ...
      _PUSH(&user_files, (/*typ = array_string   tmp_typ=string*/ file), tmp52,
            string);
    };
  };
  ...
  array_string tmp70 = DepGraph_imports(&/* ? */ *deps_resolved);
  for (int tmp71 = 0; tmp71 < tmp70.len; tmp71++) {
    string mod = ((string *)tmp70.data)[tmp71];
    ...
    for (int tmp75 = 0; tmp75 < tmp74.len; tmp75++) {
      ...
      if (!(_IN(string, (file), v->files))) {
        ...
        _PUSH(&v->files, (/*typ = array_string   tmp_typ=string*/ file), tmp76,
              string);
      };
    };
  };
  ...
  map_FileImportTable tmp80 = v->table->file_imports;
...
V *new_v(array_string args) {
  ...
  os__rm(main__vgen_file_name);

  Option_os__File tmp86 = os__open_append(main__vgen_file_name);
  if (!tmp86.ok) {
    string err = tmp86.error;

    v_panic(err);
  }
  os__File vgen_file = *(os__File *)tmp86.data;
  ;

  os__File_writeln(vgen_file, tos2((byte *)"module main\nimport strings"));
  ...
  array_string tmp102 = builtins;
  for (int tmp103 = 0; tmp103 < tmp102.len; tmp103++) {
    string builtin = ((string *)tmp102.data)[tmp103];
    ...
    _PUSH(&files, (/*typ = array_string   tmp_typ=string*/ f), tmp106, string);
  };
...
void update_v() {
  ...
  Option_os__Result tmp122 = os__exec(_STR(
  ...
  os__Result s = *(os__Result *)tmp122.data;
  ;
  ...
  os__Result s2 = *(os__Result *)tmp124.data;
  ;
...
void install_v(array_string args) {
  ...
  if (1) {
    ...
    Option_os__Result tmp131 = os__exec(
    ...
    os__Result vgetcompilation = *(os__Result *)tmp131.data;
    ;
  ...
  os__Result vgetresult = *(os__Result *)tmp132.data;
  ;
    ...
    for (int tmp48 = 0; tmp48 < tmp47.len; tmp48++) {
      ...
      _PUSH(&alibs,
            (/*typ = array_string   tmp_typ=string*/ string_add(
                string_add(
                    tos2((byte *)"\""),
                    os__realpath(_STR("%.*s/vlib/%.*s.obj", main__ModPath.len,
                                      main__ModPath.str, imp.len, imp.str))),
                tos2((byte *)"\""))),
            tmp49, string);
    };
...
Parser V_new_parser(V *v, string path) {
  ...
  while (1) {
    ...
    _PUSH(
        &p.tokens,
        (/*typ = array_Tok   tmp_typ=Tok*/ (Tok){.tok = res.tok,
                                                 .lit = res.lit,
                                                 .line_nr = p.scanner->line_nr,
                                                 .name_idx = 0}),
        tmp7, Tok);
  ...
  while (1) {
    ...
    if (p->tok == main__Token_key_import) { /* case */
      ...
      if (0 && !Parser_first_pass(&/* ? */ *p) &&
        ...
        Option_os__File tmp20 = os__create(tos2((byte *)"/var/tmp/fmt.v"));
        if (!tmp20.ok) {
        ...
        os__File out = *(os__File *)tmp20.data;
        ;
...
void Parser_struct_decl(Parser *p) {
  ...
  while (p->tok != main__Token_rcbr) {
    ...
    _PUSH(&names, (/*typ = array_string   tmp_typ=string*/ field_name), tmp59,
          string);
...
void Parser_enum_decl(Parser *p, string _enum_name) {
  ...
  while (p->tok == main__Token_name) {
    ...
    _PUSH(&fields, (/*typ = array_string   tmp_typ=string*/ field), tmp70,
          string);
...
string Parser_get_type(Parser *p) {
  ...
  if (p->tok == main__Token_lpar) {
    ...
    while (1) {
      ...
      _PUSH(&types,
            (/*typ = array_string   tmp_typ=string*/ Parser_get_type(p)), tmp84,
            string);
...
void Parser_vh_genln(Parser *p, string s) {
  ...
  _PUSH(&p->vh_lines, (/*typ = array_string   tmp_typ=string*/ s), tmp107,
        string);
}
string Parser_statement(Parser *p, bool add_semi) {
...
void Parser_var_decl(Parser *p) {
  ...
  array_string tmp135 = names;
  for (int i = 0; i < tmp135.len; i++) {
    string name = ((string *)tmp135.data)[i];
...
string Parser_name_expr(Parser *p) {
  ...
  while (1) {
    ...
    Option_Var tmp163 = Parser_find_var_check_new_var(&/* ? */ *p, name);
    if (!tmp163.ok) {
      string err = tmp163.error;

      break;
    }
    Var v = *(Var *)tmp163.data;
    ;
    ...
    if (p->pref->autofree && string_eq(v.typ, tos2((byte *)"string")) &&
      ...
      Parser_warn(&/* ? */ *p,
                  string_add(tos2((byte *)"setting moved "), v.typ));
  ...
  if (is_c) {
    ...
    Option_Fn tmp169 = Table_find_fn(&/* ? */ *p->table, name);
    if (!tmp169.ok) {
  ...
  while (1) {
    ...
    Option_Var tmp170 = Table_find_const(&/* ? */ *p->table, name);
    if (!tmp170.ok) {
      string err = tmp170.error;

      break;
    }
    Var c = *(Var *)tmp170.data;
    ;
  ...
  Option_Fn tmp172 = Table_find_fn(&/* ? */ *p->table, name);
  if (!tmp172.ok) {
...
string Parser_dot(Parser *p, string str_typ, int method_ph) {
  ...
  if (has_field) {
    ...
    Option_Var tmp187 = Table_find_field(
  ...
  Option_Fn tmp192 = Table_find_method(
      &/* ? */ *p->table, &/*112 EXP:"Type*" GOT:"Type" */ typ, field_name);
  if (!tmp192.ok) {
...
string Parser_assoc(Parser *p) {
  ...
  Option_Var tmp249 = Parser_find_var(&/* ? */ *p, name);
  if (!tmp249.ok) {
  ...
  while (p->tok != main__Token_rcbr) {
    ...
    _PUSH(&fields, (/*typ = array_string   tmp_typ=string*/ field), tmp252,
          string);
  ...
  for (int tmp255 = 0; tmp255 < tmp254.len; tmp255++) {
    Var ffield = ((Var *)tmp254.data)[tmp255];
...
string Parser_array_init(Parser *p) {
  ...
  if (p->tok == main__Token_name && !p->inside_const) {
    ...
    if (Table_known_const(&/* ? */ *p->table, const_name)) {
      ...
      Option_Var tmp286 = Table_find_const(&/* ? */ *p->table, const_name);
      if (!tmp286.ok) {
        string err = tmp286.error;

        v_exit(1);
      }
      Var c = *(Var *)tmp286.data;
      ;
...
string Parser_struct_init(Parser *p, string typ) {
  ...
  if (peek == main__Token_colon || p->tok == main__Token_rcbr) {
    ...
    while (p->tok != main__Token_rcbr) {
      ...
      Option_Var tmp303 = Type_find_field(&/* ? */ t, field);
      if (!tmp303.ok) {
        string err = tmp303.error;

        v_panic(tos2((byte *)"field"));
      }
      Var f = *(Var *)tmp303.data;
      ;
    ...
    array_Var tmp305 = t.fields;
    for (int i = 0; i < tmp305.len; i++) {
      Var field = ((Var *)tmp305.data)[i];
      ...
      if (string_starts_with(field_typ, tos2((byte *)"map_"))) {
        ...
        _PUSH(&inited_fields,
              (/*typ = array_string   tmp_typ=string*/ field.name), tmp307,
              string);
    ...
    array_Var tmp310 = T.fields;
    for (int i = 0; i < tmp310.len; i++) {
      Var ffield = ((Var *)tmp310.data)[i];
...
void Parser_return_st(Parser *p) {
  ...
  if (fn_returns) {
    ...
    if (p->tok == main__Token_rcbr) {
      ...
      _PUSH(&types, (/*typ = array_string   tmp_typ=string*/ expr_type), tmp373,
            string);
      ...
      if (types.len > 1) {
        ...
        array_string tmp378 = string_split(ret_vals, tos2((byte *)" "));
        for (int ret_val_idx = 0; ret_val_idx < tmp378.len; ret_val_idx++) {
          string ret_val = ((string *)tmp378.data)[ret_val_idx];
      ...
      if (string_ends_with(p->cur_fn.typ, expr_type) && !is_none &&
        ...
        array_string tmp384 = p->cur_fn.defer_text;
        for (int tmp385 = 0; tmp385 < tmp384.len; tmp385++) {
          string text = ((string *)tmp384.data)[tmp385];
...
void Parser_go_statement(Parser *p) {
  ...
  if (Parser_peek(&/* ? */ *p) == main__Token_dot) {
    ...
    Option_Var tmp388 = Parser_find_var(&/* ? */ *p, var_name);
    if (!tmp388.ok) {
      string err = tmp388.error;

      return;
    }
    Var v = *(Var *)tmp388.data;
    ;
    ...
    Option_Fn tmp390 = Table_find_method(
        &/* ? */ *p->table, &/*112 EXP:"Type*" GOT:"Type" */ typ, p->lit);
    if (!tmp390.ok) {
      string err = tmp390.error;

      v_panic(tos2((byte *)"go method"));
    }
    Fn method = *(Fn *)tmp390.data;
    ;
    ...
    Option_Fn tmp391 = Table_find_fn(&/* ? */ *p->table, p->lit);
    if (!tmp391.ok) {
      string err = tmp391.error;
...
string Parser_js_decode(Parser *p) {
  ...
  if (string_eq(op, tos2((byte *)"decode"))) {
    ...
    array_Var tmp399 = T.fields;
    for (int tmp400 = 0; tmp400 < tmp399.len; tmp400++) {
      Var field = ((Var *)tmp399.data)[tmp400];
...
void Parser_check_unused_imports(Parser *p) {
  ...
  if (Parser_fileis(&/* ? */ *p, main__vgen_file_name)) {
  ...
  map_string tmp411 = p->import_table.imports;
  array_string keys_tmp411 = map_keys(&tmp411);
  for (int l = 0; l < keys_tmp411.len; l++) {
    string alias = ((string *)keys_tmp411.data)[l];
    string mod = {0};
    map_get(tmp411, alias, &mod);
...
Scanner *new_scanner(string file_path) {
  ...
  if (!tmp1.ok) {
    ...
    verror(_STR("scanner: failed to open %.*s", file_path.len, file_path.str));
  ...
  return (Scanner *)memdup(&(Scanner){.file_path = file_path,
                                      .text = raw_text,
...
void Scanner_error(Scanner *s, string msg) {
  ...
  if (s->should_print_line_on_error && lineend > linestart) {
    ...
    string tmp105 = line;
    array_byte bytes_tmp105 = string_bytes(tmp105);
    ;
    for (int i = 0; i < tmp105.len; i++) {
      byte c = ((byte *)bytes_tmp105.data)[i];
  ...
  printf("%.*s:%d:%d: %.*s\n", s->file_path.len, s->file_path.str,
         s->line_nr + 1, column + 1, msg.len, msg.str);
...
bool Scanner_expect(Scanner *s, string want, int start_pos) {
  ...
  int tmp144 = start_pos;
  ;
  for (int tmp145 = tmp144; tmp145 < end_pos; tmp145++) {
    int pos = tmp145;
...
bool contains_capital(string s) {
  ...
  string tmp154 = s;
  array_byte bytes_tmp154 = string_bytes(tmp154);
  ;
  for (int tmp155 = 0; tmp155 < tmp154.len; tmp155++) {
    byte c = ((byte *)bytes_tmp154.data)[tmp155];
...
bool good_type_name(string s) {
  ...
  int tmp156 = 2;
  ;
  for (int tmp157 = tmp156; tmp157 < s.len; tmp157++) {
    int i = tmp157;
...
void Table_register_module(Table *t, string mod) {
  ...
  _PUSH(&t->modules, (/*typ = array_string   tmp_typ=string*/ mod), tmp11,
        string);
}
void Parser_register_array(Parser *p, string typ) {
...
void Parser_add_method(Parser *p, string type_name, Fn f) {
  ...
  _PUSH(&t.methods, (/*typ = array_Fn   tmp_typ=Fn*/ f), tmp46, Fn);
  ...
  for (int i = 0; i < tmp89.len; i++) {
    ...
    if (string_eq(f.fn_name, fn_name)) {
      ...
      _PUSH(&(*(GenTable *)array__get(t->generic_fns, i)).types,
            (/*typ = array_string   tmp_typ=string*/ typ), tmp92, string);
...
void init_consts() {
  ...
  main__vgen_file_name = tos2((byte *)"vgen.tmp");
