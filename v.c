#define V_COMMIT_HASH "04e4018"
#ifndef V_COMMIT_HASH
#define V_COMMIT_HASH "b1f0df0"
#endif
...
void Parser_register_var(Parser *p, Var v) {
  ...
  if (p->var_idx >= p->local_vars.len) {
    ...
    _PUSH(&p->local_vars, (new_var), tmp29, Var);
...
void Parser_fn_decl(Parser *p) {
  ...
  if (string_starts_with(typ, tos2((byte *)"_V_MulRet_")) &&
    ...
    _PUSH(
        &p->cgen->typedefs,
        (_STR("typedef struct %.*s %.*s;", typ.len, typ.str, typ.len, typ.str)),
        tmp48, string);
  };
...
Fn *Parser_fn_call_args(Parser *p, Fn *f) {
  ...
  for (int i = 0; i < tmp120.len; i++) {
    ...
    if (is_interface) {
      ...
      array_Fn tmp141 = interface_type.methods;
      for (int tmp142 = 0; tmp142 < tmp141.len; tmp142++) {
        Fn method = ((Fn *)tmp141.data)[tmp142];
...
string Fn_str_args(Fn *f, Table *table) {
  ...
  array_Var tmp150 = f->args;
  for (int i = 0; i < tmp150.len; i++) {
    Var arg = ((Var *)tmp150.data)[i];
    ...
    if (Table_is_interface(&/* ? */ *table, arg.typ)) {
      ...
      array_Fn tmp152 = interface_type.methods;
      for (int tmp153 = 0; tmp153 < tmp152.len; tmp153++) {
        Fn method = ((Fn *)tmp152.data)[tmp153];
        ...
        if (method.args.len > 1) {
          ...
          array_Var tmp154 = array_right(method.args, 1);
          for (int tmp155 = 0; tmp155 < tmp154.len; tmp155++) {
            Var a = ((Var *)tmp154.data)[tmp155];
...
string Parser_find_misspelled_local_var(Parser *p, string name, f32 min_match) {
  ...
  array_Var tmp158 = p->local_vars;
  for (int tmp159 = 0; tmp159 < tmp158.len; tmp159++) {
    Var var = ((Var *)tmp158.data)[tmp159];
...
void V_add_v_files_to_compile(V *v) {
  ...
  if (string_ends_with(dir, tos2((byte *)".v"))) {
    ...
    for (int tmp44 = 0; tmp44 < tmp43.len; tmp44++) {
      ...
      _PUSH(&user_files, (file), tmp45, string);
    };
  };
  ...
  array_string tmp63 = DepGraph_imports(&/* ? */ *deps_resolved);
  for (int tmp64 = 0; tmp64 < tmp63.len; tmp64++) {
    string mod = ((string *)tmp63.data)[tmp64];
    ...
    for (int tmp68 = 0; tmp68 < tmp67.len; tmp68++) {
      ...
      if (!(_IN(string, (file), v->files))) {
        ...
        _PUSH(&v->files, (file), tmp69, string);
      };
    };
  };
  ...
  map_FileImportTable tmp73 = v->table->file_imports;
...
array_string env_vflags_and_os_args() {
  ...
  if (string_ne(tos2((byte *)""), vflags)) {
    ...
    _PUSH_MANY(&args, (string_split(vflags, tos2((byte *)" "))), tmp110,
               array_string);
...
void update_v() {
  ...
  Option_os__Result tmp114 = os__exec(_STR(
...
void install_v(array_string args) {
  ...
  if (1) {
    ...
    Option_os__Result tmp123 = os__exec(
...
string Parser_bterm(Parser *p) {
  ...
  if (tok == main__Token_eq || tok == main__Token_gt || tok == main__Token_lt ||
    ...
    if (p->is_sql) {
      ...
      _PUSH(&p->sql_params, (sql_param), tmp148, string);

      _PUSH(&p->sql_types, (typ), tmp149, string);
...
string Parser_name_expr(Parser *p) {
  ...
  while (1) {
    ...
    Option_Var tmp167 = Table_find_const(&/* ? */ *p->table, name);
    if (!tmp167.ok) {
      string err = tmp167.error;

      break;
    }
    Var c = *(Var *)tmp167.data;
    ;
...
string Parser_dot(Parser *p, string str_typ, int method_ph) {
  ...
  if (has_field) {
    ...
    Option_Var tmp184 = Table_find_field(
  ...
  Option_Fn tmp189 = Table_find_method(
      &/* ? */ *p->table, &/*112 EXP:"Type*" GOT:"Type" */ typ, field_name);
  if (!tmp189.ok) {
...
string Parser_assoc(Parser *p) {
  ...
  Option_Var tmp246 = Parser_find_var(&/* ? */ *p, name);
  if (!tmp246.ok) {
  ...
  while (p->tok != main__Token_rcbr) {
    ...
    _PUSH(&fields, (field), tmp249, string);
  ...
  array_Var tmp251 = T.fields;
  for (int tmp252 = 0; tmp252 < tmp251.len; tmp252++) {
    Var ffield = ((Var *)tmp251.data)[tmp252];
...
string Parser_array_init(Parser *p) {
  ...
  if (p->tok == main__Token_name && !p->inside_const) {
    ...
    if (Table_known_const(&/* ? */ *p->table, const_name)) {
      ...
      Option_Var tmp283 = Table_find_const(&/* ? */ *p->table, const_name);
      if (!tmp283.ok) {
        string err = tmp283.error;

        v_exit(1);
      }
      Var c = *(Var *)tmp283.data;
      ;
...
string Parser_struct_init(Parser *p, string typ) {
  ...
  if (peek == main__Token_colon || p->tok == main__Token_rcbr) {
    ...
    while (p->tok != main__Token_rcbr) {
      ...
      _PUSH(&inited_fields, (field), tmp304, string);
    ...
    for (int i = 0; i < tmp305.len; i++) {
      ...
      if (string_starts_with(field_typ, tos2((byte *)"map_"))) {
        ...
        _PUSH(&inited_fields, (field.name), tmp307, string);
...
void Parser_return_st(Parser *p) {
  ...
  if (fn_returns) {
    ...
    if (p->tok == main__Token_rcbr) {
      ...
      _PUSH(&types, (expr_type), tmp373, string);

      while (p->tok == main__Token_comma) {

        Parser_check(p, main__Token_comma);

        _PUSH(&types, (Parser_bool_expression(p)), tmp374, string);
      };
...
string Parser_js_decode(Parser *p) {
  ...
  if (string_eq(op, tos2((byte *)"decode"))) {
    ...
    _PUSH(&p->cgen->typedefs,
          (_STR("typedef Option %.*s;", opt_type.len, opt_type.str)), tmp403,
          string);
...
bool Scanner_expect(Scanner *s, string want, int start_pos) {
  ...
  int tmp150 = start_pos;
  ;
  for (int tmp151 = tmp150; tmp151 < end_pos; tmp151++) {
    int pos = tmp151;
...
void Scanner_create_type_string(Scanner *s, Type T, string name) {
  ...
  array_Var tmp180 = T.fields;
  for (int i = 0; i < tmp180.len; i++) {
    Var field = ((Var *)tmp180.data)[i];
...
bool good_type_name(string s) {
  ...
  int tmp185 = 2;
  ;
  for (int tmp186 = tmp185; tmp186 < s.len; tmp186++) {
    int i = tmp186;
