#define V_COMMIT_HASH "0cf3e2e"
#ifndef V_COMMIT_HASH
#define V_COMMIT_HASH "3741330"
#endif
...
array_string compiler__Parser_replace_type_params(compiler__Parser *p,
  ...
  for (int _ = 0; _ < tmp175.len; _++) {
    ...
    if (_IN(string, (fi), map_keys(&/* ? */ ti.inst))) {
      ...
      bool tmp186 = map_get(/*fn.v : 1166*/ ti.inst, fi, &tmp185);

      if (!tmp186)
        tmp185 = tos((byte *)"", 0);

      fr = string_add(fr, tmp185);
    ...
    _PUSH(&r, (/*typ = array_string   tmp_typ=string*/ fr), tmp187, string);
  };
...
compiler__Parser_fn_call_vargs(compiler__Parser *p, compiler__Fn f) {
  ...
  while (p->tok != compiler__compiler__TokenKind_rpar) {
    ...
    if (!f.is_generic) {
      ...
      if (types.len > 0) {
        ...
        array_string tmp197 = types;
        for (int tmp198 = 0; tmp198 < tmp197.len; tmp198++) {
          string t = ((string *)tmp197.data)[tmp198];
    ...
    _PUSH(&types, (/*typ = array_string   tmp_typ=string*/ varg_type), tmp200,
          string);
  ...
  array_compiler__VargAccess tmp202 = p->table->varg_access;
  for (int tmp203 = 0; tmp203 < tmp202.len; tmp203++) {
    compiler__VargAccess va = ((compiler__VargAccess *)tmp202.data)[tmp203];
...
string compiler__Parser_register_multi_return_stuct(compiler__Parser *p,
  ...
  array_string tmp206 = string_split(
      string_replace(string_replace(typ, tos3("_V_MulRet_"), tos3("")),
                     tos3("_PTR_"), tos3("*")),
      tos3("_V_"));
  for (int i = 0; i < tmp206.len; i++) {
    string t = ((string *)tmp206.data)[i];
  ...
  for (int tmp209 = 0; tmp209 < tmp208.len; tmp209++) {
    ...
    f->name = string_add(string_add(f->name, tos3("_")),
                         compiler__type_to_safe_str(
                             string_replace(tmp210, tos3("..."), tos3(""))));
  };
}
void compiler__Parser_dispatch_generic_fn_instance(compiler__Parser *p,
                                                   compiler__Fn *f,
  ...
  array_compiler__TypeInst tmp213 = f->type_inst;
  for (int tmp214 = 0; tmp214 < tmp213.len; tmp214++) {
    compiler__TypeInst e = ((compiler__TypeInst *)tmp213.data)[tmp214];
  ...
  if (!new_inst) {
    ...
    compiler__Fn _f = *(compiler__Fn *)tmp215.data;
    ;
  ...
  int tmp228 = 0;
  ;
  for (int tmp229 = tmp228; tmp229 < new_types.len - 1; tmp229++) {
    int i = tmp229;
  ...
  if (_IN_MAP((f->typ), ti.inst)) {
    ...
    string tmp236 = tos((byte *)"", 0);
    bool tmp237 = map_get(/*fn.v : 1324*/ ti.inst, f->typ, &tmp236);
  ...
  array_compiler__Var tmp238 = f->args;
  for (int tmp239 = 0; tmp239 < tmp238.len; tmp239++) {
    compiler__Var arg = ((compiler__Var *)tmp238.data)[tmp239];
  ...
  array_string tmp243 = p->cgen->lines;
  for (int tmp244 = 0; tmp244 < tmp243.len; tmp244++) {
    string l = ((string *)tmp243.data)[tmp244];

    _PUSH(&p->cgen->fns, (/*typ = array_string   tmp_typ=string*/ l), tmp245,
          string);
  };
...
string compiler__Fn_typ_str(compiler__Fn *f) {
  ...
  array_compiler__Var tmp247 = f->args;
  for (int i = 0; i < tmp247.len; i++) {
    compiler__Var arg = ((compiler__Var *)tmp247.data)[i];
...
string compiler__Fn_str_args(compiler__Fn *f, compiler__Table *table) {
  ...
  for (int i = 0; i < tmp249.len; i++) {
    ...
    if (compiler__Table_is_interface(&/* ? */ *table, arg.typ)) {
      ...
      array_compiler__Fn tmp251 = interface_type.methods;
      for (int tmp252 = 0; tmp252 < tmp251.len; tmp252++) {
        compiler__Fn method = ((compiler__Fn *)tmp251.data)[tmp252];
        ...
        if (method.args.len > 1) {
          ...
          array_compiler__Var tmp253 = array_right(method.args, 1);
          for (int tmp254 = 0; tmp254 < tmp253.len; tmp254++) {
            compiler__Var a = ((compiler__Var *)tmp253.data)[tmp254];
...
string compiler__Parser_find_misspelled_local_var(compiler__Parser *p,
  ...
  array_compiler__Var tmp257 = p->local_vars;
  for (int tmp258 = 0; tmp258 < tmp257.len; tmp258++) {
    compiler__Var var = ((compiler__Var *)tmp257.data)[tmp258];
...
bool array_compiler__Fn_contains(array_compiler__Fn fns, compiler__Fn f) {
  ...
  array_compiler__Fn tmp261 = fns;
  for (int tmp262 = 0; tmp262 < tmp261.len; tmp262++) {
    compiler__Fn ff = ((compiler__Fn *)tmp261.data)[tmp262];
...
void compiler__Parser_parse(compiler__Parser *p, compiler__Pass pass) {
  ...
  while (1) {
    ...
    if (p->tok == compiler__compiler__TokenKind_key_import) { /* case */
    ...
    } else if (p->tok == compiler__compiler__TokenKind_key_fn) { /* case */
      ...
      if (0 && !compiler__Parser_first_pass(&/* ? */ *p) &&
        ...
        Option_os__File tmp32 = os__create(tos3("/var/tmp/fmt.v"));
        if (!tmp32.ok) {
        ...
        os__File out = *(os__File *)tmp32.data;
        ;
...
void compiler__Parser_const_decl(compiler__Parser *p) {
  ...
  while (p->tok == compiler__compiler__TokenKind_name) {
    ...
    Option_compiler__Var tmp49 =
        compiler__Table_find_const(&/* ? */ *p->v->table, name);

    if (tmp49.ok) {
      compiler__Var my_const = *(compiler__Var *)tmp49.data;

      if (string_eq(my_const.typ, tos3("void"))) {

        array_compiler__Var tmp50 = p->v->table->consts;
        for (int i = 0; i < tmp50.len; i++) {
          compiler__Var v = ((compiler__Var *)tmp50.data)[i];
...
void compiler__Parser_struct_decl(compiler__Parser *p) {
  ...
  while (p->tok != compiler__compiler__TokenKind_rcbr) {
    ...
    _PUSH(&names, (/*typ = array_string   tmp_typ=string*/ field_name), tmp83,
          string);
    ...
    if (p->tok == compiler__compiler__TokenKind_lsbr) {
      ...
      if (p->tok == compiler__compiler__TokenKind_colon) {
        ...
        compiler__TokenKind tmp90 = p->tok;

        if (tmp90 == compiler__compiler__TokenKind_name) {

          val = compiler__Parser_check_name(p);

        } else if (tmp90 == compiler__compiler__TokenKind_str) {
...
void compiler__Parser_var_decl(compiler__Parser *p) {
  ...
  while (p->tok == compiler__compiler__TokenKind_comma) {
    ...
    if (p->tok == compiler__compiler__TokenKind_key_mut) {
      ...
      _PUSH(&var_mut, (/*typ = array_bool   tmp_typ=bool*/ 1), tmp162, bool);

    } else {

      _PUSH(&var_mut, (/*typ = array_bool   tmp_typ=bool*/ 0), tmp163, bool);
    };
  ...
  array_string tmp176 = var_names;
  for (int i = 0; i < tmp176.len; i++) {
    string var_name = ((string *)tmp176.data)[i];
    ...
    if (var_names.len > 1) {
      ...
      if (known_var) {
        ...
        Option_compiler__Var tmp188 =
            compiler__Parser_find_var(&/* ? */ *p, var_name);
        if (!tmp188.ok) {
          string err = tmp188.error;
        ...
        compiler__Var v = *(compiler__Var *)tmp188.data;
        ;
...
string compiler__Parser_dot(compiler__Parser *p, string str_typ_,
  ...
  if (has_field) {
    ...
    Option_compiler__Var tmp246 = compiler__Table_find_field(
        &/* ? */ *p->table,
    ...
    compiler__Var field = *(compiler__Var *)tmp246.data;
    ;
  ...
  Option_compiler__Fn tmp250 = compiler__Table_find_method(
      &/* ? */ *p->table,
      &/*112 EXP:"compiler__Type*" GOT:"compiler__Type" */ typ, field_name);
  if (!tmp250.ok) {
  ...
  compiler__Fn method = *(compiler__Fn *)tmp250.data;
  ;
...
string compiler__Parser_assoc(compiler__Parser *p) {
  ...
  Option_compiler__Var tmp312 = compiler__Parser_find_var(&/* ? */ *p, name);
  if (!tmp312.ok) {
    string err = tmp312.error;
  ...
  compiler__Var var = *(compiler__Var *)tmp312.data;
  ;
  ...
  while (p->tok != compiler__compiler__TokenKind_rcbr) {
    ...
    _PUSH(&fields, (/*typ = array_string   tmp_typ=string*/ field), tmp315,
          string);
  ...
  array_compiler__Var tmp317 = T.fields;
  for (int tmp318 = 0; tmp318 < tmp317.len; tmp318++) {
    compiler__Var ffield = ((compiler__Var *)tmp317.data)[tmp318];
...
string compiler__Parser_array_init(compiler__Parser *p) {
  ...
  if (p->tok == compiler__compiler__TokenKind_name && !p->inside_const) {
    ...
    if (compiler__Table_known_const(&/* ? */ *p->table, const_name)) {
      ...
      Option_compiler__Var tmp351 =
          compiler__Table_find_const(&/* ? */ *p->table, const_name);
      if (!tmp351.ok) {
        string err = tmp351.error;

        v_exit(1);
      }
      compiler__Var c = *(compiler__Var *)tmp351.data;
      ;
...
string compiler__Parser_struct_init(compiler__Parser *p, string typ) {
  ...
  if (peek == compiler__compiler__TokenKind_colon ||
    ...
    while (p->tok != compiler__compiler__TokenKind_rcbr) {
      ...
      Option_compiler__Var tmp368 =
          compiler__Type_find_field(&/* ? */ t, field);
      if (!tmp368.ok) {
        string err = tmp368.error;
      ...
      compiler__Var f = *(compiler__Var *)tmp368.data;
      ;
    ...
    array_compiler__Var tmp370 = t.fields;
    for (int i = 0; i < tmp370.len; i++) {
      compiler__Var field = ((compiler__Var *)tmp370.data)[i];
      ...
      if (string_starts_with(field_typ, tos3("map_"))) {
        ...
        _PUSH(&inited_fields,
              (/*typ = array_string   tmp_typ=string*/ sanitized_name), tmp373,
              string);
    ...
    array_compiler__Var tmp376 = T.fields;
    for (int i = 0; i < tmp376.len; i++) {
      compiler__Var ffield = ((compiler__Var *)tmp376.data)[i];
...
void compiler__Parser_return_st(compiler__Parser *p) {
  ...
  if (fn_returns) {
    ...
    _PUSH(&types, (/*typ = array_string   tmp_typ=string*/ expr_type), tmp447,
          string);
    ...
    if (types.len > 1) {
      ...
      array_string tmp452 = mr_values;
      for (int ret_val_idx = 0; ret_val_idx < tmp452.len; ret_val_idx++) {
        string ret_val = ((string *)tmp452.data)[ret_val_idx];
...
string compiler__Parser_get_deferred_text(compiler__Parser p) {
  ...
  array_string tmp459 = p.cur_fn.defer_text;
  for (int tmp460 = 0; tmp460 < tmp459.len; tmp460++) {
    string text = ((string *)tmp459.data)[tmp460];
...
void compiler__Parser_go_statement(compiler__Parser *p) {
  ...
  if (compiler__Parser_peek(&/* ? */ *p) == compiler__compiler__TokenKind_dot) {
    ...
    Option_compiler__Var tmp463 =
        compiler__Parser_find_var(&/* ? */ *p, var_name);
    if (!tmp463.ok) {
      string err = tmp463.error;

      return;
    }
    compiler__Var v = *(compiler__Var *)tmp463.data;
    ;
    ...
    Option_compiler__Fn tmp465 = compiler__Table_find_method(
        &/* ? */ *p->table,
        &/*112 EXP:"compiler__Type*" GOT:"compiler__Type" */ typ, p->lit);
    if (!tmp465.ok) {
      string err = tmp465.error;
    ...
    compiler__Fn method = *(compiler__Fn *)tmp465.data;
    ;
    ...
    Option_compiler__Fn tmp467 = compiler__Table_find_fn(
        &/* ? */ *p->table, compiler__Parser_prepend_mod(&/* ? */ *p, f_name));
    if (!tmp467.ok) {
      string err = tmp467.error;
    ...
    compiler__Fn f = *(compiler__Fn *)tmp467.data;
    ;
...
string compiler__Parser_js_decode(compiler__Parser *p) {
  ...
  if (string_eq(op, tos3("decode"))) {
    ...
    array_compiler__Var tmp476 = T.fields;
    for (int tmp477 = 0; tmp477 < tmp476.len; tmp477++) {
      compiler__Var field = ((compiler__Var *)tmp476.data)[tmp477];
...
void compiler__Parser_check_unused_imports(compiler__Parser *p) {
  ...
  map_string tmp489 = p->import_table.imports;
  array_string keys_tmp489 = map_keys(&tmp489);
  for (int l = 0; l < keys_tmp489.len; l++) {
    string alias = ((string *)keys_tmp489.data)[l];
    string mod = {0};
    map_get(tmp489, alias, &mod);
...
bool compiler__Table_known_type(compiler__Table *table, string typ_) {
  ...
  bool tmp15 = map_get(/*table.v : 348*/ table->typesmap, typ, &tmp14);
...
Option_compiler__Fn compiler__Table_find_fn(compiler__Table *t, string name) {
  ...
  bool tmp18 = map_get(/*table.v : 357*/ t->fns, name, &tmp17);
...
Option_compiler__Fn compiler__Table_find_fn_is_script(compiler__Table *t,
  ...
  bool tmp22 = map_get(/*table.v : 365*/ t->fns, name, &tmp21);
...
void compiler__Table_add_field(compiler__Table *table, string type_name,
  ...
  bool tmp32 = map_get(/*table.v : 439*/ table->typesmap, type_name, &tmp31);
...
void compiler__Parser_add_method(compiler__Parser *p, string type_name,
  ...
  bool tmp48 = map_get(/*table.v : 500*/ p->table->typesmap, type_name, &tmp47);
...
Option_compiler__Fn compiler__Table_find_method(compiler__Table *table,
  ...
  bool tmp54 = map_get(/*table.v : 519*/ table->typesmap, typ->name, &tmp53);
...
compiler__Type compiler__Table_find_type(compiler__Table *t, string name_) {
  ...
  bool tmp69 = map_get(/*table.v : 576*/ t->typesmap, name, &tmp68);
...
bool compiler__Table_is_interface(compiler__Table *table, string name) {
  ...
  bool tmp77 = map_get(/*table.v : 739*/ table->typesmap, name, &tmp76);
...
compiler__Table_get_file_import_table(compiler__Table *table,
  ...
  if (_IN_MAP((file_path_id), table->file_imports)) {
    ...
    bool tmp103 =
        map_get(/*table.v : 886*/ table->file_imports, file_path_id, &tmp102);
...
void compiler__FileImportTable_register_alias(compiler__FileImportTable *fit,
  ...
  bool tmp105 = map_get(/*table.v : 908*/ fit->imports, alias, &tmp104);
...
int compiler__FileImportTable_get_import_tok_idx(compiler__FileImportTable *fit,
  ...
  int tmp112 = 0;
  bool tmp113 = map_get(/*table.v : 929*/ fit->import_tok_idx, mod, &tmp112);
...
string compiler__FileImportTable_resolve_alias(compiler__FileImportTable *fit,
  ...
  bool tmp116 = map_get(/*table.v : 946*/ fit->imports, alias, &tmp115);
