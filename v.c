...
bool string_contains(string s, string p) {
  Option_int tmp35 = string_index(s, p);
  if (!tmp35.ok) {
...
string string_title(string s) {
  ...
  array_string tit = new_array_from_c_array(
      0, 0, sizeof(string), EMPTY_ARRAY_OF_ELEMS(string, 0){TCCSKIP(0)});
  array_string tmp37 = words;
  for (int tmp38 = 0; tmp38 < tmp37.len; tmp38++) {
    string word = ((string *)tmp37.data)[tmp38];
...
string string_find_between(string s, string start, string end) {
  Option_int tmp40 = string_index(s, start);
  int start_pos;
  if (!tmp40.ok) {
  ...
  start_pos = *(int *)tmp40.data;
  ;
  string val = string_right(s, start_pos + start.len);
  Option_int tmp41 = string_index(val, end);
  int end_pos;
  if (!tmp41.ok) {
  ...
  end_pos = *(int *)tmp41.data;
  ;
  return string_left(val, end_pos);
}
bool array_string_contains(array_string ar, string val) {
  array_string tmp42 = ar;
  for (int tmp43 = 0; tmp43 < tmp42.len; tmp43++) {
    string s = ((string *)tmp42.data)[tmp43];
...
ustring string_ustring(string s) {
  ...
  for (int i = 0; i < s.len; i++) {
    ...
    _PUSH(&res.runes, (/*typ = array_int   tmp_typ=int*/ i), tmp45, int);
    i += char_len - 1;
    res.len++;
  };
  return res;
...
ustring ustring_add(ustring u, ustring a) {
  ...
  for (int i = 0; i < u.s.len; i++) {
    ...
    _PUSH(&res.runes, (/*typ = array_int   tmp_typ=int*/ j), tmp46, int);
    i += char_len - 1;
    j += char_len;
    res.len++;
  };
...
string string_all_before(string s, string dot) {
  Option_int tmp52 = string_index(s, dot);
  int pos;
  if (!tmp52.ok) {
...
string array_string_join(array_string a, string del) {
  ...
  int len = 0;
  array_string tmp53 = a;
  for (int i = 0; i < tmp53.len; i++) {
    string val = ((string *)tmp53.data)[i];
  ...
  int idx = 0;
  array_string tmp54 = a;
  for (int i = 0; i < tmp54.len; i++) {
    string val = ((string *)tmp54.data)[i];
...
string string_repeat(string s, int count) {
  ...
  int tmp57 = 0;
  ;
  for (int tmp58 = tmp57; tmp58 < count; tmp58++) {
    int i = tmp58;

    int tmp59 = 0;
    ;
    for (int tmp60 = tmp59; tmp60 < s.len; tmp60++) {
      int j = tmp60;
...
void compiler__Parser_assign_statement(compiler__Parser *p, compiler__Var v,
  ...
  if (string_eq(expr_type, tos3("void"))) {
    _V_MulRet_bool_V_string _V_mret_6318___fn_name =
        compiler__Parser_is_expr_fn_call(&/* ? */ *p, p->token_idx - 3);
    string fn_name = _V_mret_6318___fn_name.var_1;
    compiler__Parser_error_with_token_index(
        p,
...
void compiler__Parser_var_decl(compiler__Parser *p) {
  ...
  if (string_eq(t, tos3("void"))) {
    _V_MulRet_bool_V_string _V_mret_7172___fn_name =
        compiler__Parser_is_expr_fn_call(&/* ? */ *p, p->token_idx - 3);
    string fn_name = _V_mret_7172___fn_name.var_1;
    compiler__Parser_error_with_token_index(
        p,
        _STR("%.*s() %.*s", fn_name.len, fn_name.str,
             compiler__err_used_as_value.len, compiler__err_used_as_value.str),
...
string compiler__Parser_map_init(compiler__Parser *p) {
  ...
  if (p->tok == compiler__compiler__TokenKind_lcbr) {
    ...
    while (1) {
      ...
      compiler__Parser_check(p, compiler__compiler__TokenKind_colon);
      ;
      _V_MulRet_string_V_string _V_mret_11028_t_val_expr =
          compiler__Parser_tmp_expr(p);
      string t = _V_mret_11028_t_val_expr.var_0;
      string val_expr = _V_mret_11028_t_val_expr.var_1;
      if (i == 0) {
        val_type = t;
      };
      i++;
...
void compiler__Parser_return_st(compiler__Parser *p) {
  ...
  if (fn_returns) {
    ...
    while (p->tok == compiler__compiler__TokenKind_comma) {
      ...
      compiler__Parser_check(p, compiler__compiler__TokenKind_comma);
      _V_MulRet_string_V_string _V_mret_12381_typ_expr =
          compiler__Parser_tmp_expr(p);
      string typ = _V_mret_12381_typ_expr.var_0;
      string expr = _V_mret_12381_typ_expr.var_1;
...
string compiler__Parser_js_decode(compiler__Parser *p) {
  ...
  if (string_eq(op, tos3("decode"))) {
    ...
    string typ = compiler__Parser_get_type(p);
    compiler__Parser_check(p, compiler__compiler__TokenKind_comma);
    _V_MulRet_string_V_string _V_mret_13160_styp_expr =
        compiler__Parser_tmp_expr(p);
    string styp = _V_mret_13160_styp_expr.var_0;
    string expr = _V_mret_13160_styp_expr.var_1;
    ...
    compiler__Parser_check(p, compiler__compiler__TokenKind_lpar);
    _V_MulRet_string_V_string _V_mret_13340_typ_expr =
        compiler__Parser_tmp_expr(p);
    string typ = _V_mret_13340_typ_expr.var_0;
    string expr = _V_mret_13340_typ_expr.var_1;
    compiler__Type T = compiler__Table_find_type(&/* ? */ *p->table, typ);
    compiler__Parser_gen_json_for_type(p, T);
    compiler__Parser_check(p, compiler__compiler__TokenKind_rpar);
