#define V_COMMIT_HASH "0796e1d"
#ifndef V_COMMIT_HASH
#define V_COMMIT_HASH "febd532"
#endif
...
void map_exists(map m, string key) {
  ...
  v_panic(tos3("map.exists(key) was removed from the language. Use `key in "
               "map` instead."));
}
bool map__exists(map m, string key) {
...
f32 strings__dice_coefficient(string s1, string s2) {
  ...
  for (int i = 0; i < a.len - 1; i++) {
    ...
    map__set(&first_bigrams, bigram, &(int[]){q});
  };
...
void V_cc(V *v) {
  ...
  os__Result res = *(os__Result *)tmp36.data;
  ;
...
void V_cc_windows_cross(V *c) {
  ...
  if (c->pref->build_mode == main__BuildMode_default_mode) {
    ...
    array_string tmp44 = c->table->imports;
    for (int tmp45 = 0; tmp45 < tmp44.len; tmp45++) {
      string imp = ((string *)tmp44.data)[tmp45];
...
string main__get_cmdline_cflags(array_string args) {
  ...
  array_string tmp60 = args;
  for (int ci = 0; ci < tmp60.len; ci++) {
    string cv = ((string *)tmp60.data)[ci];

    if (string_eq(cv, tos3("-cflags"))) {

      cflags = string_add(
          cflags, string_add((*(string *)array__get(args, ci + 1)), tos3(" ")));
    };
  };
...
string V_type_definitions(V *v) {
  ...
  for (int tmp42 = 0; tmp42 < tmp41.len; tmp42++) {
    ...
    Type tmp43 = {0};
    bool tmp44 = map_get(/*cgen.v : 311*/ v->table->typesmap, builtin, &tmp43);
...
void Parser_fn_decl(Parser *p) {
  ...
  if (is_c) {
    ...
    f.is_c = 1;
             ...
             !string_contains(p->file_path, tos3("view.v"))) {
  ...
  if (Parser_first_pass(&/* ? */ *p) && receiver_typ.len == 0) {
    ...
    while (1) {
      ...
      Option_Fn tmp41 = Table_find_fn(&/* ? */ *p->table, f.name);
      if (!tmp41.ok) {
        string err = tmp41.error;

        break;
      }
      Fn existing_fn = *(Fn *)tmp41.data;
      ;
  ...
  if (!is_c && !is_live && !p->is_vh && !is_fn_header && !skip_main_in_test) {
    ...
    if (is_generic) {
      ...
      array_string tmp55 = gen_types;
      for (int tmp56 = 0; tmp56 < tmp55.len; tmp56++) {
        string gen_type = ((string *)tmp55.data)[tmp56];
  ...
  if (p->is_vh || Parser_first_pass(&/* ? */ *p) || is_live || is_fn_header ||
    ...
    if (is_live && Parser_first_pass(&/* ? */ *p) &&
      ...
      _PUSH(&p->cgen->so_fns,
            (/*typ = array_string   tmp_typ=string*/ fn_name_cgen), tmp58,
            string);
  ...
  if (string_eq(f.name, tos3("main__main")) ||
    ...
    if (p->pref->is_test &&
        !string_contains(p->scanner->file_path, tos3("/volt"))) {
...
void Parser_fn_args(Parser *p, Fn *f) {
  ...
  if (f->is_interface) {
    ...
    _PUSH(&f->args, (/*typ = array_Var   tmp_typ=Var*/ int_arg), tmp85, Var);
  };
  ...
  if (types_only) {
    ...
    while (p->tok != main__TokenKind_rpar) {
      ...
      _PUSH(&f->args, (/*typ = array_Var   tmp_typ=Var*/ v), tmp89, Var);
  ...
  while (p->tok != main__TokenKind_rpar) {
    ...
    array_string tmp97 = names;
    for (int tmp98 = 0; tmp98 < tmp97.len; tmp98++) {
      string name = ((string *)tmp97.data)[tmp98];
      ...
      _PUSH(&f->args, (/*typ = array_Var   tmp_typ=Var*/ v), tmp100, Var);
    };
...
Fn *Parser_fn_call_args(Parser *p, Fn *f) {
  ...
  if (p->v->pref->is_debug && string_eq(f->name, tos3("panic")) && !p->is_js) {
    ...
    string file_path = string_replace(p->file_path, tos3("\\"), tos3("\\\\"));
  ...
  for (int i = 0; i < tmp109.len; i++) {
    ...
    if (arg.is_mut) {
      ...
      Option_Var tmp114 = Parser_find_var(&/* ? */ *p, var_name);
      if (!tmp114.ok) {
    ...
    if (!is_interface) {
      ...
      array_Fn tmp129 = interface_type.methods;
      for (int tmp130 = 0; tmp130 < tmp129.len; tmp130++) {
        Fn method = ((Fn *)tmp129.data)[tmp130];
...
void Parser_fn_gen_caller_vargs(Parser *p, Fn *f) {
  ...
  array_VargAccess tmp140 = p->table->varg_access;
  for (int tmp141 = 0; tmp141 < tmp140.len; tmp141++) {
    VargAccess va = ((VargAccess *)tmp140.data)[tmp141];
...
string Fn_str_args(Fn *f, Table *table) {
  ...
  for (int i = 0; i < tmp149.len; i++) {
    ...
    if (Table_is_interface(&/* ? */ *table, arg.typ)) {
      ...
      array_Fn tmp151 = interface_type.methods;
      for (int tmp152 = 0; tmp152 < tmp151.len; tmp152++) {
        Fn method = ((Fn *)tmp151.data)[tmp152];
        ...
        if (method.args.len > 1) {
          ...
          array_Var tmp153 = array_right(method.args, 1);
          for (int tmp154 = 0; tmp154 < tmp153.len; tmp154++) {
            Var a = ((Var *)tmp153.data)[tmp154];
...
string Parser_find_misspelled_local_var(Parser *p, string name, f32 min_match) {
  ...
  array_Var tmp157 = p->local_vars;
  for (int tmp158 = 0; tmp158 < tmp157.len; tmp158++) {
    Var var = ((Var *)tmp157.data)[tmp158];
...
void V_generate_hot_reload_code(V *v) {
  ...
  if (v->pref->is_live) {
    ...
    if (string_eq(os__user_os(), tos3("windows"))) {
      ...
      vexe = string_replace(vexe, tos3("\\"), tos3("\\\\"));

      file = string_replace(file, tos3("\\"), tos3("\\\\"));
    };
...
void main__main() {
  ...
  if (args.len < 2 ||
      (args.len == 2 &&
       string_eq((*(string *)array__get(args, 1)), tos3("-"))) ||
...
void V_add_v_files_to_compile(V *v) {
  ...
  array_string tmp41 = builtin_files;
  for (int tmp42 = 0; tmp42 < tmp41.len; tmp42++) {
  ...
  array_string tmp45 = V_get_user_files(&/* ? */ *v);
  ...
  array_string tmp49 = imported_mods;
  for (int tmp50 = 0; tmp50 < tmp49.len; tmp50++) {
    string mod = ((string *)tmp49.data)[tmp50];
    ...
    Option_string tmp53 = V_find_module_path(&/* ? */ *v, mod);
    if (!tmp53.ok) {
    ...
    string mod_path = *(string *)tmp53.data;
    ;

    array_string vfiles = V_v_files_from_dir(&/* ? */ *v, mod_path);

    array_string tmp55 = vfiles;
    for (int tmp56 = 0; tmp56 < tmp55.len; tmp56++) {
      string file = ((string *)tmp55.data)[tmp56];

      _PUSH(&v->files, (/*typ = array_string   tmp_typ=string*/ file), tmp57,
            string);
    };
  };

  map_FileImportTable tmp58 = v->table->file_imports;
...
array_string V_get_user_files(V *v) {
  ...
  if (is_test_with_imports) {
    ...
    _PUSH(&user_files, (/*typ = array_string   tmp_typ=string*/ dir), tmp64,
          string);
  ...
  if (string_ends_with(dir, tos3(".v"))) {
    ...
    array_string tmp68 = files;
    for (int tmp69 = 0; tmp69 < tmp68.len; tmp69++) {
      string file = ((string *)tmp68.data)[tmp69];

      _PUSH(&user_files, (/*typ = array_string   tmp_typ=string*/ file), tmp70,
            string);
    };
  };
  ...
  while (1) {
    ...
    for (int l = 0; l < keys_tmp73.len; l++) {
      ...
      for (int l = 0; l < keys_tmp74.len; l++) {
        ...
        Option_string tmp75 = V_find_module_path(&/* ? */ *v, mod);
        if (!tmp75.ok) {
        ...
        string import_path = *(string *)tmp75.data;
        ;
...
string main__v_type_str(string typ_) {
  ...
  if (string_contains(typ, tos3("__"))) {
    ...
    return string_all_after(typ, tos3("__"));
  };
...
void V_generate_vh(V *v) {
  ...
  os__File_writeln(file, tos3("// Types2"));
  ...
  for (int l = 0; l < keys_tmp21.len; l++) {
    ...
    if (typ.cat == main__TypeCategory_struct_ ||
      ...
      array_Var tmp24 = typ.fields;
      for (int tmp25 = 0; tmp25 < tmp24.len; tmp25++) {
        Var field = ((Var *)tmp24.data)[tmp25];
      ...
      array_Var tmp28 = typ.fields;
      for (int tmp29 = 0; tmp29 < tmp28.len; tmp29++) {
        Var field = ((Var *)tmp28.data)[tmp29];
  ...
  for (int l = 0; l < keys_tmp32.len; l++) {
    ...
    if (string_eq(f.mod, v->mod) || string_eq(f.mod, tos3(""))) {
      ...
      _PUSH(&fns, (/*typ = array_Fn   tmp_typ=Fn*/ f), tmp33, Fn);

    } else {

      printf("skipping fn %.*s mod=%.*s\n", f.name.len, f.name.str, f.mod.len,
             f.mod.str);
    };
  };

  array_Fn tmp34 = fns;
  for (int _ = 0; _ < tmp34.len; _++) {
    Fn f = ((Fn *)tmp34.data)[_];
  ...
  array_Fn tmp35 = fns;
  for (int _ = 0; _ < tmp35.len; _++) {
    Fn f = ((Fn *)tmp35.data)[_];
  ...
  map_Type tmp36 = v->table->typesmap;
  ...
  for (int l = 0; l < keys_tmp36.len; l++) {
    ...
    array_Fn tmp37 = typ.methods;
    for (int tmp38 = 0; tmp38 < tmp37.len; tmp38++) {
      Fn method = ((Fn *)tmp37.data)[tmp38];
...
void Parser_struct_decl(Parser *p) {
  ...
  while (p->tok != main__TokenKind_rcbr) {
    ...
    _PUSH(&names, (/*typ = array_string   tmp_typ=string*/ field_name), tmp75,
          string);
...
void Parser_enum_decl(Parser *p, string _enum_name) {
  ...
  while (p->tok == main__TokenKind_name) {
    ...
    _PUSH(&fields, (/*typ = array_string   tmp_typ=string*/ field), tmp86,
          string);
...
void Parser_var_decl(Parser *p) {
  ...
  if (string_ne((*(string *)array__get(var_names, 0)), tos3("_")) &&
  ...
  if (is_decl_assign && var_names.len == 1 &&
      string_eq((*(string *)array__get(var_names, 0)), tos3("_"))) {
  ...
  array_string tmp171 = var_names;
  for (int i = 0; i < tmp171.len; i++) {
    string var_name = ((string *)tmp171.data)[i];
    ...
    if (var_names.len > 1) {
      ...
      if (known_var) {
        ...
        Option_Var tmp183 = Parser_find_var(&/* ? */ *p, var_name);
        if (!tmp183.ok) {
...
string Parser_name_expr(Parser *p) {
  ...
  while (1) {
    ...
    Option_Var tmp209 = Parser_find_var_check_new_var(&/* ? */ *p, name);
    if (!tmp209.ok) {
      string err = tmp209.error;

      break;
    }
    Var v = *(Var *)tmp209.data;
    ;
  ...
  while (1) {
    ...
    Option_Var tmp212 = Parser_find_var_check_new_var(&/* ? */ *p, name);
    if (!tmp212.ok) {
      string err = tmp212.error;

      break;
    }
    Var v = *(Var *)tmp212.data;
    ;
  ...
  if (is_c) {
    ...
    Option_Fn tmp218 = Table_find_fn(&/* ? */ *p->table, name);
    if (!tmp218.ok) {
  ...
  while (1) {
    ...
    Option_Var tmp219 = Table_find_const(&/* ? */ *p->table, name);
    if (!tmp219.ok) {
      string err = tmp219.error;

      break;
    }
    Var c = *(Var *)tmp219.data;
    ;
  ...
  Option_Fn tmp221 = Table_find_fn(&/* ? */ *p->table, name);
  if (!tmp221.ok) {
...
string Parser_dot(Parser *p, string str_typ_, int method_ph) {
  ...
  if (has_field) {
    ...
    Option_Var tmp239 = Table_find_field(
  ...
  Option_Fn tmp244 = Table_find_method(
      &/* ? */ *p->table, &/*112 EXP:"Type*" GOT:"Type" */ typ, field_name);
  if (!tmp244.ok) {
...
string Parser_assoc(Parser *p) {
  ...
  Option_Var tmp306 = Parser_find_var(&/* ? */ *p, name);
  if (!tmp306.ok) {
  ...
  while (p->tok != main__TokenKind_rcbr) {
    ...
    _PUSH(&fields, (/*typ = array_string   tmp_typ=string*/ field), tmp309,
          string);
  ...
  for (int tmp312 = 0; tmp312 < tmp311.len; tmp312++) {
    Var ffield = ((Var *)tmp311.data)[tmp312];
...
void Parser_string_expr(Parser *p) {
  ...
  if (Parser_peek(&/* ? */ *p) != main__TokenKind_dollar || is_raw) {
    ...
    string f = (is_raw) ? (string_replace(str, tos3("\\"), tos3("\\\\")))
...
string Parser_array_init(Parser *p) {
  ...
  if (p->tok == main__TokenKind_name && !p->inside_const) {
    ...
    if (Table_known_const(&/* ? */ *p->table, const_name)) {
      ...
      Option_Var tmp344 = Table_find_const(&/* ? */ *p->table, const_name);
      if (!tmp344.ok) {
        string err = tmp344.error;

        v_exit(1);
      }
      Var c = *(Var *)tmp344.data;
      ;
...
string Parser_struct_init(Parser *p, string typ) {
  ...
  if (peek == main__TokenKind_colon || p->tok == main__TokenKind_rcbr) {
    ...
    while (p->tok != main__TokenKind_rcbr) {
      ...
      Option_Var tmp361 = Type_find_field(&/* ? */ t, field);
      if (!tmp361.ok) {
    ...
    array_Var tmp363 = t.fields;
    for (int i = 0; i < tmp363.len; i++) {
      Var field = ((Var *)tmp363.data)[i];
...
void Parser_assert_statement(Parser *p) {
  ...
  string filename = string_replace(p->file_path, tos3("\\"), tos3("\\\\"));
...
void Parser_return_st(Parser *p) {
  ...
  if (fn_returns) {
    ...
    if (p->tok == main__TokenKind_rcbr) {
      ...
      _PUSH(&types, (/*typ = array_string   tmp_typ=string*/ expr_type), tmp437,
            string);
      ...
      if (types.len > 1) {
        ...
        array_string tmp442 = mr_values;
        for (int ret_val_idx = 0; ret_val_idx < tmp442.len; ret_val_idx++) {
          string ret_val = ((string *)tmp442.data)[ret_val_idx];
      ...
      if (string_ends_with(p->cur_fn.typ, expr_type) && !is_none &&
        ...
        array_string tmp448 = p->cur_fn.defer_text;
        for (int tmp449 = 0; tmp449 < tmp448.len; tmp449++) {
          string text = ((string *)tmp448.data)[tmp449];
...
void Parser_go_statement(Parser *p) {
  ...
  if (Parser_peek(&/* ? */ *p) == main__TokenKind_dot) {
    ...
    Option_Var tmp453 = Parser_find_var(&/* ? */ *p, var_name);
    if (!tmp453.ok) {
      string err = tmp453.error;

      return;
    }
    Var v = *(Var *)tmp453.data;
    ;
    ...
    Option_Fn tmp455 = Table_find_method(
    ...
    Option_Fn tmp457 = Table_find_fn(&/* ? */ *p->table,
...
string Parser_js_decode(Parser *p) {
  ...
  if (string_eq(op, tos3("decode"))) {
    ...
    array_Var tmp466 = T.fields;
    for (int tmp467 = 0; tmp467 < tmp466.len; tmp467++) {
      Var field = ((Var *)tmp466.data)[tmp467];
...
void Parser_check_unused_imports(Parser *p) {
  ...
  map_string tmp479 = p->import_table.imports;
  array_string keys_tmp479 = map_keys(&tmp479);
  for (int l = 0; l < keys_tmp479.len; l++) {
    string alias = ((string *)keys_tmp479.data)[l];
    string mod = {0};
    map_get(tmp479, alias, &mod);
...
string Parser_select_query(Parser *p, int fn_ph) {
  ...
  if (string_ne((*(Var *)array__get(fields, 0)).name, tos3("id"))) {
...
ScanRes Scanner_scan(Scanner *s) {
  ...
  if (c == '+') { /* case */
    ...
    if (string_eq(name, tos3("FILE"))) {
      ...
      return main__scan_res(
          main__TokenKind_str,
          string_replace(os__realpath(s->file_path), tos3("\\"), tos3("\\\\")));
    };
...
void init_consts() {
  ...
  main__CommonCHeaders = tos3(
      ...
      "0\n#endif\n\n//============================== HELPER C MACROS "
      "=============================*/\n#define _PUSH(arr, val, tmp, tmp_typ) "
      "{tmp_typ tmp = (val); array__push(arr, &tmp);}\n#define _PUSH_MANY(arr, "
      "val, tmp, tmp_typ) {tmp_typ tmp = (val); array__push_many(arr, "
