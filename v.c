...
array_string compiler__get_cmdline_multiple_values(array_string args,
                                                   string optname);
_V_MulRet_array_string_V_array_string
compiler__parse_defines(array_string defines);
...
u32 strconv__TEN;
array_u64 strconv__pos_exp;
array_u64 strconv__neg_exp;
...
string string_replace_each(string s, array_string vals) {
  ...
  for (int rep_i = 0; rep_i < vals.len; rep_i += 2) {
    ...
    while (1) {
      ...
      _PUSH(
          &idxs,
          (/*typ = array_RepIndex   tmp_typ=RepIndex*/ (RepIndex){idx, rep_i}),
          tmp11, RepIndex);
      idx++;
...
array_string string_split_nth(string s, string delim, int nth) {
  ...
  if (delim.len == 0) {
    ...
    for (int tmp21 = 0; tmp21 < tmp20.len; tmp21++) {
      ...
      _PUSH(&res, (/*typ = array_string   tmp_typ=string*/ byte_str(ch)), tmp23,
            string);
      i++;
    };
    return res;
  ...
  if (string_ends_with(s, delim) && (nth < 1 || res.len < nth)) {
    _PUSH(&res, (/*typ = array_string   tmp_typ=string*/ tos3("")), tmp25,
          string);
  };
  return res;
}
array_string string_split_into_lines(string s) {
  ...
  for (int i = 0; i < s.len; i++) {
    ...
    if (((int)(s.str[i] /*rbyte 0*/)) == 10 || last) {
      ...
      _PUSH(&res, (/*typ = array_string   tmp_typ=string*/ line), tmp26,
            string);
      start = i + 1;
    };
  };
...
string string_title(string s) {
  ...
  array_string tit = new_array_from_c_array(
      0, 0, sizeof(string), EMPTY_ARRAY_OF_ELEMS(string, 0){TCCSKIP(0)});
  array_string tmp39 = words;
  for (int tmp40 = 0; tmp40 < tmp39.len; tmp40++) {
    string word = ((string *)tmp39.data)[tmp40];
...
bool array_string_contains(array_string ar, string val) {
  array_string tmp44 = ar;
  for (int tmp45 = 0; tmp45 < tmp44.len; tmp45++) {
    string s = ((string *)tmp44.data)[tmp45];
...
string string_repeat(string s, int count) {
  ...
  int tmp61 = 0;
  ;
  for (int tmp62 = tmp61; tmp62 < count; tmp62++) {
    int i = tmp62;

    int tmp63 = 0;
    ;
    for (int tmp64 = tmp63; tmp64 < s.len; tmp64++) {
      int j = tmp64;
...
f64 strconv__atof_quick(string s) {
  ...
  if (string_at(s, i) == 'i' && i + 2 < s.len && string_at(s, i + 1) == 'n' &&
      string_at(s, i + 2) == 'f') {
    u64 *d = ((u64 *)(&f));
    if (f64_gt(sign, 0.0)) {
      *d = strconv__DOUBLE_PLUS_INFINITY;
    } else {
      *d = strconv__DOUBLE_MINUS_INFINITY;
    };
    return f;
  };
  while (i < s.len && string_at(s, i) == '0') {
    ...
    if (i >= s.len) {
      u64 *d = ((u64 *)(&f));
      if (f64_gt(sign, 0.0)) {
        *d = strconv__DOUBLE_PLUS_ZERO;
      } else {
        *d = strconv__DOUBLE_MINUS_ZERO;
      };
      return f;
    };
  };
  ...
  if (i < s.len && string_at(s, i) == '.') {
    ...
    while (i < s.len && (string_at(s, i) >= '0' && string_at(s, i) <= '9')) {
      ...
      f += ((f64)(string_at(s, i) - '0')) * frac_mul;
      frac_mul *= ((f64)(0.1));
      i++;
    };
  };
  if (i < s.len && (string_at(s, i) == 'e' || string_at(s, i) == 'E')) {
    ...
    if (exp_sign == 1) {
      ...
      f64 tmp_mul = ((f64)(0.0));
      u64 *ptr_d = ((u64 *)(&tmp_mul));
      *ptr_d = (*(u64 *)array_get(strconv__pos_exp, exp));
      f = f * tmp_mul;
    } else {
      if (exp > strconv__neg_exp.len) {
        u64 *d = ((u64 *)(&f));
        if ((f64_gt(sign, 0))) {
          *d = strconv__DOUBLE_PLUS_ZERO;
        } else {
      ...
      f64 tmp_mul = ((f64)(0.0));
      u64 *ptr_d = ((u64 *)(&tmp_mul));
      *ptr_d = (*(u64 *)array_get(strconv__neg_exp, exp));
      f = f * tmp_mul;
    };
  };
  f = f * sign;
    ...
    string adjasted_path =
        ((os__is_dir(dest_path))
             ? (filepath__join(
                   dest_path,
                   &(varg_string){.len = 1,
                                  .args = {filepath__filename(source_path)}}))
             : (dest_path));
...
Option_array_string os__read_lines(string path) {
  ...
  if (isnil(fp)) {
    return v_error(
        _STR("read_lines() failed to open file \"%.*s\"", path.len, path.str));
  };
  int buf_index = 0;
...
int os__vpclose(void *f) {
  ...
  return _pclose(f);
#else
  _V_MulRet_int_V_bool _V_mret_1985_ret__ =
      os__posix_wait4_to_exit_status(pclose(f));
  int ret = _V_mret_1985_ret__.var_0;
  return ret;
#endif
  ;
}
...
#ifndef _WIN32
  _V_MulRet_int_V_bool _V_mret_2104_pret_is_signaled =
      os__posix_wait4_to_exit_status(ret);
  int pret = _V_mret_2104_pret_is_signaled.var_0;
  bool is_signaled = _V_mret_2104_pret_is_signaled.var_1;
  if (is_signaled) {
...
string os__sigint_to_signal_name(int si) {
  ...
  if ((tmp35 == 30) || (tmp35 == 10) || (tmp35 == 16)) {
    return tos3("SIGUSR1");
  } else if ((tmp35 == 31) || (tmp35 == 12) || (tmp35 == 17)) {
    return tos3("SIGUSR2");
  } else if ((tmp35 == 20) || (tmp35 == 17) || (tmp35 == 18)) {
    return tos3("SIGCHLD");
  } else if ((tmp35 == 19) || (tmp35 == 18) || (tmp35 == 25)) {
    return tos3("SIGCONT");
  } else if ((tmp35 == 17) || (tmp35 == 19) || (tmp35 == 23)) {
    return tos3("SIGSTOP");
  } else if ((tmp35 == 18) || (tmp35 == 20) || (tmp35 == 24)) {
    return tos3("SIGTSTP");
  } else if ((tmp35 == 21) || (tmp35 == 21) || (tmp35 == 26)) {
    return tos3("SIGTTIN");
  } else if ((tmp35 == 22) || (tmp35 == 22) || (tmp35 == 27)) {
    return tos3("SIGTTOU");
  } else if (tmp35 == 5) {
    return tos3("SIGTRAP");
  } else if (tmp35 == 7) {
...
Option_os__Result os__exec(string cmd) {
  ...
  os__Result tmp8 = OPTION_CAST(os__Result)(
...
string filepath__ext(string path) {
  Option_int tmp1 = string_last_index(path, tos3("."));
  int pos;
  if (!tmp1.ok) {
    string err = tmp1.error;
    int errcode = tmp1.ecode;
...
string filepath__join(string base, varg_string *dirs) {
  ...
  _PUSH(&result,
        (/*typ = array_string   tmp_typ=string*/ string_trim_right(
            base, tos3("\\/"))),
        tmp12, string);
  for (int tmp14 = 0; tmp14 < dirs->len; tmp14++) {
    string d = ((string *)dirs->args)[tmp14];

    _PUSH(&result, (/*typ = array_string   tmp_typ=string*/ d), tmp15, string);
  };
...
string filepath__dir(string path) {
  ...
  Option_int tmp16 = string_last_index(path, os__path_separator);
  int pos;
  if (!tmp16.ok) {
...
string filepath__basedir(string path) {
  Option_int tmp19 = string_last_index(path, os__path_separator);
  int pos;
  if (!tmp19.ok) {
...
string time__Time_get_fmt_time_str(time__Time t, time__FormatTime fmt_time) {
  ...
  return (
      (tmp31 == time__time__FormatTime_hhmm12)
          ? (_STR("%d:%02d %.*s", hour, t.minute, tp.len, tp.str))
          : ((tmp31 == time__time__FormatTime_hhmm24)
                 ? (_STR("%02d:%02d", t.hour, t.minute))
                 : ((tmp31 == time__time__FormatTime_hhmmss12)
                        ? (_STR("%d:%02d:%02d %.*s", hour, t.minute, t.second,
                                tp.len, tp.str))
                        : ((tmp31 == time__time__FormatTime_hhmmss24)
...
string time__Time_get_fmt_date_str(time__Time t,
  ...
  return string_replace(
      ((tmp34 == time__time__FormatDate_ddmmyy)
           ? (_STR("%02d|%02d|%.*s", t.day, t.month, year.len, year.str))
           : ((tmp34 == time__time__FormatDate_ddmmyyyy)
                  ? (_STR("%02d|%02d|%d", t.day, t.month, t.year))
                  : ((tmp34 == time__time__FormatDate_mmddyy)
                         ? (_STR("%02d|%02d|%.*s", t.month, t.day, year.len,
                                 year.str))
                         : ((tmp34 == time__time__FormatDate_mmddyyyy)
                                ? (_STR("%02d|%02d|%d", t.month, t.day, t.year))
                                : ((tmp34 == time__time__FormatDate_mmmd)
                                                     ...
                                                     : ((tmp34 ==
                                                         time__time__FormatDate_yyyymmdd)
                                                            ...
                                                            : (_STR(
                                                                  ...
                                                                  fmt_date)))))))))),
      tos3("|"),
      ((tmp35 == time__time__FormatDelimiter_dot)
           ? (tos3("."))
           : ((tmp35 == time__time__FormatDelimiter_hyphen)
                  ? (tos3("-"))
                  : ((tmp35 == time__time__FormatDelimiter_slash)
                         ? (tos3("/"))
                         : ((tmp35 == time__time__FormatDelimiter_space)
                                ? (tos3(" "))
...
void compiler__V_cc(compiler__V *v) {
  ...
  if (v->pref->fast) {
    ...
    if (string_eq(v->pref->ccompiler, tos3("cc")) && os__exists(tcc_path)) {
      ...
      _PUSH(&a, (/*typ = array_string   tmp_typ=string*/ tos3("-m64")), tmp3,
            string);
    };
#endif
    ;
  ...
  _PUSH(&a,
        ...
        tmp28, string);
  _PUSH(&a, (/*typ = array_string   tmp_typ=string*/ libs), tmp29, string);
  if (!v->pref->is_bare &&
    ...
    if (v->os == compiler__compiler__OS_linux) {
      _PUSH(&a, (/*typ = array_string   tmp_typ=string*/ tos3(" -ldl ")), tmp31,
            string);
    };
  ...
  if (!v->pref->is_bare && v->os == compiler__compiler__OS_js &&
    ...
    _PUSH(&a, (/*typ = array_string   tmp_typ=string*/ tos3("-lm")), tmp33,
          string);
  };
...
string compiler__find_c_compiler_thirdparty_options() {
  ...
  array_string cflags =
      compiler__get_cmdline_multiple_values(fullargs, tos3("-cflags"));
#ifndef _WIN32
  _PUSH(&cflags, (/*typ = array_string   tmp_typ=string*/ tos3("-fPIC")), tmp39,
        string);
#endif
  ;
...
array_string compiler__get_cmdline_multiple_values(array_string args,
  ...
  array_string flags = new_array_from_c_array(
      0, 0, sizeof(string), EMPTY_ARRAY_OF_ELEMS(string, 0){TCCSKIP(0)});
  array_string tmp41 = args;
  for (int ci = 0; ci < tmp41.len; ci++) {
    string cv = ((string *)tmp41.data)[ci];
...
_V_MulRet_array_string_V_array_string
compiler__parse_defines(array_string defines) {
  array_string compile_defines = new_array_from_c_array(
      0, 0, sizeof(string), EMPTY_ARRAY_OF_ELEMS(string, 0){TCCSKIP(0)});
  array_string compile_defines_all = new_array_from_c_array(
      0, 0, sizeof(string), EMPTY_ARRAY_OF_ELEMS(string, 0){TCCSKIP(0)});
  array_string tmp45 = defines;
  for (int tmp46 = 0; tmp46 < tmp45.len; tmp46++) {
    string dfn = ((string *)tmp45.data)[tmp46];
  ...
  return (_V_MulRet_array_string_V_array_string){.var_0 = compile_defines,
                                                 .var_1 = compile_defines_all};
}
string compiler__missing_compiler_info() {
#ifdef _WIN32
  return tos3(
...
array_compiler__CFlag compiler__V_get_os_cflags(compiler__V *v) {
  ...
  if (v->compile_defines.len > 0) {
    _PUSH_MANY(&ctimedefines,
               (/*typ = array_string   tmp_typ=string*/ v->compile_defines),
               tmp1, array_string);
  };
...
Option_bool compiler__Table_parse_cflag(compiler__Table *table, string cflag,
  ...
  array_string allowed_os_overrides = new_array_from_c_array(
      4, 4, sizeof(string),
      EMPTY_ARRAY_OF_ELEMS(string, 4){tos3("linux"), tos3("darwin"),
...
void compiler__build_thirdparty_obj_file(string path,
  ...
  string parent = filepath__dir(obj_path);
...
void compiler__Parser_comp_time(compiler__Parser *p) {
  ...
  if (p->tok == compiler__compiler__TokenKind_key_if) {
    ...
    if ((_IN(string, (name), compiler__supported_platforms))) {
      ...
      if (((!not&&os != p->os) || (not&&os == p->os)) && !p->scanner->is_fmt &&
    ...
    _V_MulRet_string_V_string _V_mret_697___val = compiler__Parser_tmp_expr(p);
    string val = _V_mret_697___val.var_1;
    compiler__Parser_check(p, compiler__compiler__TokenKind_rcbr);
    ...
    if (!os__exists(path)) {
      path = string_add(
          string_add(filepath__dir(p->scanner->file_path), tos3("/")), path);
      if (!os__exists(path)) {
...
string compiler__Parser_gen_array_map(compiler__Parser *p, string str_typ,
  ...
  string a = p->expr_var.name;
  _V_MulRet_string_V_string _V_mret_2217_map_type_expr =
      compiler__Parser_tmp_expr(p);
  string map_type = _V_mret_2217_map_type_expr.var_0;
  string expr = _V_mret_2217_map_type_expr.var_1;
  compiler__CGen_set_placeholder(
      p->cgen, method_ph,
...
string compiler__Parser_name_expr(compiler__Parser *p) {
  ...
  if ((_IN(string, (name), map_keys(&/* ? */ p->generic_dispatch.inst)))) {
    string tmp10 = tos3("");
    bool tmp11 =
        map_get(/*expression.v : 218*/ p->generic_dispatch.inst, name, &tmp10);
  ...
  if (compiler__Table_known_type(&/* ? */ *p->table, name)) {
    if (compiler__Parser_peek(&/* ? */ *p) ==
      ...
      if (byte_is_capital(string_at(val, 0))) {
        ...
        compiler__Parser_check(p, compiler__compiler__TokenKind_lpar);
        int idx = 0;
        array_string tmp16 = enum_type.enum_vals;
        for (int i = 0; i < tmp16.len; i++) {
          string val_ = ((string *)tmp16.data)[i];
        ...
        bool tmp18 = map_get(/*expression.v : 363*/ p->table->tuple_variants,
...
string compiler__Parser_term(compiler__Parser *p) {
  ...
  while (p->tok == compiler__compiler__TokenKind_mul ||
    ...
    if (!compiler__is_primitive_type(expr_type) && string_eq(expr_type, typ)) {
      ...
      compiler__TokenKind tmp31 = tok;

      if (tmp31 == compiler__compiler__TokenKind_mul) {
        compiler__Parser_handle_operator(p, tos3("*"), typ, tos3("op_mul"), ph,
                                         &/*114*/ T);
      } else if (tmp31 == compiler__compiler__TokenKind_div) {
        compiler__Parser_handle_operator(p, tos3("/"), typ, tos3("op_div"), ph,
                                         &/*114*/ T);
      } else if (tmp31 == compiler__compiler__TokenKind_mod) {
...
string compiler__Parser_unary(compiler__Parser *p) {
  ...
  if (tmp32 == compiler__compiler__TokenKind_not) {
    compiler__Parser_gen(p, tos3("!"));
    compiler__Parser_check(p, compiler__compiler__TokenKind_not);
  ...
  } else if (tmp32 == compiler__compiler__TokenKind_bit_not) {
    compiler__Parser_gen(p, tos3("~"));
...
string compiler__Parser_factor(compiler__Parser *p) {
  string typ = tos3("");
  compiler__TokenKind tok = p->tok;
  compiler__TokenKind tmp33 = tok;

  if (tmp33 == compiler__compiler__TokenKind_key_none) {
    ...
    return p->expected_type;
  } else if (tmp33 == compiler__compiler__TokenKind_number) {
    typ = tos3("int");
  ...
  } else if (tmp33 == compiler__compiler__TokenKind_minus) {
    compiler__Parser_gen(p, tos3("-"));
    compiler__Parser_next(p);
    return compiler__Parser_factor(p);
  } else if (tmp33 == compiler__compiler__TokenKind_key_sizeof) {
    ...
    return tos3("int");
  } else if ((tmp33 == compiler__compiler__TokenKind_amp) ||
             (tmp33 == compiler__compiler__TokenKind_dot) ||
             (tmp33 == compiler__compiler__TokenKind_mul)) {
    return compiler__Parser_name_expr(p);
  } else if (tmp33 == compiler__compiler__TokenKind_name) {
    ...
    return typ;
  } else if (tmp33 == compiler__compiler__TokenKind_lpar) {
    compiler__Parser_gen(p, tos3("("));
    compiler__Parser_check(p, compiler__compiler__TokenKind_lpar);
    typ = compiler__Parser_bool_expression(p);
    if (!p->ptr_cast) {
    ...
    compiler__Parser_gen(p, tos3(")"));
    return typ;
  } else if (tmp33 == compiler__compiler__TokenKind_chartoken) {
    compiler__Parser_char_expr(p);
    typ = tos3("byte");
    return typ;
  } else if (tmp33 == compiler__compiler__TokenKind_str) {
    compiler__Parser_string_expr(p);
    typ = tos3("string");
    return typ;
  } else if (tmp33 == compiler__compiler__TokenKind_key_false) {
    typ = tos3("bool");
    compiler__Parser_gen(p, tos3("0"));
  } else if (tmp33 == compiler__compiler__TokenKind_key_true) {
    typ = tos3("bool");
    compiler__Parser_gen(p, tos3("1"));
  } else if (tmp33 == compiler__compiler__TokenKind_lsbr) {
    return compiler__Parser_array_init(p);
  } else if (tmp33 == compiler__compiler__TokenKind_lcbr) {
    if (compiler__Parser_peek(&/* ? */ *p) ==
    ...
    return typ;
  } else if (tmp33 == compiler__compiler__TokenKind_key_match) {
...
void compiler__Parser_fn_call(compiler__Parser *p, compiler__Fn *f,
  ...
  bool is_comptime_define =
      ...
      !((_IN(string, (f->comptime_define), p->v->compile_defines)));
  ...
  if (f->is_method) {
    ...
    if (string_ends_with(receiver.typ, tos3("er"))) {
      ...
      if (t.cat == compiler__compiler__TypeCategory_interface_) {
        int idx = 0;
        array_compiler__Fn tmp64 = t.methods;
        for (int i = 0; i < tmp64.len; i++) {
          compiler__Fn method = ((compiler__Fn *)tmp64.data)[i];
...
void compiler__Parser_fn_args(compiler__Parser *p, compiler__Fn *f) {
  ...
  while (p->tok != compiler__compiler__TokenKind_rpar) {
    ...
    array_string tmp70 = names;
    for (int tmp71 = 0; tmp71 < tmp70.len; tmp71++) {
      string name = ((string *)tmp70.data)[tmp71];
...
void compiler__Parser_fn_call_args(compiler__Parser *p, compiler__Fn *f,
  ...
  array_compiler__Var tmp74 = f->args;
  for (int i = 0; i < tmp74.len; i++) {
    compiler__Var arg = ((compiler__Var *)tmp74.data)[i];
    ...
    if (i == 0 &&
      ...
      if (string_ne(fmt, tos3("")) && string_ne(typ, tos3("bool"))) {
        string nl = ((string_eq(f->name, tos3("println"))) ? (tos3("\\n"))
                                                           : (tos3("")));
        compiler__CGen_resetln(
    ...
    if (type_mismatch && f->is_generic) {
      _PUSH(&saved_args, (/*typ = array_string   tmp_typ=string*/ got), tmp76,
            string);
  ...
  _V_MulRet_string_V_array_string _V_mret_6212_varg_type_varg_values =
      compiler__Parser_fn_call_vargs(p, *f);
  string varg_type = _V_mret_6212_varg_type_varg_values.var_0;
  array_string varg_values = _V_mret_6212_varg_type_varg_values.var_1;
  if (f->is_variadic) {
...
compiler__TypeInst compiler__Parser_extract_type_inst(compiler__Parser *p,
  ...
  if (string_ne(f->typ, tos3("void"))) {
    _PUSH(&args, (/*typ = array_string   tmp_typ=string*/ f->typ), tmp81,
          string);
  };
  array_string tmp82 = args;
  for (int tmp83 = 0; tmp83 < tmp82.len; tmp83++) {
    string e = ((string *)tmp82.data)[tmp83];
    ...
    if (string_starts_with(ti, tos3("fn ("))) {
      array_string fn_args = string_split(
          ...
          tos3(","));
      bool found = 0;
      array_string tmp88 = fn_args;
      for (int tmp89 = 0; tmp89 < tmp88.len; tmp89++) {
        string fa_ = ((string *)tmp88.data)[tmp89];
      ...
      if (string_ne(tmp96, ti)) {
        ...
        compiler__Parser_error(
            p, _STR("type parameter `%.*s` has type %.*s, not `%.*s`", tp.len,
                    tp.str, tmp98.len, tmp98.str, ti.len, ti.str));
      };
      continue;
    };
  ...
  if (string_eq(tmp100, tos3("")) && (_IN(string, (f->typ), f->type_pars))) {
    map_set(&r.inst, f->typ, &(string[]){tos3("_ANYTYPE_")});
  };
  array_string tmp102 = f->type_pars;
...
string compiler__replace_generic_type(string gen_type, compiler__TypeInst *ti) {
  ...
  if ((_IN_MAP((typ), ti->inst))) {
    ...
    typ = string_replace(gen_type, typ, tmp108);
    return typ;
  };
  typ = gen_type;
  if (string_starts_with(typ, tos3("fn ("))) {
    ...
    array_string args_r = new_array_from_c_array(
        0, 0, sizeof(string), EMPTY_ARRAY_OF_ELEMS(string, 0){TCCSKIP(0)});
    array_string tmp112 = args;
    for (int tmp113 = 0; tmp113 < tmp112.len; tmp113++) {
      string arg = ((string *)tmp112.data)[tmp113];
...
void compiler__replace_generic_type_params(compiler__Fn *f,
  ...
  array_compiler__Var args = new_array_from_c_array(
      0, 0, sizeof(compiler__Var),
      EMPTY_ARRAY_OF_ELEMS(compiler__Var, 0){TCCSKIP(0)});
  array_compiler__Var tmp115 = f->args;
  ...
  if (string_ends_with(f->typ, tos3("_T"))) {
    ...
    f->typ = string_add(string_add(f->typ, tos3("_")), tmp121);
  };
}
string compiler__Parser_register_vargs_stuct(compiler__Parser *p, string typ,
                                             int len) {
...
compiler__Parser_fn_call_vargs(compiler__Parser *p, compiler__Fn f) {
  ...
  while (p->tok != compiler__compiler__TokenKind_rpar) {
    ...
    _V_MulRet_string_V_string _V_mret_7159_varg_type_varg_value =
        compiler__Parser_tmp_expr(p);
    string varg_type = _V_mret_7159_varg_type_varg_value.var_0;
    string varg_value = _V_mret_7159_varg_type_varg_value.var_1;
    ...
    if (!f.is_generic) {
      ...
      if (types.len > 0) {
        array_string tmp130 = types;
        for (int tmp131 = 0; tmp131 < tmp130.len; tmp131++) {
          string t = ((string *)tmp130.data)[tmp131];
                         ...
                         !string_ends_with(varg_type, tos3("*")))
                            ...
                            : (((!string_ends_with(last_arg.typ, tos3("*")) &&
                                 string_ends_with(varg_type, tos3("*")))
                                    ? (tos3("*"))
                                    : (tos3("")))));
    _PUSH(&types, (/*typ = array_string   tmp_typ=string*/ varg_type), tmp132,
          string);
    _PUSH(&values,
          (/*typ = array_string   tmp_typ=string*/ _STR(
  ...
  array_compiler__VargAccess tmp134 = p->table->varg_access;
  for (int tmp135 = 0; tmp135 < tmp134.len; tmp135++) {
    compiler__VargAccess va = ((compiler__VargAccess *)tmp134.data)[tmp135];
...
void compiler__rename_generic_fn_instance(compiler__Fn *f,
  ...
  array_string tmp142 = map_keys(&/* ? */ ti->inst);
  for (int tmp143 = 0; tmp143 < tmp142.len; tmp143++) {
    ...
    f->name = string_add(string_add(f->name, tos3("_")),
                         compiler__type_to_safe_str(tmp144));
  };
}
void compiler__Parser_dispatch_generic_fn_instance(compiler__Parser *p,
                                                   compiler__Fn *f,
                                                   compiler__TypeInst *ti) {
  bool new_inst = 1;
  array_compiler__TypeInst tmp146 = f->type_inst;
  for (int tmp147 = 0; tmp147 < tmp146.len; tmp147++) {
    compiler__TypeInst e = ((compiler__TypeInst *)tmp146.data)[tmp147];
  ...
  _PUSH_MANY(&p->cgen->lines_extra,
             (/*typ = array_string   tmp_typ=string*/ p->cgen->lines), tmp152,
             array_string);
...
string compiler__Fn_typ_str(compiler__Fn *f) {
  ...
  array_compiler__Var tmp154 = f->args;
  for (int i = 0; i < tmp154.len; i++) {
    compiler__Var arg = ((compiler__Var *)tmp154.data)[i];
...
string compiler__Parser_find_misspelled_local_var(compiler__Parser *p,
  ...
  array_compiler__Var tmp156 = p->local_vars;
  for (int tmp157 = 0; tmp157 < tmp156.len; tmp157++) {
    compiler__Var var = ((compiler__Var *)tmp156.data)[tmp157];
...
bool array_compiler__Fn_contains(array_compiler__Fn fns, compiler__Fn f) {
  array_compiler__Fn tmp158 = fns;
  for (int tmp159 = 0; tmp159 < tmp158.len; tmp159++) {
    compiler__Fn ff = ((compiler__Fn *)tmp158.data)[tmp159];
...
string compiler__Fn_str_for_error(compiler__Fn *f) {
  ...
  for (int i = 0; i < tmp160.len; i++) {
    compiler__Var a = ((compiler__Var *)tmp160.data)[i];
...
void compiler__Parser_for_st(compiler__Parser *p) {
  ...
  if (p->tok == compiler__compiler__TokenKind_lcbr) {
    ...
    ;
    string tmp = compiler__Parser_get_tmp(p);
    _V_MulRet_string_V_string _V_mret_643_typ_expr =
        compiler__Parser_tmp_expr(p);
    string typ = _V_mret_643_typ_expr.var_0;
    string expr = _V_mret_643_typ_expr.var_1;
    ...
    if (is_range) {
      ...
      _V_MulRet_string_V_string _V_mret_707_range_typ_range_expr =
          compiler__Parser_tmp_expr(p);
      string range_typ = _V_mret_707_range_typ_range_expr.var_0;
      string range_expr = _V_mret_707_range_typ_range_expr.var_1;
...
void compiler__Parser_gen_blank_identifier_assign(compiler__Parser *p) {
  ...
  _V_MulRet_bool_V_string _V_mret_342_is_fn_call_next_expr =
...
void compiler__V_generate_hot_reload_code(compiler__V *v) {
  ...
  if (v->pref->is_live) {
    ...
    string file_base =
        string_replace(filepath__filename(file), tos3(".v"), tos3(""));
    string so_name = string_add(file_base, tos3(".so"));
...
int compiler__V_add_parser(compiler__V *v, compiler__Parser parser) {
  ...
  string file_path =
      ((filepath__is_abs(parser.file_path)) ? (parser.file_path)
...
void compiler__V_compile(compiler__V *v) {
  ...
  if (v->compile_defines_all.len > 0) {
    compiler__CGen_genln(cgen, tos3(""));
    compiler__CGen_genln(
        cgen, string_add(tos3("// All custom defines      : "),
                         array_string_join(v->compile_defines_all, tos3(","))));
    compiler__CGen_genln(
        cgen, string_add(tos3("// Turned ON custom defines: "),
                         array_string_join(v->compile_defines, tos3(","))));
    array_string tmp10 = v->compile_defines;
          ...
          &/* ? */ *v, filepath__join(v->pref->vlib_path,
                                      &(varg_string){.len = 2,
                                                     .args = {tos3("builtin"),
                                                              tos3("bare")}}))),
      tmp16, array_string);
  _PUSH(&v->files, (/*typ = array_string   tmp_typ=string*/ v->dir), tmp17,
        string);
...
void compiler__V_generate_init(compiler__V *v) {
  ...
  if (v->pref->build_mode == compiler__compiler__BuildMode_default_mode) {
    ...
    for (int tmp23 = 0; tmp23 < tmp22.len; tmp23++) {
      string mod = ((string *)tmp22.data)[tmp23];
...
void compiler__V_generate_main(compiler__V *v) {
  ...
  if (v->pref->build_mode != compiler__compiler__BuildMode_build_module) {
    if (!compiler__Table_main_exists(&/* ? */ *v->table) && !v->pref->is_test) {
      ...
      for (int tmp26 = 0; tmp26 < tmp25.len; tmp26++) {
        string tfname = ((string *)tmp25.data)[tmp26];
...
void compiler__V_run_compiled_executable_and_exit(compiler__V v) {
  ...
  array_string tmp27 = args;
  for (int i = 0; i < tmp27.len; i++) {
    string a = ((string *)tmp27.data)[i];
...
array_string compiler__V_v_files_from_dir(compiler__V *v, string dir) {
  ...
  Option_array_string tmp30 = os__ls(dir);
  array_string files;
  if (!tmp30.ok) {
    string err = tmp30.error;
    int errcode = tmp30.ecode;
  ...
  for (int tmp32 = 0; tmp32 < tmp31.len; tmp32++) {
    ...
    if (v->compile_defines_all.len > 0 && string_contains(file, tos3("_d_"))) {
      bool allowed = 0;
      array_string tmp33 = v->compile_defines;
      for (int tmp34 = 0; tmp34 < tmp33.len; tmp34++) {
        string cdefine = ((string *)tmp33.data)[tmp34];
...
void compiler__V_add_v_files_to_compile(compiler__V *v) {
  ...
  if (v->pref->is_cache) {
    ...
    if (os__exists(builtin_vh)) {
      _PUSH(&v->cached_mods,
            (/*typ = array_string   tmp_typ=string*/ tos3("builtin")), tmp36,
            string);
  ...
  if (v->pref->is_verbose) {
    string tmp37 = array_string_str(builtin_files);

    compiler__V_log(&/* ? */ *v,
                    _STR("v.add_v_files_to_compile > builtin_files: %.*s ",
                         tmp37.len, tmp37.str));
  };
  array_string tmp38 = builtin_files;
  for (int tmp39 = 0; tmp39 < tmp38.len; tmp39++) {
    string file = ((string *)tmp38.data)[tmp39];

    _PUSH(&v->files, (/*typ = array_string   tmp_typ=string*/ file), tmp40,
          string);
    compiler__Parser p = compiler__V_new_parser_from_file(v, file);
    compiler__Parser_parse(&/* ? */ p, compiler__compiler__Pass_imports);
    compiler__V_add_parser(v, p);
  };
  array_string tmp41 = compiler__V_get_user_files(&/* ? */ *v);
  for (int tmp42 = 0; tmp42 < tmp41.len; tmp42++) {
    ...
    if (p.v_script) {
      ...
      _PUSH(&p.table->imports,
            (/*typ = array_string   tmp_typ=string*/ tos3("os")), tmp43,
            string);
  ...
  array_string tmp44 = imported_mods;
  for (int tmp45 = 0; tmp45 < tmp44.len; tmp45++) {
...
array_string compiler__V_get_builtin_files(compiler__V *v) {
  if (v->pref->is_bare) {
    return compiler__V_v_files_from_dir(
        ...
        filepath__join(
            v->pref->vlib_path,
            &(varg_string){.len = 2, .args = {tos3("builtin"), tos3("bare")}}));
  };
#ifdef _VJS
  return compiler__V_v_files_from_dir(
      &/* ? */ *v,
      filepath__join(
          v->pref->vlib_path,
          &(varg_string){.len = 2, .args = {tos3("builtin"), tos3("js")}}));
#endif
  ;
  return compiler__V_v_files_from_dir(
      &/* ? */ *v,
      filepath__join(v->pref->vlib_path,
...
array_string compiler__V_get_user_files(compiler__V *v) {
  ...
  if (is_test) {
    Option_string tmp58 = os__read_file(dir);
    string tcontent;
    if (!tmp58.ok) {
      string err = tmp58.error;
      int errcode = tmp58.ecode;
      v_panic(_STR("%.*s does not exist", dir.len, dir.str));
    }
    tcontent = *(string *)tmp58.data;
    ;
  ...
  if (is_internal_module_test) {
    ...
    _PUSH(&user_files,
          (/*typ = array_string   tmp_typ=string*/ single_test_v_file), tmp59,
          string);
    dir = filepath__basedir(single_test_v_file);
  };
  if (string_ends_with(dir, tos3(".v")) ||
    ...
    for (int tmp62 = 0; tmp62 < tmp61.len; tmp62++) {
      ...
      _PUSH(&user_files, (/*typ = array_string   tmp_typ=string*/ file), tmp63,
            string);
    };
  };
  if (user_files.len == 0) {
  ...
  if (v->pref->is_verbose) {
    string tmp64 = array_string_str(user_files);

    compiler__V_log(&/* ? */ *v,
                    _STR("user_files: %.*s ", tmp64.len, tmp64.str));
  };
  return user_files;
}
array_string compiler__V_get_imported_module_files(compiler__V *v, string mod) {
  array_string files = new_array_from_c_array(
      0, 0, sizeof(string), EMPTY_ARRAY_OF_ELEMS(string, 0){TCCSKIP(0)});
  array_compiler__Parser tmp65 = v->parsers;
  for (int tmp66 = 0; tmp66 < tmp65.len; tmp66++) {
    compiler__Parser p = ((compiler__Parser *)tmp65.data)[tmp66];
...
void compiler__V_parse_lib_imports(compiler__V *v) {
  ...
  int tmp68 = 0;
  ;
  for (int tmp69 = tmp68; tmp69 < v->parsers.len; tmp69++) {
    int i = tmp69;
    ...
    for (int l = 0; l < keys_tmp70.len; l++) {
      ...
      Option_string tmp73 = compiler__V_find_module_path(&/* ? */ *v, mod);
      string import_path;
      if (!tmp73.ok) {
      ...
      _PUSH(&done_imports, (/*typ = array_string   tmp_typ=string*/ mod), tmp90,
            string);
    };
  };
}
...
string compiler__get_param_after(string joined_args, string arg, string def) {
  ...
  Option_int tmp91 = string_index(joined_args, key);
  int pos;
  if (!tmp91.ok) {
...
string compiler__get_cmdline_option(array_string args, string param,
                                    string def) {
  bool found = 0;
  array_string tmp94 = args;
  for (int tmp95 = 0; tmp95 < tmp94.len; tmp95++) {
    string arg = ((string *)tmp94.data)[tmp95];
...
compiler__V *compiler__new_v(array_string args) {
  if (!os__is_dir(compiler__v_modules_path)) {
    Option_bool tmp96 = os__mkdir(compiler__v_modules_path);
    if (!tmp96.ok) {
  ...
  string vlib_path = compiler__get_cmdline_option(
      args, tos3("-vlib-path"),
      filepath__join(vroot, &(varg_string){.len = 1, .args = {tos3("vlib")}}));
  string vpath = compiler__get_cmdline_option(args, tos3("-vpath"),
                                              compiler__v_modules_path);
  ...
  if (string_contains(out_name, os__path_separator)) {
    ...
    if (!os__is_dir(d)) {
      ...
      Option_bool tmp104 = os__mkdir(d);
      if (!tmp104.ok) {
        string err = tmp104.error;
        int errcode = tmp104.ecode;
        v_panic(err);
      };
    };
  };
  ...
  if (string_eq(target_os, tos3(""))) {
    ...
    ;
#ifdef __haiku__
    _os = compiler__compiler__OS_haiku;
#endif
    ;
  } else {
  ...
  string cflags = array_string_join(
      compiler__get_cmdline_multiple_values(args, tos3("-cflags")), tos3(" "));
  array_string defines =
      compiler__get_cmdline_multiple_values(args, tos3("-d"));
  _V_MulRet_array_string_V_array_string
      _V_mret_4643_compile_defines_compile_defines_all =
          compiler__parse_defines(defines);
  array_string compile_defines =
      _V_mret_4643_compile_defines_compile_defines_all.var_0;
  array_string compile_defines_all =
      _V_mret_4643_compile_defines_compile_defines_all.var_1;
  ...
  compiler__Preferences *pref = (compiler__Preferences *)memdup(
      &(compiler__Preferences){
          ...
          .is_solive = (_IN(string, (tos3("-solive")), args)),
  ...
  return (compiler__V *)memdup(
      &(compiler__V){
          ...
          .out_name = out_name,
          .dir = dir,
          .compiled_dir = ((os__is_dir(rdir)) ? (rdir) : (filepath__dir(rdir))),
          .lang_dir = vroot,
...
array_string compiler__non_empty(array_string a) {
  ...
  array_string tmp105 = new_array(0, a.len, sizeof(string));
  for (int i = 0; i < a.len; i++) {
    string it = ((string *)a.data)[i];
    if (it.len != 0)
      array_push(&tmp105, &it);
  }
  return tmp105;
}
array_string compiler__env_vflags_and_os_args() {
...
compiler__OS compiler__os_from_string(string os) {
  ...
  if (string_eq(tmp114, tos3("linux"))) {
    return compiler__compiler__OS_linux;
  } else if (string_eq(tmp114, tos3("windows"))) {
    return compiler__compiler__OS_windows;
  } else if (string_eq(tmp114, tos3("mac"))) {
    return compiler__compiler__OS_mac;
  } else if (string_eq(tmp114, tos3("macos"))) {
    return compiler__compiler__OS_mac;
  } else if (string_eq(tmp114, tos3("freebsd"))) {
    return compiler__compiler__OS_freebsd;
  } else if (string_eq(tmp114, tos3("openbsd"))) {
    return compiler__compiler__OS_openbsd;
  } else if (string_eq(tmp114, tos3("netbsd"))) {
    return compiler__compiler__OS_netbsd;
  } else if (string_eq(tmp114, tos3("dragonfly"))) {
    return compiler__compiler__OS_dragonfly;
  } else if (string_eq(tmp114, tos3("js"))) {
    return compiler__compiler__OS_js;
  } else if (string_eq(tmp114, tos3("solaris"))) {
    return compiler__compiler__OS_solaris;
  } else if (string_eq(tmp114, tos3("android"))) {
    return compiler__compiler__OS_android;
  } else if (string_eq(tmp114, tos3("msvc"))) {
    compiler__verror(tos3("use the flag `-cc msvc` to build using msvc"));
  } else if (string_eq(tmp114, tos3("haiku"))) {
...
compiler__V *compiler__new_v_compiler_with_args(array_string args) {
  ...
  _PUSH_MANY(&allargs, (/*typ = array_string   tmp_typ=string*/ args), tmp115,
             array_string);
...
compiler__Parser compiler__V_new_parser_from_file(compiler__V *v, string path) {
  ...
  if (v->compile_defines.len > 0) {
    array_string tmp3 = v->compile_defines;
    for (int tmp4 = 0; tmp4 < tmp3.len; tmp4++) {
  ...
  while (1) {
    ...
    if (tmp18 == compiler__compiler__TokenKind_key_import) {
      ...
      if (p->tok == compiler__compiler__TokenKind_assign) {
        compiler__Parser_next(p);
        g = string_add(g, tos3(" = "));
        _V_MulRet_string_V_string _V_mret_2266___expr =
            compiler__Parser_tmp_expr(p);
        string expr = _V_mret_2266___expr.var_1;
        g = string_add(g, expr);
      };
...
void compiler__Parser_type_decl(compiler__Parser *p) {
  ...
  compiler__Table_register_type(
      ...
      (compiler__Type){.name = name,
                       ...
                       .is_flag = 0,
                       .is_generic = 0});
  if (p->tok != compiler__compiler__TokenKind_key_type) {
    ;
  };
}
compiler__Fn *compiler__Parser_interface_method(compiler__Parser *p,
...
void compiler__Parser_check_not_reserved(compiler__Parser *p) {
  bool tmp36 = 0;
  bool tmp37 =
      map_get(/*parser.v : 869*/ compiler__reserved_types, p->lit, &tmp36);
...
string compiler__Parser_get_type(compiler__Parser *p) {
  ...
  if ((_IN(string, (p->lit), map_keys(&/* ? */ ti)))) {
    string tmp39 = tos3("");
    bool tmp40 = map_get(/*parser.v : 1057*/ ti, p->lit, &tmp39);
  ...
  if (string_eq(p->lit, tos3("C")) &&
    ...
    if (string_eq(t.name, tos3("")) && !p->builtin_mod) {
      ...
      if (string_eq(t.name, tos3("")) && !p->pref->translated &&
          ...
          !string_starts_with(typ, tos3("["))) {
        println(tos3("get_type() bad type"));
        _V_MulRet_string_V_string _V_mret_4829_t_suggest_tc_suggest =
            compiler__Table_find_misspelled_type(&/* ? */ *p->table, typ, p,
                                                 0.50);
        string t_suggest = _V_mret_4829_t_suggest_tc_suggest.var_0;
        string tc_suggest = _V_mret_4829_t_suggest_tc_suggest.var_1;
        if (t_suggest.len > 0) {
          t_suggest = _STR(". did you mean: (%.*s) `%.*s`", tc_suggest.len,
                           tc_suggest.str, t_suggest.len, t_suggest.str);
        };
  ...
  if (compiler__Parser_peek(&/* ? */ *p) == compiler__compiler__TokenKind_lt) {
    ...
    while (1) {
      ...
      if ((_IN_MAP((type_param), p->generic_dispatch.inst))) {
        string tmp41 = tos3("");
        bool tmp42 = map_get(/*parser.v : 1118*/ p->generic_dispatch.inst,
                             type_param, &tmp41);

        if (!tmp42)
          tmp41 = tos((byte *)"", 0);

        typ = string_add(_STR("%.*s_", typ.len, typ.str), tmp41);
      };
      if (p->tok != compiler__compiler__TokenKind_comma) {
        break;
      };
...
string compiler__Parser_statement(compiler__Parser *p, bool add_semi) {
  ...
  if (tmp49 == compiler__compiler__TokenKind_name) {
    compiler__TokenKind next = compiler__Parser_peek(&/* ? */ *p);
    if (compiler__Parser_peek(&/* ? */ *p) ==
  ...
  } else if (tmp49 == compiler__compiler__TokenKind_key_goto) {
    compiler__Parser_check(p, compiler__compiler__TokenKind_key_goto);
    ;
    string label = compiler__Parser_check_name(p);
    compiler__Parser_genln(p, _STR("goto %.*s;", label.len, label.str));
    return tos3("");
  } else if (tmp49 == compiler__compiler__TokenKind_key_defer) {
    compiler__Parser_defer_st(p);
    return tos3("");
  } else if (tmp49 == compiler__compiler__TokenKind_hash) {
    compiler__Parser_chash(p);
    return tos3("");
  } else if (tmp49 == compiler__compiler__TokenKind_key_unsafe) {
    compiler__Parser_next(p);
    p->inside_unsafe = 1;
    compiler__Parser_check(p, compiler__compiler__TokenKind_lcbr);
    compiler__Parser_genln(p, tos3("{"));
    compiler__Parser_statements(p);
    p->inside_unsafe = 0;
  } else if (tmp49 == compiler__compiler__TokenKind_dollar) {
    compiler__Parser_comp_time(p);
  } else if (tmp49 == compiler__compiler__TokenKind_key_if) {
    compiler__Parser_if_statement(p, 0, 0);
  } else if (tmp49 == compiler__compiler__TokenKind_key_for) {
    compiler__Parser_for_st(p);
  } else if (tmp49 == compiler__compiler__TokenKind_key_switch) {
    compiler__Parser_switch_statement(p);
  } else if (tmp49 == compiler__compiler__TokenKind_key_match) {
    compiler__Parser_match_statement(p, 0);
  } else if ((tmp49 == compiler__compiler__TokenKind_key_mut) ||
             (tmp49 == compiler__compiler__TokenKind_key_static)) {
    compiler__Parser_var_decl(p);
  } else if (tmp49 == compiler__compiler__TokenKind_key_return) {
    compiler__Parser_return_st(p);
  } else if (tmp49 == compiler__compiler__TokenKind_lcbr) {
    ...
    compiler__Parser_statements(p);
    return tos3("");
  } else if (tmp49 == compiler__compiler__TokenKind_key_continue) {
    if (p->for_expr_cnt == 0) {
  ...
  } else if (tmp49 == compiler__compiler__TokenKind_key_break) {
    if (p->for_expr_cnt == 0) {
  ...
  } else if (tmp49 == compiler__compiler__TokenKind_key_go) {
    compiler__Parser_go_statement(p);
  } else if (tmp49 == compiler__compiler__TokenKind_key_assert) {
    compiler__Parser_assert_statement(p);
  } else if (tmp49 == compiler__compiler__TokenKind_key_asm) {
...
void compiler__Parser_assign_statement(compiler__Parser *p, compiler__Var v,
  ...
  compiler__TokenKind tmp52 = tok;

  if (tmp52 == compiler__compiler__TokenKind_assign) {
    if (!is_map && !p->is_empty_c_struct_init) {
      compiler__Parser_gen(p, tos3(" = "));
    };
  } else if (tmp52 == compiler__compiler__TokenKind_plus_assign) {
    if (is_str && !p->is_js) {
  ...
  if (string_eq(expr_type, tos3("void"))) {
    _V_MulRet_bool_V_string _V_mret_6621___fn_name =
        compiler__Parser_is_expr_fn_call(&/* ? */ *p, p->token_idx - 3);
    string fn_name = _V_mret_6621___fn_name.var_1;
    compiler__Parser_error_with_token_index(
        p,
  ...
  if (string_starts_with(p->assigned_type, tos3("Option_")) &&
    ...
    string vname = string_replace(string_substr2(line, 0, pos, false),
                                  tos3("="), tos3(""));
    Option_int tmp63 = string_index(line, tos3("="));

    if (tmp63.ok) {
      int idx = *(int *)tmp63.data;
      compiler__CGen_resetln(
...
void compiler__Parser_var_decl(compiler__Parser *p) {
  ...
  while (p->tok == compiler__compiler__TokenKind_comma) {
    ...
    if (p->tok == compiler__compiler__TokenKind_key_mut) {
      compiler__Parser_check(p, compiler__compiler__TokenKind_key_mut);
      ;
      _PUSH(&var_mut, (/*typ = array_bool   tmp_typ=bool*/ 1), tmp80, bool);
    } else {
      _PUSH(&var_mut, (/*typ = array_bool   tmp_typ=bool*/ 0), tmp81, bool);
    };
    ...
    _PUSH(&var_names, (/*typ = array_string   tmp_typ=string*/ var_name), tmp83,
          string);
  };
  ...
  if (string_eq(t, tos3("void"))) {
    _V_MulRet_bool_V_string _V_mret_7492___fn_name =
        compiler__Parser_is_expr_fn_call(&/* ? */ *p, p->token_idx - 3);
    string fn_name = _V_mret_7492___fn_name.var_1;
    compiler__Parser_error_with_token_index(
        p,
        _STR("%.*s() %.*s", fn_name.len, fn_name.str,
             compiler__err_used_as_value.len, compiler__err_used_as_value.str),
  ...
  array_string tmp88 = var_names;
  for (int i = 0; i < tmp88.len; i++) {
    string var_name = ((string *)tmp88.data)[i];
...
string compiler__Parser_get_var_type(compiler__Parser *p, string name,
  ...
  if (is_ptr) {
    ...
    int tmp99 = 0;
    ;
    for (int tmp100 = tmp99; tmp100 < deref_nr; tmp100++) {
  ...
  if (deref_nr > 0) {
    ...
    int tmp101 = 0;
    ;
    for (int tmp102 = tmp101; tmp102 < deref_nr; tmp102++) {
...
string compiler__Parser_dot(compiler__Parser *p, string str_typ_,
  ...
  if (has_field) {
    string struct_field =
        ((string_ne(typ.name, tos3("Option")))
             ? (compiler__Table_var_cgen_name(&/* ? */ *p->table, field_name))
             : (field_name));
    Option_compiler__Var tmp109 = compiler__Table_find_field(
        &/* ? */ *p->table, &/*114*/ typ, struct_field);
    compiler__Var field;
    if (!tmp109.ok) {
      string err = tmp109.error;
      int errcode = tmp109.ecode;
      compiler__Parser_error_with_token_index(
          p,
    ...
    field = *(compiler__Var *)tmp109.data;
    ;
  ...
  method = *(compiler__Fn *)tmp110.data;
  ;
...
string compiler__Parser_index_expr(compiler__Parser *p, string typ_,
  ...
  if (is_variadic_arg) {
    if (is_indexer) {
      ...
      Option_int tmp127 = string_last_index(l, tos3(" "));
      int idx;
      if (!tmp127.ok) {
...
bool compiler__Parser_fileis(compiler__Parser *p, string s) {
  return string_contains(filepath__filename(p->scanner->file_path), s);
}
string compiler__Parser_indot_expr(compiler__Parser *p) {
  int ph = compiler__CGen_add_placeholder(&/* ? */ *p->cgen);
...
string compiler__Parser_assoc(compiler__Parser *p) {
  ...
  while (p->tok != compiler__compiler__TokenKind_rcbr) {
    ...
    f = *(compiler__Var *)tmp136.data;
    ;
    _PUSH(&fields, (/*typ = array_string   tmp_typ=string*/ field), tmp137,
          string);
    compiler__Parser_gen(p, _STR(".%.*s = ", field.len, field.str));
    compiler__Parser_check(p, compiler__compiler__TokenKind_colon);
  ...
  array_compiler__Var tmp138 = typ.fields;
  for (int tmp139 = 0; tmp139 < tmp138.len; tmp139++) {
    compiler__Var ffield = ((compiler__Var *)tmp138.data)[tmp139];
...
string compiler__Parser_map_init(compiler__Parser *p) {
  ...
  if (p->tok == compiler__compiler__TokenKind_lcbr) {
    ...
    while (1) {
      ...
      compiler__Parser_check(p, compiler__compiler__TokenKind_colon);
      ;
      _V_MulRet_string_V_string _V_mret_11636_t_val_expr =
          compiler__Parser_tmp_expr(p);
      string t = _V_mret_11636_t_val_expr.var_0;
      string val_expr = _V_mret_11636_t_val_expr.var_1;
      if (i == 0) {
        val_type = t;
      };
      i++;
    ...
    if (exp_array) {
      string type_expected =
          string_replace(string_substr2(p->expected_type, 6, -1, true),
                         tos3("ptr_"), tos3("&"));
      compiler__Parser_error(p, _STR("no need to specify the full array type "
...
void compiler__Parser_return_st(compiler__Parser *p) {
  ...
  if (fn_returns) {
    ...
    _PUSH(&types, (/*typ = array_string   tmp_typ=string*/ expr_type), tmp151,
          string);
    while (p->tok == compiler__compiler__TokenKind_comma) {

      compiler__Parser_check(p, compiler__compiler__TokenKind_comma);
      _V_MulRet_string_V_string _V_mret_13084_typ_expr =
          compiler__Parser_tmp_expr(p);
      string typ = _V_mret_13084_typ_expr.var_0;
      string expr = _V_mret_13084_typ_expr.var_1;
      _PUSH(&types, (/*typ = array_string   tmp_typ=string*/ typ), tmp152,
            string);
      _PUSH(&mr_values,
    ...
    if (types.len > 1) {
      ...
      array_string tmp154 = mr_values;
      for (int ret_val_idx = 0; ret_val_idx < tmp154.len; ret_val_idx++) {
        string ret_val = ((string *)tmp154.data)[ret_val_idx];
...
string compiler__Parser_get_deferred_text(compiler__Parser *p) {
  ...
  array_string tmp159 = p->cur_fn.defer_text;
  for (int tmp160 = 0; tmp160 < tmp159.len; tmp160++) {
    string text = ((string *)tmp159.data)[tmp160];
...
void compiler__Parser_go_statement(compiler__Parser *p) {
  ...
  if (compiler__Parser_peek(&/* ? */ *p) == compiler__compiler__TokenKind_dot) {
    ...
    method = *(compiler__Fn *)tmp162.data;
    ;
    compiler__Parser_async_fn_call(p, method, 0, var_name, v.typ);
  } else {
    string f_name = p->lit;
    Option_compiler__Fn tmp163 = compiler__Table_find_fn(
        &/* ? */ *p->table, compiler__Parser_prepend_mod(&/* ? */ *p, f_name));
    compiler__Fn f;
    if (!tmp163.ok) {
      string err = tmp163.error;
      int errcode = tmp163.ecode;
...
string compiler__Parser_js_decode(compiler__Parser *p) {
  ...
  if (string_eq(op, tos3("decode"))) {
    ...
    string typ = compiler__Parser_get_type(p);
    compiler__Parser_check(p, compiler__compiler__TokenKind_comma);
    _V_MulRet_string_V_string _V_mret_13869_styp_expr =
        compiler__Parser_tmp_expr(p);
    string styp = _V_mret_13869_styp_expr.var_0;
    string expr = _V_mret_13869_styp_expr.var_1;
    ...
    compiler__Type T = compiler__Table_find_type(&/* ? */ *p->table, typ);
    array_compiler__Var tmp164 = T.fields;
    for (int tmp165 = 0; tmp165 < tmp164.len; tmp165++) {
      compiler__Var field = ((compiler__Var *)tmp164.data)[tmp165];
    ...
    compiler__Parser_check(p, compiler__compiler__TokenKind_lpar);
    _V_MulRet_string_V_string _V_mret_14049_typ_expr =
        compiler__Parser_tmp_expr(p);
    string typ = _V_mret_14049_typ_expr.var_0;
    string expr = _V_mret_14049_typ_expr.var_1;
    compiler__Type T = compiler__Table_find_type(&/* ? */ *p->table, typ);
    compiler__Parser_gen_json_for_type(p, T);
    compiler__Parser_check(p, compiler__compiler__TokenKind_rpar);
...
void compiler__Parser_check_and_register_used_imported_type(compiler__Parser *p,
                                                            string typ_name) {
  Option_int tmp171 = string_index(typ_name, tos3("__"));
  int us_idx;
  if (!tmp171.ok) {
...
void compiler__Parser_check_unused_imports(compiler__Parser *p) {
  ...
  string output = tos3("");
  map_string tmp174 = p->import_table.imports;
  array_string keys_tmp174 = map_keys(&tmp174);
  for (int l = 0; l < keys_tmp174.len; l++) {
    string alias = ((string *)keys_tmp174.data)[l];
    string mod = tos3("");
    map_get(tmp174, alias, &mod);
...
compiler__ScanRes compiler__Scanner_scan(compiler__Scanner *s) {
  ...
  } else if ((tmp48 == compiler__single_quote) ||
             (tmp48 == compiler__double_quote)) {
    return compiler__scan_res(compiler__compiler__TokenKind_str,
                              compiler__Scanner_ident_string(s));
  } else if (tmp48 == '`') {
...
bool compiler__Scanner_expect(compiler__Scanner *s, string want,
  ...
  int tmp101 = start_pos;
  ;
  for (int tmp102 = tmp101; tmp102 < end_pos; tmp102++) {
    int pos = tmp102;
...
bool compiler__good_type_name(string s) {
  ...
  int tmp118 = 2;
  ;
  for (int tmp119 = tmp118; tmp119 < s.len; tmp119++) {
    int i = tmp119;
...
void compiler__Parser_string_expr(compiler__Parser *p) {
  ...
  while (p->tok == compiler__compiler__TokenKind_str) {
    ...
    _V_MulRet_string_V_string _V_mret_283_typ_val_ =
        compiler__Parser_tmp_expr(p);
...
void compiler__Parser_struct_decl(compiler__Parser *p,
  ...
  while (p->tok != compiler__compiler__TokenKind_rcbr) {
    ...
    _PUSH(&names, (/*typ = array_string   tmp_typ=string*/ field_name), tmp11,
          string);
    if (is_interface) {
    ...
    if (p->tok == compiler__compiler__TokenKind_lsbr) {
      ...
      if (p->tok == compiler__compiler__TokenKind_colon) {
        compiler__Parser_check(p, compiler__compiler__TokenKind_colon);
        string val = tos3("");
        compiler__TokenKind tmp12 = p->tok;

        if (tmp12 == compiler__compiler__TokenKind_name) {
          val = compiler__Parser_check_name(p);
        } else if (tmp12 == compiler__compiler__TokenKind_str) {
...
string compiler__Parser_struct_init(compiler__Parser *p, string typ_) {
  ...
  if (peek == compiler__compiler__TokenKind_colon ||
    ...
    while (p->tok != compiler__compiler__TokenKind_rcbr) {
      ...
      Option_compiler__Var tmp16 = compiler__Type_find_field(&/* ? */ t, field);
      compiler__Var f;
      if (!tmp16.ok) {
        string err = tmp16.error;
        int errcode = tmp16.ecode;
    ...
    array_compiler__Var tmp18 = t.fields;
    for (int i = 0; i < tmp18.len; i++) {
      compiler__Var field = ((compiler__Var *)tmp18.data)[i];
      ...
      if (string_starts_with(field_typ, tos3("map_"))) {
        ...
        _PUSH(&inited_fields,
              (/*typ = array_string   tmp_typ=string*/ sanitized_name), tmp21,
              string);
        if (i != t.fields.len - 1) {
          compiler__Parser_gen(p, tos3(","));
        };
    ...
    array_compiler__Var tmp26 = T.fields;
    for (int i = 0; i < tmp26.len; i++) {
      compiler__Var ffield = ((compiler__Var *)tmp26.data)[i];
...
void compiler__Type_rename_generic_struct(compiler__Type *t,
  ...
  map_string tmp27 = generic_types;
...
void compiler__Parser_dispatch_generic_struct(compiler__Parser *p,
  ...
  if ((_IN_MAP((t->name), p->table->generic_struct_params))) {
    ...
    for (int _ = 0; _ < tmp28.len; _++) {
      string v = ((string *)tmp28.data)[_];
  ...
  _PUSH_MANY(&p->cgen->lines_extra,
             (/*typ = array_string   tmp_typ=string*/ p->cgen->lines), tmp36,
             array_string);
  compiler__Parser_restore_state(p, saved_state, 0, 1);
}
bool array_compiler__AccessMod_contains(array_compiler__AccessMod a,
...
Option_compiler__Fn compiler__Table_find_fn_is_script(compiler__Table *t,
  ...
  bool tmp17 = map_get(/*table.v : 353*/ t->fns, name, &tmp16);
...
void compiler__Table_add_field(compiler__Table *table, string type_name,
  ...
  bool tmp25 = map_get(/*table.v : 438*/ table->typesmap, type_name, &tmp24);
...
void compiler__Table_add_default_val(compiler__Table *table, int idx,
  ...
  bool tmp28 = map_get(/*table.v : 452*/ table->typesmap, type_name, &tmp27);
...
void compiler__Parser_add_method(compiler__Parser *p, string type_name,
  ...
  compiler__Type tmp40 = {0};
  bool tmp41 = map_get(/*table.v : 513*/ p->table->typesmap, type_name, &tmp40);
...
Option_compiler__Fn compiler__Table_find_method(compiler__Table *table,
  ...
  bool tmp46 = map_get(/*table.v : 536*/ table->typesmap, typ->name, &tmp45);
...
void compiler__Table_add_gen_type(compiler__Table *table, string type_name,
  ...
  bool tmp57 = map_get(/*table.v : 566*/ table->typesmap, type_name, &tmp56);
...
compiler__Type compiler__Table_find_type(compiler__Table *t, string name_) {
  ...
  bool tmp60 = map_get(/*table.v : 592*/ t->typesmap, name, &tmp59);
...
bool compiler__Table_is_interface(compiler__Table *table, string name) {
  ...
  bool tmp68 = map_get(/*table.v : 782*/ table->typesmap, name, &tmp67);

  compiler__Type t = tmp67;
  return t.cat == compiler__compiler__TypeCategory_interface_;
}
bool compiler__Table_main_exists(compiler__Table *t) {
  map_compiler__Fn tmp69 = t->fns;
...
array_string compiler__Table_all_test_function_names(compiler__Table *t) {
  ...
  for (int l = 0; l < keys_tmp70.len; l++) {
    ...
    if (string_contains(f.name, tos3("__test_"))) {
      _PUSH(&res, (/*typ = array_string   tmp_typ=string*/ f.name), tmp71,
            string);
    };
  };
  return res;
...
bool compiler__is_valid_int_const(string val, string typ) {
  ...
  return ((string_eq(tmp77, tos3("byte")))
              ? (0 <= x && x <= 255)
              : ((string_eq(tmp77, tos3("u16")))
                     ? (0 <= x && x <= 65535)
                     : ((string_eq(tmp77, tos3("i8"))) ? (-128 <= x && x <= 127)
                                                       : (1))));
}
string compiler__Parser_typ_to_fmt(compiler__Parser *p, string typ, int level) {
  compiler__Type t = compiler__Table_find_type(&/* ? */ *p->table, typ);
  ...
  if (string_eq(tmp78, tos3("string"))) {
    ...
    return tos3("%.*s");
  } else if ((string_eq(tmp78, tos3("byte"))) ||
             (string_eq(tmp78, tos3("bool"))) ||
             (string_eq(tmp78, tos3("int"))) ||
             (string_eq(tmp78, tos3("char"))) ||
             (string_eq(tmp78, tos3("byte"))) ||
             (string_eq(tmp78, tos3("i16"))) ||
             (string_eq(tmp78, tos3("i8")))) {
    return tos3("%d");
  } else if ((string_eq(tmp78, tos3("u16"))) ||
             (string_eq(tmp78, tos3("u32")))) {
    return tos3("%u");
  } else if ((string_eq(tmp78, tos3("f64"))) ||
             (string_eq(tmp78, tos3("f32")))) {
    return tos3("%f");
...
bool compiler__Type_contains_field_type(compiler__Type *t, string typ) {
  ...
  array_compiler__Var tmp83 = t->fields;
  for (int tmp84 = 0; tmp84 < tmp83.len; tmp84++) {
    compiler__Var field = ((compiler__Var *)tmp83.data)[tmp84];
...
string compiler__Parser_identify_typo(compiler__Parser *p, string name) {
  ...
  _V_MulRet_string_V_string _V_mret_3869_typ_type_cat =
      compiler__Table_find_misspelled_type(&/* ? */ *p->table, name, p,
                                           min_match);
  string typ = _V_mret_3869_typ_type_cat.var_0;
  string type_cat = _V_mret_3869_typ_type_cat.var_1;
  if (typ.len > 0) {
    output = string_add(output, _STR("\n  * %.*s: `%.*s`", type_cat.len,
...
string compiler__Table_find_misspelled_fn(compiler__Table *table, string name,
  ...
  map_compiler__Fn tmp93 = table->fns;
...
string compiler__Table_find_misspelled_const(compiler__Table *table,
  ...
  array_compiler__Var tmp97 = table->consts;
  for (int tmp98 = 0; tmp98 < tmp97.len; tmp98++) {
    compiler__Var cnst = ((compiler__Var *)tmp97.data)[tmp98];
...
compiler__Table_find_misspelled_type(compiler__Table *table, string name,
  ...
  map_compiler__Type tmp99 = table->typesmap;
...
string compiler__type_cat_str(compiler__TypeCategory tc) {
  ...
  string tc_str =
      ((tmp100 == compiler__compiler__TypeCategory_builtin)
           ? (tos3("builtin"))
           : ((tmp100 == compiler__compiler__TypeCategory_struct_)
                  ? (tos3("struct"))
                  : ((tmp100 == compiler__compiler__TypeCategory_func)
                         ? (tos3("function"))
                         : ((tmp100 ==
...
void init() {
  ...
  strconv__TEN = ((u32)(10));
  strconv__pos_exp = new_array_from_c_array(
      309, 309, sizeof(u64),
      EMPTY_ARRAY_OF_ELEMS(u64, 309){
