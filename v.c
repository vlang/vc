...
array_string env_vflags_and_os_args() {
  ...
  if (string_ne(tos2((byte *)""), vflags)) {
    ...
    _PUSH(&args, ((*(string *)array__get(os__args, 0))), tmp103, string);

    _PUSH_MANY(&args, (string_split(vflags, tos2((byte *)" "))), tmp106,
               array_string);

    if (os__args.len > 1) {

      _PUSH_MANY(&args, (array_right(os__args, 1)), tmp107, array_string);
    };

  } else {

    _PUSH_MANY(&args, (os__args), tmp108, array_string);
  };
...
void install_v(array_string args) {
  ...
  if (1) {
    ...
    os__Result vgetcompilation = *(os__Result *)tmp117.data;
    ;
