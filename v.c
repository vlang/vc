...
void Parser_fn_decl(Parser *p) {
  ...
  string fn_name_cgen =
      Table_cgen_name(p->table, &/*112 EXP:"Fn*" GOT:"Fn" */ f);
...
V *new_v(array_string args) {
  ...
  array_string tmp90 = builtins;
  ;
  for (int tmp91 = 0; tmp91 < tmp90.len; tmp91++) {
    string builtin = ((string *)tmp90.data)[tmp91];
  ...
  array_string tmp95 = args;
  ;
  for (int ci = 0; ci < tmp95.len; ci++) {
    string cv = ((string *)tmp95.data)[ci];
...
void install_v(array_string args) {
  ...
  if (1) {
    ...
    Option_os__Result tmp120 = os__exec(
    ...
    os__Result vgetcompilation = *(os__Result *)tmp120.data;
    ;
  ...
  os__Result vgetresult = *(os__Result *)tmp121.data;
  ;
...
void test_v() {
  ...
  array_string tmp129 = test_files;
  ;
  for (int tmp130 = 0; tmp130 < tmp129.len; tmp130++) {
    string dot_relative_file = ((string *)tmp129.data)[tmp130];
    ...
    Option_os__Result tmp135 = os__exec(cmd);
    if (!tmp135.ok) {
    ...
    os__Result r = *(os__Result *)tmp135.data;
    ;
...
string Parser_bterm(Parser *p) {
  ...
  if (tok == main__Token_eq || tok == main__Token_gt || tok == main__Token_lt ||
    ...
    if (p->is_sql) {
      ...
      _PUSH(&p->sql_params, (sql_param), tmp136, string);

      _PUSH(&p->sql_types, (typ), tmp137, string);
...
string Parser_assoc(Parser *p) {
  ...
  while (p->tok != main__Token_rcbr) {
    ...
    _PUSH(&fields, (field), tmp240, string);
  ...
  array_Var tmp242 = T.fields;
  ;
  for (int tmp243 = 0; tmp243 < tmp242.len; tmp243++) {
    Var ffield = ((Var *)tmp242.data)[tmp243];
...
string Parser_struct_init(Parser *p, string typ, bool is_c_struct_init) {
  ...
  if (peek == main__Token_colon || p->tok == main__Token_rcbr) {
    ...
    while (p->tok != main__Token_rcbr) {
      ...
      _PUSH(&inited_fields, (field), tmp297, string);
    ...
    array_Var tmp298 = t.fields;
    ;
    for (int i = 0; i < tmp298.len; i++) {
      Var field = ((Var *)tmp298.data)[i];
      ...
      if (string_starts_with(field_typ, tos2((byte *)"map_"))) {
        ...
        _PUSH(&inited_fields, (field.name), tmp300, string);
  ...
  if (fn_returns) {
    ...
    if (p->tok == main__Token_rcbr) {
      ...
      if (string_ends_with(p->cur_fn.typ, expr_type) &&
        ...
        array_string tmp374 = p->cur_fn.defer_text;
        ;
        for (int tmp375 = 0; tmp375 < tmp374.len; tmp375++) {
          string text = ((string *)tmp374.data)[tmp375];
...
string Parser_js_decode(Parser *p) {
  ...
  if (string_eq(op, tos2((byte *)"decode"))) {
    ...
    array_Var tmp389 = T.fields;
    ;
    for (int tmp390 = 0; tmp390 < tmp389.len; tmp390++) {
      Var field = ((Var *)tmp389.data)[tmp390];
    ...
    _PUSH(&p->cgen->typedefs,
          (_STR("typedef Option %.*s;", opt_type.len, opt_type.str)), tmp393,
          string);
