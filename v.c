...
void Parser_fn_decl(Parser *p) {
  ...
  if (!is_c && !is_live && !is_sig && !is_fn_header && !skip_main_in_test) {
    ...
    if (is_generic) {
      ...
      array_string tmp63 = gen_types;
      for (int tmp64 = 0; tmp64 < tmp63.len; tmp64++) {
        string gen_type = ((string *)tmp63.data)[tmp64];
  ...
  if (is_sig || Parser_first_pass(&/* ? */ *p) || is_live || is_fn_header ||
    ...
    if (is_live && Parser_first_pass(&/* ? */ *p) &&
      ...
      _PUSH(&p->cgen->so_fns, (fn_name_cgen), tmp77, string);
    ...
    if (!is_c && string_ne(f.name, tos2((byte *)"main")) &&
      ...
      _PUSH(&p->cgen->fns, (string_add(fn_decl, tos2((byte *)";"))), tmp79,
            string);
    };
...
void Parser_check_unused_variables(Parser *p) {
  ...
  array_Var tmp86 = p->cur_fn.local_vars;
  for (int tmp87 = 0; tmp87 < tmp86.len; tmp87++) {
    Var var = ((Var *)tmp86.data)[tmp87];
...
void Parser_async_fn_call(Parser *p, Fn f, int method_ph, string receiver_var,
  ...
  array_Var tmp95 = f.args;
  for (int i = 0; i < tmp95.len; i++) {
    Var arg = ((Var *)tmp95.data)[i];
...
void Parser_fn_args(Parser *p, Fn *f) {
  ...
  if (f->is_interface) {
    ...
    _PUSH(&f->args, (int_arg), tmp105, Var);
  };
  ...
  while (p->tok != main__Token_rpar) {
    ...
    while (p->tok == main__Token_comma) {
      ...
      _PUSH(&names, (Parser_check_name(p)), tmp111, string);
    };
    ...
    array_string tmp114 = names;
    for (int tmp115 = 0; tmp115 < tmp114.len; tmp115++) {
      string name = ((string *)tmp114.data)[tmp115];
...
Fn *Parser_fn_call_args(Parser *p, Fn *f) {
  ...
  array_Var tmp124 = f->args;
  for (int i = 0; i < tmp124.len; i++) {
    Var arg = ((Var *)tmp124.data)[i];
    ...
    if (arg.is_mut) {
      ...
      Option_Var tmp129 = Fn_find_var(&/* ? */ p->cur_fn, var_name);
      if (!tmp129.ok) {
    ...
    if (is_interface) {
      ...
      array_Fn tmp143 = interface_type.methods;
      for (int tmp144 = 0; tmp144 < tmp143.len; tmp144++) {
        Fn method = ((Fn *)tmp143.data)[tmp144];
...
string Fn_str_args(Fn *f, Table *table) {
  ...
  for (int i = 0; i < tmp152.len; i++) {
    ...
    if (Table_is_interface(&/* ? */ *table, arg.typ)) {
      ...
      array_Fn tmp154 = interface_type.methods;
      for (int tmp155 = 0; tmp155 < tmp154.len; tmp155++) {
        Fn method = ((Fn *)tmp154.data)[tmp155];
        ...
        if (method.args.len > 1) {
          ...
          array_Var tmp156 = array_right(method.args, 1);
          for (int tmp157 = 0; tmp157 < tmp156.len; tmp157++) {
            Var a = ((Var *)tmp156.data)[tmp157];
...
void Parser_var_decl(Parser *p) {
  ...
  if (!p->builtin_mod && Fn_known_var(&/* ? */ p->cur_fn, name)) {
    ...
    Option_Var v = Fn_find_var(&/* ? */ p->cur_fn, name);
...
string Parser_name_expr(Parser *p) {
  ...
  while (1) {
    ...
    Option_Var tmp143 = Parser_find_var_check_new_var(&/* ? */ *p, name);
    if (!tmp143.ok) {
      string err = tmp143.error;

      break;
    }
    Var v = *(Var *)tmp143.data;
    ;
  ...
  while (1) {
    ...
    Option_Var tmp150 = Table_find_const(&/* ? */ *p->table, name);
    if (!tmp150.ok) {
      string err = tmp150.error;

      break;
    }
    Var c = *(Var *)tmp150.data;
    ;
  ...
  Option_Fn tmp152 = Table_find_fn(&/* ? */ *p->table, name);
  if (!tmp152.ok) {
...
string Parser_dot(Parser *p, string str_typ, int method_ph) {
  ...
  if (has_field) {
    ...
    Option_Var tmp167 = Table_find_field(
  ...
  Option_Fn tmp172 = Table_find_method(
      &/* ? */ *p->table, &/*112 EXP:"Type*" GOT:"Type" */ typ, field_name);
  if (!tmp172.ok) {
...
string Parser_assoc(Parser *p) {
  ...
  Option_Var tmp225 = Fn_find_var(&/* ? */ p->cur_fn, name);
  if (!tmp225.ok) {
...
string Parser_array_init(Parser *p) {
  ...
  if (p->tok == main__Token_name && !p->inside_const) {
    ...
    if (Table_known_const(&/* ? */ *p->table, const_name)) {
      ...
      Option_Var tmp262 = Table_find_const(&/* ? */ *p->table, const_name);
      if (!tmp262.ok) {
        string err = tmp262.error;

        v_exit(1);
      }
      Var c = *(Var *)tmp262.data;
      ;
...
string Parser_struct_init(Parser *p, string typ) {
  ...
  if (peek == main__Token_colon || p->tok == main__Token_rcbr) {
    ...
    while (p->tok != main__Token_rcbr) {
      ...
      Option_Var tmp282 = Type_find_field(&/* ? */ t, field);
      if (!tmp282.ok) {
        string err = tmp282.error;

        v_panic(tos2((byte *)"field"));
      }
      Var f = *(Var *)tmp282.data;
      ;

      _PUSH(&inited_fields, (field), tmp283, string);
    ...
    array_Var tmp284 = t.fields;
    for (int i = 0; i < tmp284.len; i++) {
      Var field = ((Var *)tmp284.data)[i];
      ...
      if (string_starts_with(field_typ, tos2((byte *)"map_"))) {
        ...
        _PUSH(&inited_fields, (field.name), tmp286, string);
    ...
    array_Var tmp289 = T.fields;
    for (int i = 0; i < tmp289.len; i++) {
      Var ffield = ((Var *)tmp289.data)[i];
  ...
  if (fn_returns) {
    ...
    if (p->tok == main__Token_rcbr) {
      ...
      if (string_ends_with(p->cur_fn.typ, expr_type) && !is_none &&
        ...
        array_string tmp356 = p->cur_fn.defer_text;
        for (int tmp357 = 0; tmp357 < tmp356.len; tmp357++) {
          string text = ((string *)tmp356.data)[tmp357];
...
void Parser_go_statement(Parser *p) {
  ...
  if (Parser_peek(p) == main__Token_dot) {
    ...
    Option_Var tmp360 = Fn_find_var(&/* ? */ p->cur_fn, var_name);
    if (!tmp360.ok) {
      string err = tmp360.error;

      return;
    }
    Var v = *(Var *)tmp360.data;
    ;
    ...
    Option_Fn tmp362 = Table_find_method(
    ...
    Option_Fn tmp363 = Table_find_fn(&/* ? */ *p->table, p->lit);
    if (!tmp363.ok) {
...
string Parser_js_decode(Parser *p) {
  ...
  if (string_eq(op, tos2((byte *)"decode"))) {
    ...
    array_Var tmp371 = T.fields;
    for (int tmp372 = 0; tmp372 < tmp371.len; tmp372++) {
      Var field = ((Var *)tmp371.data)[tmp372];
    ...
    _PUSH(&p->cgen->typedefs,
          (_STR("typedef Option %.*s;", opt_type.len, opt_type.str)), tmp375,
          string);
...
Option_Var Table_find_const(Table *t, string name) {
  ...
  array_Var tmp76 = t->consts;
  for (int tmp77 = 0; tmp77 < tmp76.len; tmp77++) {
    Var c = ((Var *)tmp76.data)[tmp77];

    if (string_eq(c.name, name)) {

      Var tmp78 = OPTION_CAST(Var)(c);
      return opt_ok(&tmp78, sizeof(Var));
    };
  };
...
array_string Table_fn_gen_types(Table *t, string fn_name) {
  ...
  array_GenTable tmp87 = t->generic_fns;
  for (int _ = 0; _ < tmp87.len; _++) {
    GenTable f = ((GenTable *)tmp87.data)[_];
...
void Table_register_generic_fn_type(Table *t, string fn_name, string typ) {
  ...
  for (int i = 0; i < tmp88.len; i++) {
    GenTable f = ((GenTable *)tmp88.data)[i];

    if (string_eq(f.fn_name, fn_name)) {

      _PUSH(&(*(GenTable *)array__get(t->generic_fns, i)).types, (typ), tmp91,
            string);
...
void FileImportTable_register_alias(FileImportTable *fit, string alias,
  ...
  if (string_contains(mod, tos2((byte *)".internal."))) {
    ...
    for (int tmp105 = 0; tmp105 < tmp104.len; tmp105++) {
      ...
      _PUSH(&internal_mod_parts, (part), tmp106, string);
    };
...
bool Type_contains_field_type(Type *t, string typ) {
  ...
  array_Var tmp113 = t->fields;
  for (int tmp114 = 0; tmp114 < tmp113.len; tmp114++) {
    Var field = ((Var *)tmp113.data)[tmp114];
...
string Table_find_misspelled_fn(Table *table, string name, FileImportTable *fit,
  ...
  map_Fn tmp122 = table->fns;
  array_string keys_tmp122 = map_keys(&tmp122);
  for (int l = 0; l < keys_tmp122.len; l++) {
    string _ = ((string *)keys_tmp122.data)[l];
    Fn f = {0};
    map_get(tmp122, _, &f);
...
string Table_find_misspelled_imported_mod(Table *table, string name,
  ...
  map_string tmp129 = fit->imports;
  array_string keys_tmp129 = map_keys(&tmp129);
  for (int l = 0; l < keys_tmp129.len; l++) {
    string alias = ((string *)keys_tmp129.data)[l];
    string mod = {0};
    map_get(tmp129, alias, &mod);
