...
/* returns 0 */ Option_string os__exec(string _cmd) {
  ...
  string tmp53 = OPTION_CAST(string)(res);
  return opt_ok(&tmp53, sizeof(string));
}
...
/* returns 0 */ array_string os__walk_ext(string path, string ext) {
  ...
  array_string tmp109 = files;
  ;
  for (int i = 0; i < tmp109.len; i++) {
    string file = ((string *)tmp109.data)[i];
    ...
    if (os__is_dir(p)) {
      ...
      _PUSH_MANY(&res, (os__walk_ext(p, ext)), tmp111, array_string);

    } else if (string_ends_with(file, ext)) {

      _PUSH(&res, (p), tmp112, string);
    };
  };
...
/* returns 0 */ Option_string os__get_module_filename(HANDLE handle) {
  ...
  while (1) {
    ...
    if ((status == os__SUCCESS)) { /* case */
      ...
      string tmp16 = OPTION_CAST(string)(_filename);
...
/* returns 0 */ int main(int argc, char **argv) {
  ...
  if (_IN(string, tos2((byte *)"install"), args)) {
    ...
    if (1) {
      ...
      Option_string tmp6 = os__exec(_STR("%.*s -o %.*s vget.v", vexec.len,
...
/* returns 0 */ void V_cc(V *v) {
  ...
  _PUSH(&a, (_STR("\".%.*s\"", v->out_name_c.len, v->out_name_c.str)), tmp73,
        string);
  ...
  if (v->os == main__OS_linux && !linux_host &&
    ...
    Option_string tmp87 = os__exec(string_add(
        string_add(
            string_add(
                string_add(
                    string_add(
...
/* returns 0 */ array_string env_vflags_and_os_args() {
  ...
  if (string_ne(tos2((byte *)""), vflags)) {
    ...
    _PUSH_MANY(&args, (string_split(vflags, tos2((byte *)" "))), tmp188,
               array_string);
...
/* returns 0 */ void update_v() {
  ...
  Option_string tmp192 = os__exec(_STR(
...
/* returns 0 */ Option_VsInstallation find_vs() {
  ...
  Option_string tmp26 = os__exec(tos2(
  ...
  string lib_path = _STR("%.*s\\VC\\Tools\\MSVC\\%.*s\\lib\\x64", res.len,
                         res.str, v.len, v.str);

  string include_path = _STR("%.*s\\VC\\Tools\\MSVC\\%.*s\\include", res.len,
                             res.str, v.len, v.str);

  if (os__file_exists(
          _STR("%.*s\\vcruntime.lib", lib_path.len, lib_path.str))) {

    string p = _STR("%.*s\\VC\\Tools\\MSVC\\%.*s\\bin\\Hostx64\\x64", res.len,
                    res.str, v.len, v.str);
...
/* returns 0 */ void V_cc_msvc(V *v) {
  ...
  _PUSH(&a, (_STR("/OUT:%.*s", v->out_name.len, v->out_name.str)), tmp64,
        string);
...
/* returns 0 */ void build_thirdparty_obj_file_with_msvc(string flag) {
  ...
  Option_string tmp85 = os__exec(_STR(
...
/* returns 0 */ void Parser_parse(Parser *p) {
  ...
  p->can_chash = string_eq(p->mod, tos2((byte *)"freetype")) ||
...
/* returns 0 */ string Parser_bterm(Parser *p) {
  ...
  if (tok == main__Token_eq || (tok == main__Token_assign && p->is_sql) ||
      tok == main__Token_gt || tok == main__Token_lt || tok == main__Token_le ||
      tok == main__Token_ge || tok == main__Token_ne) {
    ...
    if (p->is_sql) {
      ...
      p->sql_params = string_add(string_add(p->sql_params, CGen_cut(p->cgen)),
                                 tos2((byte *)","));
...
/* returns 0 */ string Parser_js_decode(Parser *p) {
  ...
  if (string_eq(op, tos2((byte *)"decode"))) {
    ...
    _PUSH(&p->cgen->typedefs,
          (_STR("typedef Option %.*s;", opt_type.len, opt_type.str)), tmp361,
          string);
...
/* returns 0 */ string Parser_select_query(Parser *p, int fn_ph) {
  ...
  if (string_eq(n, tos2((byte *)"count"))) {
    ...
    for (int i = 0; i < tmp17.len; i++) {
      ...
      strings__Builder_writeln(
          &/* ? */ obj_gen,
          _STR("%.*s . %.*s = %.*s( *(string*)array__get(row.vals, %d) );",
               tmp.len, tmp.str, field.name.len, field.name.str, cast.len,
               cast.str, i));
    };
    ...
    if (query_one) {
      ...
      CGen_insert_before(
          p->cgen, _STR("\n\npg__Row row = pg__DB_exec_one(db, "
                        "tos2(\"%.*s\"));\n%.*s %.*s; \n%.*s \n\n",
                        q.len, q.str, table_name.len, table_name.str, tmp.len,
                        tmp.str, strings__Builder_str(obj_gen).len,
                        strings__Builder_str(obj_gen).str));
      ...
      for (int i = 0; i < tmp21.len; i++) {
        ...
        params_gen =
            string_add(params_gen, _STR("params[%d] = int_str(%.*s).str;", i,
                                        param.len, param.str));
      };

      CGen_insert_before(
          p->cgen,
          _STR("char* params[%d];\n%.*s \n\nvoid* res = PQexecParams(db.conn, "
               "\"%.*s\", %d, 0, params, 0, 0, 0)  ; \narray_pg__Row rows = "
               "pg__res_to_rows(res); \n\n// TODO preallocate \narray arr_%.*s "
               "= new_array(0, 0, sizeof(%.*s));  \nfor (int i = 0; i < "
               "rows.len; i++) { \n	pg__Row row = "
               "*(pg__Row*)array__get(rows, i); \n	%.*s %.*s; \n	%.*s "
               "\n	_PUSH(&arr_%.*s, %.*s, %.*s2, %.*s);  \n} \n",
               p->sql_i, params_gen.len, params_gen.str, q.len, q.str, p->sql_i,
               tmp.len, tmp.str, table_name.len, table_name.str, table_name.len,
               table_name.str, tmp.len, tmp.str,
               strings__Builder_str(obj_gen).len,
               strings__Builder_str(obj_gen).str, tmp.len, tmp.str, tmp.len,
